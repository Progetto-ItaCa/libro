...
\begin{example}[Categorie discrete e codiscrete]\label{ex_fun_co_disc}\index{Categoria!--- discreta}\index{Categoria!--- codiscreta}\index{Funtore!--- categoria discreta}\index{Funtore!--- categoria codiscreta}
	Mandare un insieme \(A\) nella sua categoria discreta \(A^\delta\) di \ref{ex_cat_discreta} è un funtore \((\blank)^\delta \colon \ctSet \fun \ctCat\): ogni funzione \(f \colon A \to B\) induce un funtore, tautologico, \(f^\delta \colon A \to B\) che agisce come \(f\) sugli oggetti, ed è univocamente determinato dalla richiesta che \(f^\delta(\id_a) = \id_{fa}\), imposta da \ref{p_1}.

	Similmente, mandare un insieme \(A\) nella sua categoria codiscreta \(A^\chi\) è un funtore \(\ctSet \fun\ctCat\); qui è meno immediato che esista un funtore \(f^\chi \colon A^\chi\fun B^\chi\), ma usando la caratterizzazione di un funtore in \ref{def_alternativa_funtore} è sufficiente trovare una famiglia di funzioni
	\[\begin{tikzcd}
			f^\chi_{aa'} : \Hom{A^\chi}(a,a') \ar[r] & \Hom{B^\chi}(fa, fa')
		\end{tikzcd}\]
	e l'unica freccia \(u_{aa'} \colon a\to a'\) deve per forza essere mandata nell'unica freccia \(v_{fa,fa'} \colon fa \to fa'\) in \(B^\chi\).
\end{example}
\begin{example}\label{dom_e_cod}\index{Funtore!--- dominio}\index{Funtore!--- codominio}
	...
	\[\dmFun{d}{\ctC/X}{\ctC}\]
	che manda ogni oggetto \(f \colon A\to X\) di \(\ctC/X\) nel suo dominio \(A\).

	Similmente, esiste un funtore `codominio'
	\[\dmFun{c}{X/\ctC}{\ctC}\]
	che manda ogni oggetto \(f \colon X\to A\) nel suo codominio \(A\).
\end{example}
\begin{example}\index{Funtore!--- insieme delle parti}\label{ex_fun_parti}
	Ad ogni insieme possiamo associare il suo \emph{insieme delle parti}, l'insieme \(\pow X\) delle funzioni \(\chi \colon X \to \{0,1\}\); questo definisce l'azione sugli oggetti di un funtore \(\ctP \colon \ctSet\fun\ctSet\),	e in effetti, di due, uno covariante e uno controvariante:
	\begin{itemize}
		\item il \emph{funtore potenza covariante}, che manda una funzione \(f \colon X\to Y\) nella funzione
		      \[\dmFun{\ctP_* f}{\pow X}{\pow Y}\]
		      definita mandando \(\chi \colon X\to \{0,1\}\) nella funzione \[\ctP_* f(\chi)(y) \defeq  \begin{cases}
				      1 & \exists x . (y = fx) \land (\chi(x) = 1) \\
				      0 & \text{altrimenti}                        \\
			      \end{cases}\]
		\item il \emph{funtore potenza controvariante}, che manda una funzione \(f \colon X\to Y\) nella funzione
		      \[\dmFun{\ctP^* f}{\pow Y}{\pow X}\]
		      definita mandando \(\varpi \colon Y \to \{0,1\}\) nella funzione \(\ctP^* f(\varpi)(x) \defeq  \varpi(f(x))\).
	\end{itemize}
...
\begin{remark}[Ordini come categorie]\label{ord_come_cat}\index{Preordine}\index{Preordine!---i come categorie}
	...
	È usuale non denotare in alcun modo particolare questo funtore, cosicché `considerare la categoria \((P,\le)\)' ha come unico significato possibile considerare la categoria \(t(P,\le)\) i cui oggetti sono \(p\in P\) e dove c'è esattamente una freccia \(p\to p'\) se e solo se \(p\le p'\).
\end{remark}
\index{Funtore!--- pieno}
...
\begin{example}[Le componenti connesse di un (multidi)grafo]\index{Funtore!--- componenti connesse di un grafo}\index{aaa_pi0@\(\pi_0\)|see {Componenti connesse}}\index{Componenti connesse!--- di un grafo}\label{ex_fun_cpt_conn}\index{Grafo}
	...
	che su un omomorfismo di multidigrafi \(f \colon \ctG \to\ctH\) è definita mandando la componente connessa di \(v\in V_\ctG\) nella componente connessa di \(f_V(v)\in V_\ctH\) (è immediato verificare dal fatto che \(f_E(e \colon x\to y)=f_Ee \colon f_Vx\to f_Vy\) che questa è una buona definizione, cioè che \(f_V\) scende al quoziente).
\end{example}
\begin{example}[La categoria libera associata a un grafo]\index{Funtore!--- categoria libera}
	...
	Un omomorfismo di grafi \((f_E,f_V) \colon \ctG \to \ctH\) infatti è il dato sufficiente per definire un funtore \(F(f_E,f_V) \colon \bfF\ctG \fun F\ctH\) sugli oggetti (con \(f_V\)) e sulle frecce generatrici (con \(f_E\)), mandando la successione di lati contigui \([\tup en;]\) in \(\bfF\ctG\) in \([\tup {{f_E}e}n;]\) in \(F\ctH\) (se \(n=0\) questo significa che \(F(f_E,f_V)\emptyList = \emptyList\)).
\end{example}
\begin{example}\label{ex_grafi_refl}\index{Grafo!--- riflessivo}
	Un (multidi)grafo \(\ctG=(E,V)\) si dice \emph{riflessivo} quando le funzioni di dominio e codominio \(s,t\) ammettono una \emph{sezione} comune; ciò significa che esiste una funzione \(j \colon V \to E\) con la proprietà che \(s\cmp j = t\cmp j = \id_V\).

	Questa condizione si traduce nel fatto seguente: in un grafo riflessivo ogni vertice ammette un \emph{cappio} cioè per ogni vertice esiste un lato \(jv \colon s(jv)=v \to v=t(jv)\). Ad ogni grafo è possibile associare un grafo riflessivo in due modi complementari:
	\begin{itemize}
		...
		\item l'\emph{inviluppo riflessivo} \(\widetilde{\ctG}\) di \(\ctG\) si ottiene definendo un nuovo grafo che ha lo stesso insieme di vertici di \(\ctG\) e l'unione disgiunta \(E+V\) per insieme dei lati; le nuove funzioni dominio e codominio di \(\widetilde\ctG\) sono definite da
		      \[\tilde s = \begin{cases}
			      ...
			      \tilde t = \begin{cases}
				      ...
				      v \mapsto v
			      \end{cases}\]
		      Evidentemente l'inclusione \(j \colon V \mono E+V\) è la sezione comune di \(\tilde s,\tilde t\).
	\end{itemize}
	...
	dove la categoria \(\cate{r}\ctdGph\) è piena in \(\ctdGph\) e ha per oggetti i grafi riflessivi. Evidentemente un omomorfismo di grafi \(f=(f_V,f_E) \colon \ctG \to \ctH\) induce un omomorfismo \(f^r \colon \ctG^r \to \ctH^r\) per restrizione (perché se \(v\) è tale che esiste un cappio \(e \colon v \to v\), allora \(f_Ee \colon f_Vv\to f_Vv\) è a sua volta un cappio). Le proprietà di funtorialità sono ovvie. Per quanto riguarda l'inviluppo riflessivo, resta indotto anche un omomorfismo di grafi \(\tilde f\) tale che i diagrammi di \ref{ex_cat_grafi} siano commutativi:
...
\begin{example}[Realizzare \(\ctMat\) in \(\ctVect\)]\index{Funtore!--- matrici reali}
	...
	\[\dmFun{\dim}{\ctVect}{\ctMat}\]
	che assegna a uno spazio vettoriale \(V\) la sua dimensione (rispetto a una base scelta), e ad una mappa \(\bbF\)-lineare \(f \colon V\to W\) tra spazi vettoriali la matrice di \(f\) nelle basi scelte su \(V\) e \(W\).

	Più precisamente, la base \(b(V)\) è stata scelta per determinare un isomorfismo \(\theta_V \colon V \to \bbF^n\) (se \(n=\dim_\bbF V)\), e il quadrato di applicazioni lineari
...
\end{example}
\begin{example}[Successioni simmetriche, specie combinatorie]\index{Funtore!--- specie combinatoria}\index{Specie combinatoria}\index{Successione simmetrica|see {Specie combinatoria}}\label{ex_fun_specie}
	Si ricordi la definizione della categoria \(\ctBij\) in \ref{ex_cat_gruppoide_naturali}; un funtore
	\[\dmFun{f}{\ctBij}{\ctSet}\]
	...
	\end{itemize}
	Il funtore \(f\) si dice una \emph{specie combinatoria} e la famiglia \(f(n)=X_n\) è una \emph{successione simmetrica}. Questo tipo di funtori, molto ricchi di struttura e di proprietà interessanti, trovano applicazioni in combinatoria enumerativa \cite{bergeron1998combinatorial}, teoria delle rappresentazioni \cite{aguiar2010monoidal}, geometria e topologia \cite{fresse-operads} (quando invece che insiemi gli \(X_n\) sono spazi vettoriali), logica e informatica teorica \cite{Gambino2017,Hasegawa2002}. Denotando con \(\# S\) la cardinalità di un insieme, ad ogni specie combinatoria \(f \colon \ctBij \fun\ctSet\) tale che ogni \(f(n)\) sia finito possiamo associare la serie formale \(F(t)\in\mathbb{N}\llbracket t\rrbracket\) definita come \(F(t) \defeq \sum_{n\ge 1} \frac{\# X_n}{n!}t^n\). Questa si chiama la \emph{funzione generatrice} della specie combinatoria \(F\), e le sue proprietà algebriche o analitiche (qualora \(F(t)\) converga) sono in corrispondenza diretta con le proprietà del funtore \(f\).
\end{example}
\begin{example}[Moduli sinistri; moduli destri; bimoduli]\index{Moduli su un anello!--- come funtori}\index{Modulo}
	...
	consiste di un gruppo abeliano \(M\) su cui è definita un'azione \(a\) del monoide \((R,\cdot,1)\) mediante mappe bilineari. L'azione \(a \colon R\times M\to M\) soddisfa cioè le proprietà
	\begin{itemize}
...
\begin{example}[Spazio delle orbite di un \(G\)-insieme]\index{Funtore!--- orbite di un \(G\)-insieme}\label{ex_fun_orbite}
	...
	che manda il \(G\)-insieme \(X\) nell'insieme \(X/G\). Affinché questo sia vero, una funzione equivariante \(f \colon X\to Y\) deve indurre una funzione ben definita tra gli spazi delle orbite \(X/G\) e \(Y/G\), in modo che gli assiomi \ref{p_1} e \ref{p_2} siano soddisfatti. È facile verificare che \(f/G \colon X/G \to Y/G \colon [x]_G\mapsto [f(x)]_G\) ha tutte queste proprietà. (In particolare, se chiamiamo \(\pi_X \colon X\to X/G\) la funzione \(x\mapsto [x]_G\) che manda ogni elemento nella sua orbita, \(f/G\) è l'unica funzione tale che \(f/G(\pi_X(x)) = \pi_Y(f(x))\).)
\end{example}
\begin{example}\index{Funtore!--- gruppo delle unità}
	...
	Infatti ogni omomorfismo di anelli \(f \colon R\to S\) induce un omomorfismo di gruppi \(f^\times \colon R^\times \to S^\times\), dato che se \(u\in R\) è invertibile, tale è anche \(f(u)\) (e \(f(u)^{-1}=f(u^{-1})\)).
\end{example}
\begin{example}\index{Funtore!--- di abelianizzazione}
	...
	di abelianizzazione. Un omomorfismo di gruppi \(f \colon G \to H\) infatti scende al quoziente ad un omomorfismo \(f^\aa \colon G^\aa \to H^\aa\) tale che
	\[
		\begin{tikzcd}
...
\begin{example}\index{Funtore!--- campo dei quozienti}
	...
	(il dominio è la categoria degli anelli integri, il codominio è la sottocategoria -piena- di \(\ctcRing\) i cui oggetti sono i campi). Infatti, ogni omomorfismo di anelli \(f \colon R \to S\) che siano domini di integrità induce un omomorfismo di anelli \(Q(f) \colon Q(R) \to Q(S)\) definito da \(f(\frac pq) = \frac{fp}{fq}\).
\end{example}
\begin{example}[I gruppi lineari come funtori]\label{ex_fun_GLn}\index{Funtore!--- gruppo generale lineare}\index{GL|see {Gruppo generale lineare}}\index{Gruppo generale lineare}
	...
	Per vederlo, basta definire un omomorfismo di gruppi \(\GL_n(f) \colon \GL_n(R)\to \GL_n(S)\) dato un omomorfismo di anelli \(f \colon R\to S\); ragioniamo nel caso \(n=2\) per comodità, e il caso generale è completamente analogo: la corrispondenza
...
\begin{example}\label{ker_e_coker}\index{Funtore!--- nucleo e conucleo}\index{ker|see {nucleo}}\index{coker|see {conucleo}}
	Data una applicazione lineare \(f \colon V\to W\) tra \(k\)-spazi vettoriali, il suo \emph{nucleo} consiste del sottospazio \(\ker f\le V\) di tutti i vettori \(v\in V\) tali che \(f(v)=0\). Questa assegnazione è un funtore
	...
	Dualmente, il \emph{conucleo} di \(f \colon V\to W\) è lo spazio vettoriale quoziente \(W/fV\), e l'assegnazione \(f\mapsto \coker f\) è un funtore
...
\begin{example}\index{Funtore!--- di compattificazione}
	...
	\begin{itemize}
		...
		\item \(X\mapsto X_\infty\) è un funtore
		      ...
		      (la cui immagine però è la sottocategoria degli spazi compatti), che induce, per ogni mappa propria \(f \colon X\to Y\), una mappa propria tra le loro compattificazioni
		      \[\begin{tikzcd}
...
\begin{example}\label{fun_ex_omoto_omolo}\index{Funtore!--- omotopia}\index{Funtore!--- omologia}
	...
	Ad ogni spazio topologico puntato \((X,x_0)\) si può associare l'insieme delle classi di omotopia di mappe \((S^1, \copmap[r] 10)\to (X, x_0)\) che preservano il punto base; tale insieme è un gruppo, e dato che la relazione di omotopia è una \emph{congruenza} rispetto alla composizione (ossia che, se \(f\simeq g\) allora ogni volta che ciò ha senso si ha \(h\cmp f\cmp k\simeq h\cmp g\cmp k\)), è anche un funtore
	\[\dmFun{\pi_1}\ctTop\ctGrp\]
	e in effetti, dato che mappe omotope \(f\simeq g \colon X\to Y\) inducono lo stesso omomorfismo \(f_* = g_* \colon \pi_1(X,x_0) \to \pi_1(Y,y_0)\), un funtore
	\[\dmFun{\pi_1}\ctHoTop\ctGrp\]
	dove \(\ctHoTop\) è la categoria dell'omotopia di \ref{ex_cat_hotop}.

	Similmente (per chi avesse già familiarità con queste definizioni), i gruppi di omotopia superiore \(\pi_n(X,x_0) \defeq [(S^n,\copmap[r] 1{\mathbf{0}}), (X,x_0)]\) definiscono dei funtori\footnote{Pensando \(S^n\subseteq\bbR^{n+1}\), come il sottospazio \(\{x\in\bbR^{n+1}\mid \|x\|^2=1\}\) il suo punto base è \(\copmap[r]1{\mathbf 0}=(1,0,\dots,0)\). Sebbene al giorno d'oggi questa definizione si dia in generale per ogni indice \(n\ge 0\), per diverso tempo tutto l'interesse in questo genere di oggetti fu concentrato sul caso \(n=1\), quando già Poincaré definisce il \emph{gruppo fondamentale} di uno spazio \(X\) come l'insieme delle classi di omotopia di funzioni continue \(f \colon S^1 \to X\). Si credeva, a torto, che la struttura dei gruppi \(\{\pi_n(X)\mid n\ge 2\}\) fosse di poco interesse, dato che essi sono tutti abeliani: si veda \cite{FreudenthalDSB},
	\begin{quote}
	...
	Per una definizione precisa dei gruppi di omologia e coomologia singolare rimandiamo chi legge a testi classici come \cite{Cartan1956,Rotman1979,MacLane1965} e a \cite[pp. 5--6]{Vick73a}.\index{Gruppo!--- di omologia}\index{Gruppo!--- di coomologia}\index{Coomologia}\index{Omologia}
	% 	Per l'omologia, va mostrato che ogni omomorfismo tra complessi \(f_\bullet \colon (A_\bullet,d_\bullet) \to (B_\bullet,d'_\bullet)\) induce un omomorfismo di gruppi abeliani in maniera che valgano le proprietà di funtorialità; si definisce
...
\begin{definition}\label{def_insieme_simpliciale}
	...
	Un \emph{insieme simpliciale} consta di un funtore \(X_* \colon \bsDelta^\op\fun\ctSet\), dove \(\bsDelta\) è la categoria dei simplessi di \ref{rmk_delta_e_deltaPlus}.
	...
	\begin{itemize}
		...
		\item Una famiglia di funzioni \(d_i^n \colon X_n\to X_{n-1}\), dette \emph{facce}, per \(0\leq i\leq n\); la faccia \(d_i^n(t)\) di un \(n\)-simplesso \(t\) è detta \emph{\(i\)-esima faccia} di \(t\);
		\item Una famiglia di funzioni \(s_i^n \colon X_n\to X_{n+1}\), dette \emph{degenerazioni}, per \(0\leq i\leq n\).
	\end{itemize}
	Questi dati sono soggetti (per funtorialità) alle \emph{identità simpliciali}: se \(X(\delta_n^i) \defeq  d_i^n\) e \(X(\sigma_n^i) \defeq  s_i^n\), allora
	\begin{gather}
...
\begin{definition}[Realizzazione topologica di simplessi]\label{def_real_geom}\index{Simplesso!realizzazione topologica di ---}\index{Funtore!realizzazione topologica}
	...
	\[\textstyle\bigl\{(t_0,t_1,\dots,t_n)\in\bbR^{n+1} \mid t_i\ge 0, \sum_{i=0}^n t_i = 1\bigr\}\]
	...
	Ora, per dimostrare che la realizzazione topologica induce un funtore, va mostrato che una funzione monotòna \(f \colon [m]\to [n]\) in \(\bsDelta\) induce una funzione continua \(|f| \colon |\Delta[m]|\to |\Delta[n]|\). Dato che, come si è visto in \ref{rmk_combinatoria_simplessi}, \(f\) risulta da una composizione \(\delta^{i_1} \cmp\dots\cmp \delta^{i_k}\cmp \sigma^{j_1} \cmp\dots\cmp \sigma^{j_h}\), essenzialmente unica, di cofacce e codegenerazioni, è sufficiente dimostrare che queste ultime inducono tutte funzioni continue. La tesi seguirà dal definire \(|f|\defeq |\delta^{i_1}|\cmp\dots\cmp|\delta^{i_k}|\cmp|\sigma^{j_1}|\cmp\dots\cmp|\sigma^{j_h}|\).
...
\begin{definition}\label{def_insieme_globulare}\index{Insieme!--- globulare}\index{Insieme globulare}
	...
	\begin{itemize}
		...
		\item Una famiglia di funzioni \(s^n,t^n \colon G_n\to G_{n-1}\), dette \emph{dominio} e \emph{codominio} \(n\)-dimensionale.
	\end{itemize}
...
\begin{example}\index{Funtore!--- quoziente di uno pseudoinsieme}
	...
	poiché ogni omomorfismo di pseudoinsiemi \(f \colon (X,\sim_X)\to (Y,\sim_Y)\) soddisfa esattamente la proprietà necessaria a indurre un'unica funzione \(qf \colon X/_{\!\sim_X}\to Y/_{\!\sim_Y}\), che manda la classe di equivalenza \([x]_X\) nella classe di equivalenza \([fx]_Y\) (si dimostrino per esercizio le identità funtoriali: \(q(\id_{(X,\sim)})=\id_{X/_{\!\sim}}\) e \(q(g\cmp f)=qg\cmp qf\)).
\end{example}
...
\begin{esercizi}
	...
	che manda \((X,w)\mapsto \{x\in X\mid |x|\le \lambda\}\), con particolare attenzione alle scelte \(\lambda=0,1,\infty\). Il funtore \(B_\lambda\) è pieno? Fedele? Definire funtori \(W_\lambda \colon \ctSet \fun\cate{wSet}\) nella direzione opposta (sono pieni, fedeli?) tali che se \(\singleton\) denota un qualsiasi singoletto, \(B_\lambda X\) si identifica con l'insieme \(\Hom(W_\lambda\singleton,(X,w))\).
	...
	\item \label{gfdpgubai_4} Si costruisca, tramite il differenziale, un funtore simile a quello di \ref{exa_derivata_funtore}, ma sulla categoria \(\cate{Mfd}\) delle varietà e funzioni lisce, che sugli oggetti assegna ad una varietà liscia \(M\) il suo fibrato tangente \(\text{T} M\); tale \(\text{T} M\) è naturalmente equipaggiato con una mappa liscia \(p_M \colon TM\to M\): il funtore \(\text{T}\) è di tipo \(\cate{Mfd} \to \cate{Mfd}^\to\)? Ovvero, esiste un quadrato commutativo
	\[\xymatrix{
		\text{T} M \ar[r]^{\text{T} f}\ar[d]_{p_M} & \text{T} N \ar[d]^{p_N}\\
		M \ar[r]_f & N
		}\]
	indotto da una mappa liscia \(f \colon M\to N\) di varietà?
	\item \label{exe_cpt_conn} Definiamo il funtore delle componenti connesse di una categoria
	\[\dmFun{\pi_0^\ctCat}{\ctCat}{\ctSet}\]
	come la composizione del funtore \(\pi_0 \colon \ctGph \fun\ctSet\) di \ref{ex_fun_cpt_conn} e del funtore \(U \colon \ctCat \fun\ctGph\) che restituisce il grafo sottostante a una categoria. Diciamo che una categoria \(\ctC\) è connessa quando lo è \(U\ctC\), cioè quando \(\pi_0(U\ctC)\) ha un solo elemento.
