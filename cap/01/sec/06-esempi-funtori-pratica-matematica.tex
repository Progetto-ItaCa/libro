...
\begin{example}\label{ex_grafi_refl}\index{Grafo!--- riflessivo}
	...
	\begin{itemize}
		\item il \emph{cuore riflessivo} di un grafo \(\ctG\) consiste del massimo sottografo \(\ctG^r \subseteq\ctG\) generato da quei vertici che ammettono un cappio; formalmente, si considerano come vertici di \(\ctG^r\) solo i vertici di \(\ctG\) tali che esiste un lato \(e\in E\) con \(s(e)=t(e)=v\), e come lati di \(\ctG^r\) tutti i lati di \(\ctG\) tra questi ultimi. (Farsi dei disegni, e confrontarli con la Figura \ref{} che contiene un semplice esempio di inviluppo e cuore riflessivo di un grafo dato.)
...
\begin{figure}
	\begin{center}
		...
		\end{tikzpicture}
	\end{center}
	\caption{Esempio di cuore e inviluppo riflessivo di un grafo (semplice, cioè senza lati multipli tra due vertici fissati).}
\end{figure}
...
\begin{example}\label{fun_ex_omoto_omolo}\index{Funtore!--- omotopia}\index{Funtore!--- omologia}
	...
	Per una definizione precisa dei gruppi di omologia e coomologia rimandiamo chi legge a testi classici come \ref{Cartan1956,Rotman1979} e a \cite[pp. 5--6]{Vick73a}.\index{Gruppo!--- di omologia}\index{Gruppo!--- di coomologia}\index{Coomologia}\index{Omologia}
