...
\begin{definition}[Isomorfismo]\label{def_isomorfismo}\index{Isomorfismo}\index{Morfismo!isomorfismo}
	Dati due oggetti \(X\) e \(Y\) in una categoria \(\ctC\), un \emph{isomorfismo} o una \emph{freccia iso} tra \(X\) e \(Y\) consiste di una coppia di frecce \(f:X\to Y\) e \(g:Y\to X\) in direzioni opposte, tali che \(g\cmp f=\id_X\) e \(f\cmp g=\id_Y\).
	...
	Se esiste un isomorfismo tra \(X\) e \(Y\) diciamo che i due oggetti sono \emph{isomorfi} (spesso tralasciando mediante quale coppia di frecce, dato che è chiaro dal contesto) e scriviamo \(X\iso Y\).\footnote{Da un punto di vista costruttivo, \(X\iso Y\) consiste di una quadrupla \((f,g, i,j)\) dove \(f : X\to Y, g: Y\to X\) e \(i,j\) sono rispettivamente due dimostrazioni, che \(f\cmp g= \id_Y\) e che \(g\cmp f=\id_X\), però non insisteremo mai su questo dettaglio. Spesso diremo che `\(f\) è un isomorfismo' sottintendendo l'esistenza di \(g\), e viceversa.}
\end{definition}
...
\begin{examples}\label{esempi_di_iso}\index{Isomorfismo}
	...
	\begin{enumtag}{ei}
		\item \label{ei_1} È evidente che in ogni categoria \(\ctC\), tutte le identità \(\id_X : X\to X\) sono isomorfismi che coincidono col loro inverso, e che la composizione di isomorfismi \(f_1\cmp f_2\) è un isomorfismo di inverso \(g_2\cmp g_1\), cioè \((f_1\cmp f_2)^{-1} = f_2 {-1}\cmp f_1^{-1}\).
...
\begin{definition}\label{def_automorfismo}\index{Automorfismo}\index{Morfismo!auto---}
	Un \emph{automorfismo} è un endomorfismo (una freccia da un oggetto a sé stesso) che è un isomorfismo (non si usa mai la locuzione `freccia auto', anche se sarebbe coerente con la nomenclatura di questa sezione e di \ref{sec_monoepi}). In un gruppo \((G,\cdot)\) guardato come una categoria \(\susp(G,\cdot,1)\) tutte le frecce/morfismi \(g : \star\to\star\) sono automorfismi.
\end{definition}
...
\begin{definition}[Il gruppoide fondamentale di uno spazio]\label{es_gruppoide_fondamentale}\index{Categoria!Gruppoide fondamentale}\index{Gruppoide!--- fondamentale}
	...
	\begin{itemize}
		...
		\item per frecce \(x\to y\) le classi di omotopia di cammini \(\gamma : [0,1] \to X\) di estremi \(x,y\), cioè le classi di omotopia di funzioni continue \(\gamma\) tali che \(\gamma(0)=x, \gamma(1)=y\).
	\end{itemize}
