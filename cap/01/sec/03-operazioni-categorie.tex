...
\begin{example}\index{Sottocategoria}\index{Categoria!sotto---}
	...
	\begin{itemize}
		...
		\item Per contro, la sottocategoria di \(\cate{T}2\ctTop\) fatta dagli spazi T2 e \emph{compatti} ha come scelta comune di frecce la classe delle mappe \(f \colon X\to Y\) continue e \emph{proprie}, tali cioè che la controimmagine di un sottoinsieme compatto \(K\Subset Y\) mediante \(f\) sia un sottoinsieme compatto di \(X\).
	\end{itemize}
\end{example}
\begin{definition}\label{def_cat_prodotto}\index{Categoria!--- prodotto}
	...
	\begin{itemize}
		...
		\item una freccia \((C,D)\to(C',D')\) è una coppia \((f,g)\) dove \(f\colon C\to C'\) è una freccia di \(\ctC\) e \(g\colon D\to D'\) è una freccia di \(\ctD\);
...
\begin{figure}[h]
	\begin{center}
		...
		\end{tikzpicture}
		\caption{Il prodotto e la somma di due categorie: il quadrato commutativo generico (si veda \ref{ex_quadcuboncubo}), in rosso, e la spanna generica (si veda \ref{ex_spancospan}), in blu. Si noti che \(\ctC\times\ctD\) contiene tre copie di \(\ctC\) e quattro copie di \(\ctD\); si noti anche che le facce dei parallelepipedi che contengono frecce di diverso colore sono quadrati commutativi per definizione (perché?).}\index{Spanna}
...
\begin{definition}\label{def_cat_opp}\index{Categoria!--- opposta}
	...
	\begin{itemize}
		...
		\item come frecce, una freccia \(f^\op\colon Y\to X\) per ogni freccia \(f\colon X\to Y\) di \(\ctC\);
...
\begin{proposition}\index{Dualità}\index{Dualità!principio di ---}
	...
	\begin{itemize}
		\item per costanti i simboli \(A,B,C,\dots \colon \ctC_0\) degli oggetti e i simboli \(f,g,h,\dots \colon \ctC_1\) delle frecce (in particolare \(A,B,C,\dots\) hanno \emph{sorte} \(\ctC_0\) e \(f,g,h,\dots\) sorte \(\ctC_1\));
		\item le funzioni dominio e codominio \(d,c \colon \ctC_1\to\ctC_0\colon f\mapsto d(f),c(f)\);
		\item la funzione di costruzione dell'identità \(\id \colon \ctC_0\to \ctC_1 \colon A\mapsto \Id[A]\);
		\item l'operazione di composizione \(\blank\cmp\blank \colon \ctC_2 \to \ctC_1\) dove ricordiamo che
		      \[\ctC_2 \defeq  \{(f,g) : \ctC_1\times\ctC_1\mid c(f) = d(g)\}\subseteq\ctC_1\times\ctC_1\]
	\end{itemize}
	...
	Data una formula \(\Sigma\) nel linguaggio \(\mathsf{CT}\), possiamo costruire la formula duale per sostituzione, \(\Sigma^\op\defeq \Sigma[d/c,c/d,\cmp/\cmp^\op]\)\footnote{È chiaro cosa lo scambio di \(d,c\) significhi: prima si sostituisce \(d\) con un simbolo vergine \(c'\) e \(c\) con un simbolo vergine \(d'\), e poi si sosituisce \(c'\) con \(c\) e \(d'\) con \(d\):
		\[\Sigma \squig \Sigma[d/c',c/d']\squig \Sigma[c'/c,d'/d] =:\Sigma^\op\]
...
\begin{remark}\index{Dualità}
	...
	\end{itemize}
\end{remark}
\begin{proposition}\label{rel_autoduale}\index{Dualità}\index{Rel@$\ctRel$}
...
\begin{example}\index{Categoria!--- autoduale}
	Chiaramente, la categoria vuota \(\ctInit\) è autoduale (ossia \(\ctInit^\op=\ctInit\)) e altrettanto è vero per la categoria singoletto \(\ctTerm\) e per la categoria discreta \(A^\delta\) di \ref{ex_cat_discreta}.
...
\begin{example}\label{rel_autoduale}\index{Categoria!--- autoduale}
	Un esempio non banale di categoria autoduale è \(\ctRel\) di \ref{ex_cat_rels}. Dato che una relazione \(R \colon A\to B\) è per definizione un sottoinsieme \(R\subseteq A\times B\), e dato che \(A\times B\) è in biiezione con \(B\times A\) (e che questa biiezione \(\sigma \colon A\times B \to B\times A\) è una involuzione, cioè \(\sigma\cmp\sigma = 1_{A\times B}\)), la differenza tra `dominio' e `codominio' di \(R\) è trascurabile, dato che nel sottoinsieme \(R \subseteq A\times B\) è contenuta esattamente la stessa informazione che nel sottoinsieme \(\sigma(R)\subseteq B\times A\).

	Questa idea, del tutto ovvia, necessita tuttavia di parecchio linguaggio per essere resa veramente precisa: la nozione di \emph{trasformazione naturale} \ref{def_nat} e di \emph{equivalenza di categorie} \ref{funtore_equcat} costruiranno in \ref{rel_autoduale} un'equivalenza \(\ctRel\simeq \ctRel^\op\) a partire dalla famiglia di involuzioni \(\sigma_{AB} \colon \Hom{\ctRel}(A,B) = \pow {A\times B} \to \pow {B\times A} = \Hom{\ctRel}(B,A)\).
\end{example}
\begin{definition}[Incubi e succube sopra un oggetto]\label{def_cat_slice}\index{Categoria!--- incubo}\index{Categoria!--- succuba|see {incubo}}\index{Categoria!--- sopra un oggetto}\index{Categoria!--- sotto un oggetto|see {sopra un oggetto}}
	...
	La \emph{categoria incubo} sopra \(X\), o semplicemente \emph{categoria sopra} \(X\), denotata \(\ctC/X\), è definita come segue:\footnote{Nel folklore medievale un \emph{incubo} è un demone maschile, che premendo sul petto dell'addormentato causa le paralisi notturne.	Una \emph{succuba}, invece, è un'entità demoniaca femminile, che appare nei sogni soggiogando il sognatore (rigorosamente uomo) e lo obbliga a un rapporto sessuale.}
	\begin{itemize}
		\item gli oggetti sono coppie \((A,p)\) dove \(A\) è un oggetto di \(\ctC\) e \(p \colon A\to X\) è una freccia di \(\ctC\);
		\item le frecce \((A,p)\to(B,q)\) sono frecce \(f\colon A\to B\) di \(\ctC\) tali che \(q\cmp f=p\), ovvero tali che il seguente diagramma commuta.
		      \[
	...
	La \emph{categoria succuba sotto \(X\)}, o semplicemente \emph{categoria sotto} \(X\), denotata \(X/\ctC\), ha
	\begin{itemize}
		\item per oggetti, coppie \((A,p)\) dove \(A\) è un oggetto di \(\ctC\) e \(p\colon X\to A\) è una freccia di \(\ctC\);
		\item per frecce \((A,p)\to(B,q)\), frecce \(f\colon A\to B\) di \(\ctC\) tali che \(q=f\cmp p\), ovvero tali che il seguente diagramma commuta.
		      \[
			      \begin{tikzcd}[column sep=small]
...
\begin{example}\label{def_spazi_pun_succuba}\index{Categoria!--- degli spazi puntati}\index{Spazi topologici}
	La categoria degli insiemi puntati \(\ctSet_*\) costruita nell'Esempio \ref{ex_cat_puntati} si può vedere equivalentemente come la categoria succuba \(1/\ctSet\), dove \(1\) è un insieme con un solo elemento: ciò perché la scelta di una funzione \(a_0 \colon 1\to A\) consiste esattamente della scelta di un elemento \(a_0\in A\).
...
\begin{example}[Colorazioni di grafi]\label{ex_grph_colorati}\index{Categoria!--- dei grafi colorati}
	Una \emph{\(n\)-colorazione} di un grafo non diretto come in \ref{ex_cat_grafi_nondiretti} consiste della scelta di una colorazione \(c \colon V \to \{\iter n\}\), che assegna un colore ad ogni vertice \(V\) di un grafo \(\ctG=(E,V)\), in modo tale che due vertici collegati da un lato non siano dello stesso colore.
	...
	\begin{figure}[h]
		\begin{center}
			...
			\end{tikzpicture}
		\end{center}
		\caption{Una 3-colorazione di un grafo \(\ctG\) a forma di gatto. I vertici di \(\ctG\) sono etichettati da dei colori (gli elementi dell'insieme \(C = \{\yellowDot,\redDot,\blueDot\}\)), di modo che resti definito un omomorfismo \(f \colon \ctG \to\ctK_G\) di grafi non diretti verso il grafo completo costruito sull'insieme \(C\), che estende la colorazione \(c \colon V\to C\). Questo significa: dato un vertice \(v\), \(c(v)=\redDot\) se e solo se \(f(v)=\redDot\) (e lo stesso vale per gli altri colori).}
		\label{fig_n_colorazione_grafo}
	\end{figure}
\end{example}
\begin{example}[Intervalli]\label{ex_pos_slices}\index{Categoria!--- intervallo di un ordine}
	Se \((\bbR,\le)\) denota l'insieme dei numeri reali linearmente ordinato nel modo usuale, ed \(x\in\bbR\) è un numero reale, la categoria \(\bbR/x\) è il semi-intervallo inferiormente illimitato \((-\infty,t]\defeq \{t\le x\}\), e analogamente \(x/\bbR\) è il semi-intervallo \([t,+\infty)\). Più in generale, in ogni insieme parzialmente ordinato \((P,\le)\) guardato come categoria, l'incubo \(P/x\) sopra \(x\in P\) consiste dell'intervallo \(\{t\le x\}\) dotato dell'ordine indotto da \(P\), e similmente accade per \(x/P\). (Se \(a,b\in\bbR\) con l'ordine usuale, e \(a\le b\), si noti anche che \((a/\bbR)/(a\le b)\) è esattamente l'intervallo limitato \([a,b]\).)
\end{example}
\begin{example}[Spazi incubi, fibrati, rivestimenti]\label{ex_fibrati_rivestimenti}\index{Categoria!--- dei rivestimenti}
	...
	\begin{itemize}
		...
		\item i \emph{fibrati localmente banali}, ossia gli oggetti \(p \colon E\to X\) di \(\ctTop/X\) con la proprietà che esiste un ricoprimento aperto \(\{U_i\}\) di \(X\) tale che \(p|_i \colon p^{-1}U_i\to U_i\) sia un fibrato banale in \(\ctTop/U_i\). Equivalentemente, un fibrato è localmente banale se per ogni punto \(x\in X\) esiste un aperto \(U_x\) tale che \(p|_x \colon p^{-1}U_x\to U_x\) sia banale, cioè \(p^{-1}U_x\) è omeomorfo a \(U_x\times A_x\) per qualche \(A_x\in\ctTop_0\).
	\end{itemize}
...
\begin{example}[Spazi affini]\label{ex_affini}\index{Categoria!--- degli spazi affini}
	...
	\begin{itemize}
		\item gli oggetti sono mappe lineari \emph{suriettive} \(A \colon V\to k\);
		\item le frecce sono le stesse di \(\ctVect[k]/k\).
	\end{itemize}
	Ogni oggetto di \(\ctA\) determina uno spazio affine (non vuoto) modellato su \(k\) ottenuto considerando lo spazio vettoriale \(A_0 = \ker A\) la cui somma di vettori agisce in maniera transitiva sull'insieme di punti \(A_1 \defeq  A^{-1}(1_k)\) (\(0_k,1_k\) sono rispettivamente l'elemento neutro additivo e quello moltiplicativo del campo \(k\)).
\end{example}
\begin{example}[Implicazioni in contesto]\label{ex_boole_cpc}\index{Categoria!--- del calcolo proposizionale}
	È noto dai corsi di logica elementare che l'insieme \(\ctB=\{\varphi,\psi,\dots\}\) delle formule ben formate del calcolo proposizionale (classico) forma un'algebra di Boole rispetto all'ordine
...
\begin{definition}\label{def_cat_frecce}\index{Categoria!--- delle frecce}
	...
	\begin{itemize}
		\item come oggetti, le frecce \(f\colon A\to B\) di \(\ctC\);
		\item come frecce \(f\to f'\), coppie di frecce \(p\colon A\to A'\) e \(q\colon B\to B'\) che fanno commutare il seguente quadrato,
		      \[
			      \begin{tikzcd}
...
\begin{definition}[Categoria delle spanne e cospanne]\label{def_span_e_cospan}\index{Categoria!--- degli span}\index{Categoria!--- dei cospan|see {span}}
	...
	\begin{itemize}
		\item Come oggetti, le \emph{spanne} di \(\ctC\), ovvero coppie di frecce \(f,g\) di \(\ctC\) con lo stesso dominio:
		      \[
			      ...
			      \end{tikzcd}
		      \]
		\item Come frecce \((f,g)\to (f',g')\), triple di frecce \(p\colon A\to A'\), \(q\colon B\to B'\) e \(r\colon C\to C'\) che fanno commutare il seguente diagramma,
		      \begin{equation}
			      \begin{tikzcd}[row sep=small]
	...
	\begin{itemize}
		\item Come oggetti, le \emph{cospanne} di \(\ctC\), ovvero coppie di frecce \(f,g\) di \(\ctC\) con lo stesso codominio:
		      \[
			      ...
			      \end{tikzcd}
		      \]
		\item Come frecce \((p,q,r)\colon(f,g)\to (f',g')\), triple di frecce \(p\colon A\to A'\), \(q\colon B\to B'\) e \(r\colon C\to C'\) che fanno commutare il seguente diagramma,
		      \[
			      \begin{tikzcd}[row sep=small]
	...
	\end{itemize}
	Un'altra definizione possibile è quella della categoria \(\Hom{\ctCospan_\ctC}(X,Y)\) di cospanne \emph{a estremi fissati} \(X\) e \(Y\), che ha
	\begin{itemize}
		\item per oggetti le cospanne	\(\pair fg \colon X\xto{f} C\xot{g} Y\) in \(\ctC\),
		\item per frecce tra cospanne	\(\pair fg \colon X\to C\leftarrow Y\) e \(\pair{f'}{g'} \colon X\to C'\leftarrow Y\) le frecce \(r \colon C\to C'\) di \(\ctC\) tali che \(r\cmp f = f'\) e \(r\cmp g=g'\), cioè tali che il diagramma
		      \[\xymatrix@R=1mm{
			      &C\ar[dd]_r & \\
		      ...
		      sia commutativo.
	\end{itemize}
	Si noti che la categoria \(\Hom{\ctCospan_\ctC}(X,Y)\) è una sottocategoria della \(\ctCospan(\ctC)\) definita prima, precisamente fatta dagli oggetti della forma \(X\to C\leftarrow Y\) e dalle frecce dove \(p,q\) sono identità
\end{definition}
	...
	\begin{itemize}
		...
		\item le frecce \(\Hom{(\ctC\star\ctD)}(X,Y)\) sono specificate per casi:
		      \begin{itemize}
			      ...
			      \item se \(X\in\ctC_0\) e \(Y\in\ctD_0\), esiste un'unica freccia \(u_{XY} \colon X \squig Y\); questa unica freccia si chiama l'\emph{eteromorfismo} che connette \(X\) e \(Y\);
...
\begin{definition}[Cono destro e sinistro]\label{def_cono_su_C}\index{Cono!--- destro di una categoria}\index{Oggetto!--- terminale}\index{Oggetto!--- iniziale}
	...
	\end{itemize}
	Si osservi che ci sono inclusioni ovvie \(i^\rhd \colon \ctC \subset \ctC^\rhd\) e \(i^\lhd \colon \ctC \subset \ctC^\lhd\) ottenute mandando un oggetto \(C \in\ctC_0\) nell'oggetto \(C\) di \(\ctC_0 + \{\pm\infty\}\).
\end{definition}
\begin{definition}[Categoria delle frecce ritorte]\label{def_cat_frecce_ritorte}\index{Categoria!--- delle frecce ritorte}
	...
	\begin{itemize}
		\item come oggetti, le frecce	\(f \colon A \to B\) di \(\ctC\), denotate \([A,B;f]\);
		\item come frecce \((u,v) \colon [A,B;f]\leftrightarrows [X,Y;g]\) le coppie \(v \colon B\to Y\) e \(u \colon X \to A\) di frecce di \(\ctC\) tali che il quadrato
		      \[
			      \begin{tikzcd}
	...
	\end{itemize}
	L'identità è chiaramente \((\id_A,\id_B) \colon [A,b;f] \leftrightarrows [A,B;f]\); la composizione è definita incollando i quadrati
	\[
		\begin{tikzcd}
...
\end{figure}
\begin{esercizi}
	\item Sia \(\ctC\) una categoria, \(X\in\ctC_0\) un oggetto, \(f \colon A\to X\) una freccia: dimostrare che il doppio incubo \((\ctC/X)/f\) è un incubo di \(\ctC\) (su quale oggetto?). Questa `riduzione' di un incubo iterato resta vera per l'incubo della succuba \((A/\ctC)/f\), per \(f/(\ctC/X)\), e per \(f/(A/\ctC)\)?
	\item Un \emph{segmento binario} \((n_0,n_1,t,H)\) consiste di una coppia di interi positivi \(n_0 \le n_1\),con una funzione suriettiva e monotòna \(t \colon [n_1] \to [n_0]\), e un sottoinsieme \(H\subseteq [n_0]\). Un omomorfismo tra segmenti binari \((n_0,n_1,t,H) \to (n_0',n_1',t',H')\) consiste di una coppia \((f_0,f_1)\) dove
	\begin{itemize}
		\item \(f_1 \colon [n_1] \to [n_1']\) è una funzione iniettiva e monotòna;
		\item \(f_0 \colon [n_0] \to [n_0']\) è una funzione monotòna;
		\item il diagramma
		      \[\begin{tikzcd}
	...
	\item É possibile rappresentare le relazioni (si veda \ref{ex_cat_rels}) sia come spanne che come cospanne. La prima rappresentazione è molto semplice (l'esercizio consiste nel verificare quanto diciamo): una relazione \(R\subseteq A\times B\) corrisponde a una spanna
	\[
		\begin{tikzcd}
			A & R \ar[r, "b"]\ar[l, "a"']& B
		\end{tikzcd}
