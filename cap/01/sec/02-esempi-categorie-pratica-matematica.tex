...
\begin{example}\label{ex_cat_catena}\index{Categoria!--- catena generica}
 ...
 \begin{itemize}
  ...
  \item C'è un'unica freccia \(i\to j\) in \(\Delta[n]\) se e solo se \(i\le j\), di modo che esista un'unica freccia \(i\to i\) (l'identità) per ogni \(i\in\{\iter[0]{n}\}\), e un'unica freccia \(i\to i+1\) per \(i\in\{\iter[0]{n-1}\}\); in particolare l'unico morfismo \(i\to j\) è ottenuto dalla composizione
        \[i\to i+1\to\dots\to j\]
 \end{itemize}
 \index{Simplesso}
