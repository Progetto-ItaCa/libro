...
\begin{example}\label{ex_cat_discreta}\index{Categoria!--- discreta}
	Dato un insieme \(A\), la \emph{categoria discreta} su \(A\), denotata \(A^\delta\), ha per oggetti gli elementi \(a\in A\), un'unica freccia identità \(\id_a \colon a\to a\) per ogni \(a\), e nessun altro. La composizione è forzata a essere definita solamente tra identità, nel modo più ovvio possibile, e tutti gli assiomi di categoria sono ovviamente soddisfatti.
\end{example}
\begin{example}\label{ex_cat_codiscreta}\index{Categoria!--- codiscreta}
	Dato un insieme \(A\), la \emph{categoria codiscreta} su \(A\), denotata \(A^\chi\) ha per oggetti gli elementi \(a\in A\), per ogni coppia di oggetti \(a,a'\in A\) un'unica freccia \(u_{aa'}\colon a\to a'\), e la composizione è definita da \(u_{a'a''}\cmp u_{aa'}=u_{aa''}\) (in particolare, da questo segue che esiste un unico elemento \(\id_a=u_{aa}\in A^\chi(a,a)\)).
\end{example}
...
\begin{example}\label{ex_cat_freccia}\index{Categoria!--- freccia generica}\index{Freccia generica}
	La categoria \(\genArrow=\{0\to 1\}\) detta \emph{freccia generica} ha due oggetti \(0,1\) e un'unica freccia non identità \(u \colon 0\to 1\) (che di solito non ha nome). La composizione è forzata a essere definita solamente tra le identità ed \(u\), nell'unico modo ovvio, e tutti gli assiomi di categoria sono banalmente soddisfatti.
\end{example}
\begin{example}\label{ex_cat_catena}\index{Categoria!--- catena generica}
	...
	\begin{itemize}
		\item gli oggetti di \(\Delta[n]\) sono gli elementi di \(\{\iter[0]{n}\}\) (ci sono quindi \(n+1\) oggetti, per ogni \(n\ge 1\));\footnote{Si presti attenzione alla differente notazione per due insiemi finiti distinti: \(\bkt n \defeq  \{\iter n\}\), ma \([n] \defeq  \{\iter [0]n\}\) A volte è utile adottare la convenzione per cui quando \(n=-1\), l'insieme \(\{\iter[0]{n}\}\) è vuoto, e quindi la categoria `catena di lunghezza \(0\)' è la categoria vuota \(\ctInit\) di \ref{ex_cat_vuota}.}
...
\begin{example}[Doppia freccia generica]\label{ex_cat_doppiafreccia}\index{Categoria!--- doppia freccia generica}\index{Freccia generica!doppia ---}
	La categoria \emph{doppia freccia generica} \(\genAArrow=\{0\toto 1\}\) ha due oggetti \(0,1\) ed esattamente due frecce non identità e parallele \(s,t \colon 0\to 1\). La composizione è definita, ancora una volta, nell'unico modo ovvio, chiedendo che \(x\cmp \id_0=x=\id_1\cmp x\) per \(x\in\{s,t\}\).

	La doppia freccia è la `forma generica' di un grafo diretto (si veda \ref{ex_cat_libera}), dato che una coppia di funzioni \(s,t\colon E \toto V\) tra due insiemi consta precisamente di un digrafo che ha \(V\) per insieme dei vertici e \(E\) per insieme dei lati.
\end{example}
\begin{example}[Turcasso generico]\label{ex_cat_turcasso}\index{Turcasso}\index{Categoria!--- turcasso generico}
	Sia \(S\) un insieme; la categoria \(\turk\), detta \emph{turcasso generico} (con \(S\) elementi)\footnote{Il nome \emph{turcasso} denota un particolare tipo di faretra in uso agli arcieri turchi, solitamente fissata alla cintura dei soldati anziché portata a tracolla. Boccaccio scrive \cite{boccaccio1831teseide},
		...
		\end{verse}} ha due oggetti \(0,1\), e una freccia \(s \colon 0\to 1\) per ogni \(s\in S\). La composizione è possibile solo tra \(s\in S\) e una identità, nel modo ovvio.

	Chiaramente se \(S=\emptyset\) si ottiene la categoria \(\{0,1\}^\delta\), discreta su \(\{0,1\}\); se \(\#S=2\) si ottiene la doppia freccia generica di \ref{ex_cat_doppiafreccia}. Se \(S\) è finito e \(\#S=n\) possiamo chiamare brevemente la categoria turcasso con \(n\) elementi un \emph{\(n\)-turcasso}, e in effetti è sufficiente parlare di \(\kappa\)-turcassi per \(\kappa\) un qualsiasi cardinale (è quindi evidente cosa sia un \(\aleph_0\)-turcasso).
\end{example}
...
\begin{example}[Spanne e cospanne generiche]\label{ex_spancospan}\index{Categoria!--- spanna generica}
	...
	\(\Lambda^2_0\) ha tre oggetti \(0,1,2\) e due frecce non identità. Si raffigura come segue:\footnote{La scelta del termine si può intendere motivata dal fatto che una spanna è: la distanza (misurata dai codomini delle due frecce verso \(0,1\)) tra punta del pollice e punta del mignolo di una mano adulta estesa; l'arcata di un ponte (il cui punto più alto è l'oggetto \(0\)); ; un arcolaio (le cui braccia sono proprio \(1\leftarrow 0\to 2\);le due braccia di un giogo (in inglese antico \emph{ġespan} era un giogo, un collare o per estensione un bracciale); quest'ultima etimologia è dalla radice proto-indoeuropea \emph{*(s)penh\(_1\)-}, da cui derivano varie parole nel campo semantico di ciò che è capace di ruotare attorno a un asse).}
	\[\begin{tikzcd}
			1 & 0 \ar[r]\ar[l] & 2
		\end{tikzcd}\]
	...
	\begin{itemize}
		\item gli oggetti di \(S^\lhd\) sono gli elementi di \(S^+ \defeq  S\cup \{-\infty\}\);
		\item esiste un'unica freccia \(f_s \colon -\infty\to s\), per ogni \(s\in S\).
	\end{itemize}
	...
	\begin{itemize}
		\item gli oggetti di \(S^\rhd\) sono gli elementi di \(S^+ \defeq  S\cup \{+\infty\}\);
		\item esiste un'unica freccia \(f_s \colon s\to+\infty\), per ogni \(s\in S\).
	\end{itemize}
	...
	\begin{figure}
		\begin{center}
			\begin{tikzpicture}[xscale=1.5]
				...
				%
				\draw[ibmMagenta!20,line width=5mm, rounded corners, cap=round] (a0) -- (a1) -- (a2) -- (a3) -- (a4);
				\draw[ibmBlue!20,line width=5mm, rounded corners, cap=round] (a4) -- (a5) -- (a6) -- (a7);
				%
				\foreach \i in {0,...,7}
				\node (e\i) at (a\i) {$x_\i$};
				\foreach \i in {0,...,6}{
...
\begin{hExample}[La categoria dei circuiti digitali]{tech}\label{ex_cat_circuiti}\index{Categoria!--- dei circuiti}
	...
	\end{itemize}
	Si noti che ogni funzione Booleana \(f \colon \bbB^n\to\bbB^m\) risulta dal `prodotto' di \(m\) funzioni Booleane \emph{semplici} \(\tup fm, \colon \bbB^n\to\bbB\), di modo che
	\[f(\tup xn,)=(f_1(\tup xn,),\dots,f_m(\tup xn,)).\]
	Tra le funzioni Booleane semplici ci sono certamente le proiezioni canoniche \(\pi_{n,i} = \pi_i \colon \bbB^n\to\bbB \colon(\tup xn,)\mapsto x_i\), con le quali è possibile caratterizzare le \(f_i\) su menzionate: \(f_i = \pi_i\cmp f\); è anche possibile definire la mappa diagonale \(\Delta_m\colon\bbB\to\bbB^m\) come
	...
	\[\begin{circuitikz}
		...
		\end{circuitikz}\]
	È possibile comporre in serie le porte logiche, componendo le frecce che esse rappresentano, e giustapporle in parallelo, allo stesso modo in cui è possibile considerare circuiti elettrici in serie e in parallelo per costruire strutture complesse come:
	\[
...
\begin{theorem}\label{circ_thm}\index{Funzione Booleana}
	Ogni funzione Booleana semplice \(f \colon\bbB^n\to\bbB\) è esprimibile come composizione in serie e in parallelo di: proiezioni, mappe diagonali, congiunzioni e negazioni logiche.
\end{theorem}
...
È tuttavia estremamente importante notare che
\begin{quote}
...
\end{example}
Nell'esempio appena fatto, l'ordine in cui si scrivono i fattori di un prodotto cartesiano è importante, perché se \(R \colon A\to B\) è una relazione, vogliamo che \(A\) ne sia il dominio, e \(B\) il codominio. È però vero che gli insiemi \(A\times B\) e \(B\times A\) sono in biiezione canonica, e quindi si potrebbe pensare di definire una categoria \(\ctRel'\) con gli stessi oggetti di \(\ctRel\), ma con frecce da \(A\) verso \(B\) le relazioni \(R\subseteq B\times A\). In effetti, questa è la categoria opposta \(\ctRel^\op\) di \(\ctRel\) (si veda \ref{def_cat_opp}), e tra \(\ctRel^\op\) e \(\ctRel\) esiste un isomorfismo (si veda \ref{rel_autoduale}).
\begin{remark}\label{klext_delle_relazioni}\index{Relazione}\index{Relazione!estensione di ---}
	Ogni relazione \(R \colon A\to B\) induce una funzione \(R^* \colon \pow B\to \pow A\) nel modo che segue: un sottoinsieme \(U\subseteq B\) viene mandato in \(R^*U\defeq \{x\in A\mid \forall u\in U.(x,u)\in R\}\). In effetti, esiste una analoga funzione \(R_* \colon \pow A\to \pow B\), definita da \((V\subseteq A) \mapsto R_*V\defeq  \{y\in B\mid \forall v\in V.(v,y)\in R\}\), e tra le due funzioni \(R^*, R_*\) sussiste la relazione
...
\begin{remark}\label{diff_pres_cat_rel}\index{Categoria!--- delle relazioni}\index{Rel@\(\ctRel\)}
	...
	\[\dmFun RX{\pow Y}\]
	definita mandando \(x\in X\) nell'insieme \(Rx = \{y\in Y\mid xRy\}\), e una tale relazione induce un'unica funzione \(\hat R \colon \pow X\to \pow Y\) definita da \(\hat R(U) = \bigcup_{x\in U}Rx = \{y\in Y\mid \exists x\in U,\, y\in Rx\}\), chiamata \emph{estensione di Kleisli} di \(R\) (il capitolo \ref{cap_monadi}, sulle monadi, chiarirà questa terminologia: Heinrich Kleisli (\(*\)1930-\(\dag\)2011) è stato un matematico svizzero).\index{Kleisli!estensione di ---}\index{Kleisli@Heinrich Kleisli}
	...
	\begin{enumtag}{ek}
		\item \label{ek_1} per ogni insieme \(A\), esiste una \emph{relazione identica} \(\eta_A \colon A\to \pow A\), e \(\hat\eta_A\) è l'identità di \(\pow A\);
		\item \label{ek_2} se \(R \colon B\to \pow A\) è una relazione, allora \(\hat R\cmp \eta_B = R \colon B \to\pow A\);
		\item \label{ek_3} se \(R \colon B\to \pow A\) e \(S \colon C\to \pow B\) sono due relazioni, allora \(\hat S\cmp \hat R = \hat{S\cmp R} \colon C\to \pow A\).
	\end{enumtag}
\end{remark}
...
\begin{example}[Categoria delle matrici in \(\bbF\)]\label{ex_cat_matrici}\index{Categoria!--- delle matrici}
	...
	\begin{itemize}
		...
		\item per frecce \(n\to m\) le matrici con \(m\) righe ed \(n\) colonne a coefficienti in \(\bbF\), ossia:
		      ...
		      dove \(\bbF^n\defeq \bbF \times\dots\times\bbF\) è il prodotto cartesiano iterato di \(\bbF\) con sé stesso \(n\) volte (quindi \(\bbF^0\) contiene il solo vettore nullo), e \(\text{Lin}_{\bbF}(\bbF^n,\bbF^m)\) è l'insieme delle funzioni \(\bbF\)-lineari \(\bbF^n\to\bbF^m\) (che scegliendo su dominio e codominio le basi canoniche, si identifica alle matrici a componenti in \(\bbF\) di taglia \(m\times n\)).
	\end{itemize}
	La composizione è il prodotto di matrici: se \(A \colon n\to m\) e \(B \colon m\to p\) sono matrici di entrate \((A_{ij}\mid 1\le i\le m,1\le j\le n)\) e \((B_{rs}\mid 1\le r\le p,1\le s\le m)\), la loro composizione \(BA \colon n\to p\) è la matrice \(BA\in \matrici pn\bbF\) definita da \((BA)_{ij} = \sum_{k=1}^m B_{ik}A_{kj}\).
...
\begin{example}\label{ex_cat_monoidi}\index{Omomorfismo!---di monoidi}
	Dati due monoidi \(M\) e \(N\), un \emph{omomorfismo di monoidi} \(f\colon M\to N\) è una funzione \(f\colon M\to N\) tale che
	\begin{itemize}
...
\begin{example}\label{ex_cat_sigma_strutture}\index{Categoria!---}
	...
	\begin{itemize}
		...
		\item fissati due modelli \((X,f_\bullet), (Y,g_\bullet)\) un \emph{omomorfismo} di modelli è una funzione \(h \colon X\to Y\) con la proprietà che per ogni \(\omega\in\Omega\) si abbia l'uguaglianza
...
\begin{example}[Categoria dei multidigrafi]\label{ex_cat_grafi}\index{Categoria!--- dei grafi}\index{Grafo}
	...
	\begin{itemize}
		...
		\item Un omomorfismo di (multidi)grafi \((f_E,f_V) \colon (E,V,s,t)=\ctG\to\ctG'= (E', V',s',t')\) consiste in una coppia di funzioni \(f_E, f_V\) tali che
		      \begin{gather*}
			      s'\cmp f_E = f_V\cmp s\\
			      t'\cmp f_E = f_V\cmp t
		      \end{gather*}
...
\end{example}
Evidentemente, possiamo scrivere \(e \colon x\to y\) per denotare in maniera compatta il fatto che \(e \in E\) è un lato tale che \(se=x,te=y\); allora un omomorfismo di multidigrafi è tale che se \(e \colon x\to y\), allora \(f_Ee \colon f_Vx\to f_Vy\).
\begin{example}[Categoria dei grafi]\label{ex_cat_grafi_nondiretti}\index{Categoria!--- dei grafi non diretti}\index{Grafo!--- non diretto}
	...
	Un omomorfismo di grafi non diretti è una funzione tra i vertici \(f_V \colon V\to V'\) compatibile con le relazioni \(E,E'\): se \((u,v)\in E\), allora \((fu,fv)\in E'\).
\end{example}
\begin{example}\label{ex_cat_ordini}\index{Categoria!--- di insiemi ordinati}\index{Preordine}
	Si ricordi da \ref{prelim_def_preset} che un \emph{insieme preordinato} è un insieme \(P\) dotato di una relazione riflessiva e transitiva \(\le\) che si dice la \emph{(relazione d')ordine} su \(P\); una funzione \(f \colon (P,\le)\to (Q,\preceq)\) si dice \emph{monotòna} o si dice che \(f\) \emph{preserva l'ordine} se vale
	\[\forall x,y(x\le y\Rightarrow fx\preceq fy).\]

	La categoria \(\ctPOrd\) ha per oggetti e frecce gli insiemi preordinati e le funzioni monotòne. Chiaramente, l'identità \(\id_P \colon (P,\le)\to (P,\le)\) è una funzione monotòna, e la composizione di due funzioni monotòne resta monotòna.

	È tuttavia importante che l'ordine su \(P\) sia \emph{lo stesso} affinché l'identità sia monotòna: infatti, ogni insieme \(P\) può essere dotato dell'ordine \emph{banale} (dove \(x \mathrel{\le^\delta} y\) se e solo se \(x=y\)) e dell'ordine \emph{caotico} (dove \(x\mathrel{\le^\chi} y\) per ogni \(x,y\in P\)). Se \(P\) ha almeno due elementi, la funzione identità \((P,\le^\chi)\to (P,\le^\delta)\) non è, chiaramente, monotòna.
\end{example}
...
\begin{hRemark}[Combinatoria della categoria \(\bsDelta\)]{skip}\label{rmk_combinatoria_simplessi}\index{Categoria!--- dei simplessi}
	La categoria \(\bsDelta\) possiede una sottoclasse di frecce che generano tutti gli altri per composizione. Tra tutte le mappe monotòne \(f \colon [n]\to [m]\) in \(\bsDelta\) possiamo distinguere le \(n+1\) funzioni \emph{iniettive} \(\delta_n^i \colon [n-1]\mono [n]\) che evitano l'elemento \(i=\iter[0]n\), e le \(n+1\) funzioni \emph{suriettive} \(\sigma_n^j \colon [n+1]\epi [n]\) che assumono due volte il valore  \(j=\iter[0]m\):
	\[
		\delta_n^i(k) =
		\begin{cases}
			k,   & k < i    \\
...
\begin{figure}
	\begin{center}
		...
		\end{tikzpicture}
	\end{center}
	\caption{Le cinque mappe di cofaccia \(\delta_4^0,\dots,\delta_4^4 \colon [3]\mono [4]\) in \(\bsDelta\). Ciascun segmento \(\{\iter[0][<]{i-1}\}\) viene mappato in sé stesso da \(\delta_n^i\); i \(k\) maggiori o uguali di \(i\) vengono mappati in \(k+1\).}
	\label{fig:code}
\end{figure}
...
\begin{figure}
	\begin{center}
		...
		\end{tikzpicture}
	\end{center}
	\caption{Le quattro mappe di codegenerazione \(\sigma_4^0,\dots,\sigma_4^4 \colon [4]\mono [3]\) in \(\bsDelta\). Ciascun valore \(i=\iter[0] n\) viene assunto due volte da \(\sigma_n^i\).}
	\label{fig:cofa}
\end{figure}
\begin{remark}\index{Universo}\index{Ordinale}\label{tipi_di_ordinaly}
	Senza un assioma dedicato a tale scopo, non si possono generare ordinali più grandi di quelli finiti; al contrario, assumendo che esista almeno un insieme \emph{induttivo} \(N\) (cioè tale che, se \(x\in N\), allora \(x^+\defeq x\cup\{x\}\in N\)), possiamo `accedere' a ordinali più grandi, e da qui generarne di \emph{molto} grandi (abbastanza da far sì che la categoria \(\ctOrd\) abbia una classe propria di oggetti).
...
\begin{remark}\label{es_catena_trans}\index{Ordinale!induzione con ---i}
	L'importanza degli ordinali risiede nella possibilità di dare \emph{definizioni per induzione} (finita o transfinita) mediante essi: se \(\ctC\) è una classe, una funzione di classe \(H \colon \ctOrd_0\to\ctC\) si dice \emph{continua} se è univocamente determinata dalla specifica
	\begin{itemize}
	...
	\[\begin{tikzcd}
			A_0 \ar[r] & A_1 \ar[r] & A_2 \ar[r] & \cdots \ar[r] & A_\lambda \ar[r] & \cdots
		\end{tikzcd}\]
...
\begin{hExample}[Spazi topologici]{fund}\label{ex_cat_top}\index{Categoria!--- di spazi topologici}
	...
	Dati due spazi topologici \((X,\tau_X)\) e \((Y,\tau_Y)\), una funzione \(f\colon X\to Y\) è continua per le topologie \(\tau_X\) e \(\tau_Y\) se per ogni aperto \(V\in\tau_Y\), la sua preimmagine \(f^{-1}(V)\) è un aperto di \(\tau_X\).
...
\begin{hExample}[Spazi topologici e classi di omotopia]{skip}\index{Categoria!--- dell'omotopia}\index{Equivalenza!--- a omotopica}\label{ex_cat_hotop}
	...
	\begin{itemize}
		...
		\item una freccia tra due spazi topologici \(X,Y\) è una classe di omotopia di mappe continue \(X\to Y\), dove la relazione di omotopia in \(\ctTop(X,Y)\) è definita così: due mappe continue \(f,g \colon X\to Y\) sono omotope (l'accento è sulla prima \emph{o}) quando esiste una funzione continua \(H \colon X \times [0,1] \to Y\) tale che \(H(x,0) = f(x)\) e \(H(x,1)=g(x)\) per ogni \(x\in X\). Questo si denota con \(f\simeq_H g\) o brevemente con \(f\simeq g\) (quando è univoco, o ovvio, come costruire \(H\) a partire dal contesto).
	\end{itemize}
...
\begin{hExample}[La categoria degli spazi di misura]{skip}\label{ex_cat_meas}\index{Categoria!--- degli spazi misurabili}
	...
	Le frecce \((X, \fkM, \fkN) \to (X', \fkM', \fkN')\) sono ora classi di equivalenza di funzioni \(f\colon X \to X'\) tali che \(f^{-1}E \in \fkM'\) per ogni \(E\in \fkM\), e \(f^{-1}V \in \fkN'\) per ogni \(V\in \fkN\). La relazione di equivalenza data dal differire su un insieme trascurabile è compatibile con la composizione di funzioni: se \(f \sim g\) per alcune
	\[
		f,g : (X_1, \fkM_1, \fkN_1) \to (X_2, \fkM_2, \fkN_2),
	\]
	allora \(f\cmp e \sim g\cmp e\) per ogni \(e\colon (X_0, \fkM_0, \fkN_0) \to (X_1, \fkM_1, \fkN_1)\) e \(h\cmp f \sim h\cmp g\) per ogni \(h\colon (X_2, \fkM_2, \fkN_2) \to (X_3, \fkM_3, \fkN_3)\).
\end{hExample}
...
\begin{example}[La categoria dei complessi di catene]\label{ex_cat_chcomples}\index{Categoria!--- dei complessi di catene}\index{ChAb@\(\ctCh(\ctAb)\)}
	...
	\[\begin{tikzcd}
		...
		\end{tikzcd}\]
	con la proprietà che \(d_n\cmp d_{n+1}=0\) (o equivalentemente, l'immagine di \(d_{n+1}\) sia contenuta in \(\ker d_n\)). Un omomorfismo tra complessi di catene \((A_\bullet,d_\bullet) \to (B_\bullet,d'_\bullet)\) consiste di una famiglia di omomorfismi \(f_n \colon A_n \to B_n\) tali che ciascun quadrato in
	\[\begin{tikzcd}
...
\begin{example}\label{ex_cat_pfun}\index{Funzione!--- parziale}
	...
	Date due funzioni parziali \(f\colon X\parto Y\) e \(g\colon Y\parto Z\), la funzione parziale \(g\cmp f\colon X\parto Z\) è definita come segue:
	\[
		(g\cmp f) (x) \defeq \begin{cases}
			n.d.    & x\notin D_f ;              \\
			n.d.    & x\in D_f, f(x)\notin D_g ; \\
...
\begin{example}\label{ex_cat_puntati}\index{Insieme!--- puntato}
	...
	\begin{itemize}
		...
		\item come frecce tra due insiemi puntati \((X,x)\) e \((Y,y)\), le funzioni \(f\colon X\to Y\) tali che \(f(x)=y\).
	\end{itemize}
\end{example}
\begin{example}[Categoria dei \(G\)-insiemi]\label{ex_cat_g_insiemi}\index{Categoria!--- di azioni di gruppo}
	...
	Dati due \(G\)-insiemi \((X,a_X)\) e \((Y,a_Y)\) (per lo stesso gruppo o monoide \(G\)), una funzione \(f\colon X\to Y\) si dice \emph{equivariante} se per ogni \(x\in X\) e \(g\in G\),
	\[
...
\begin{example}[Categoria dei sistemi dinamici discreti]\label{ex_cat_dyn}\index{Categoria!--- dei sistemi dinamici}\index{Sistema dinamico discreto}\index{Dyn@\(\ctDyn\)}
	...
	\begin{itemize}
		\item per oggetti le coppie \(((X,x_0),f)\) dove \((X,x_0)\) è un insieme puntato nel senso di \ref{ex_cat_puntati}, ed \(f \colon X\to X\) è una endofunzione di \(X\);
		\item  le frecce tra \(((X,x_0),f)\) e \(((Y,y_0),g)\) consistono delle funzioni \(u \colon X \to Y\) tali che \(u(x_0)=y_0\) e \(u\cmp f = g\cmp u\), cioè entrambi i diagrammi di funzioni seguenti siano commutativi.
		      \[\xymatrix@R=3mm{
			      & X\ar[r]^f\ar[dd]_u & X \ar[dd]_u\\
			      \singleton\ar[ur]^{x_0}\ar[dr]_{y_0} &&\\
			      & Y \ar[r]_g & Y
			      }\]
	\end{itemize}
	In un sistema dinamico discreto si considera l'\emph{orbita} \([x_0]\defeq \{x_0,f(x_0),f(f(x_0)),\dots\}\) del \emph{punto base} \(x_0\) della dinamica per studiarne, per esempio, le proprietà geometriche quando \(X\) ha una struttura di spazio metrico (ad esempio, \([x]\subseteq X\) può essere denso, per alcune scelte di \(x_0\), o finito, per altre; può essere preservato da una misura, quando \(X\) sia dotato di una struttura di spazio di probabilità).
\end{example}
\begin{example}[\(\mathrm{C}^*\)-algebre complesse]\label{ex_cat_C_star_algebre}\index{Categoria!--- delle \(\mathrm{C}^*\)-algebre}\index{Cs-algebra@\(\mathrm{C}^*\)-algebra}
	Una \emph{\(\mathrm{C}^*\)-algebra} (\emph{C-star algebra}) consiste di uno spazio di Banach \(A\) (cioè di uno spazio vettoriale topologico sul campo dei complessi, dotato di una norma \(\norm{\blank} \colon A \to \bbR_{\ge 0}\), e completo rispetto alla metrica indotta da \(\norm{\blank}\)), che in più è dotato di
	\begin{itemize}
		\item una operazione di anello (possibilmente non unitario) \(\blank\cdot\blank \colon A \times A \to A\) che sia \emph{submoltiplicativa}, cioè tale che \(\norm{x\cdot y} \le\norm x\norm y\) (questo assicura che la moltiplicazione sia una funzione continua);
		\item una involuzione (`star') \((\blank)^* \colon A\to A\) che soddisfa la \emph{proprietà fondamentale}
		      \[\norm{x^*\cdot x} = \norm{x}^2\]
		      (questo assicura che \(x\mapsto x^*\) sia un'isometria, cioè \(\norm x = \norm{x^*}\)).
	\end{itemize}
	Uno \emph{\(*\)-omomorfismo} (o star-omomorfismo) tra \(\mathrm{C}^*\)-algebre consiste di un omomorfismo di anelli \(f \colon A\to B\) con la proprietà che \(f(a^*) = f(a)^*\).

	La categoria \(\ctCsAlg\) ha
	\begin{itemize}
		\item per oggetti le \(\mathrm{C}^*\)-algebre;
		\item per frecce, date due \(\mathrm{C}^*\)-algebre \(A,B\), tutti gli \(*\)-omomorfismi \(f \colon A \to B\).
	\end{itemize}
...
\begin{example}[Mappe stocastiche]\index{Categoria!--- delle mappe stocastiche}\index{Mappa stocastica}\index{Matrici!categoria delle --- stocastiche}\label{cat_stocazziche}
	...
	\begin{itemize}
		...
		\item per frecce \(n\to m\) le matrici \(A=(a_{ij}\mid i\in [m], j\in[n])\) con \(m\) righe ed \(n\) colonne a coefficienti reali, soggette alle restrizioni seguenti:
		      \begin{itemize}
			      \item ogni entrata \(a_{ij}\) di \(A \colon n\to m\) è un numero reale compreso tra \(0\) e \(1\) (inclusi);
...
\begin{example}[Spazi di convessità]\index{Categoria!--- degli spazi di convessità}\index{Spazio convesso|see {Spazio di convessità}}\index{Spazio di convessità}\label{ex_cat_conv}
	Uno \emph{spazio di convessità} consiste di una coppia \((X,\bsc)\) dove \(X\) è un insieme detto \emph{supporto} dello spazio, e \(\bsc=(c_\lambda\mid \lambda\in[0,1])\) è una famiglia di funzioni \(c_\lambda \colon X\times X\to X\)	soggetta agli assiomi seguenti:
	\begin{itemize}
	...
	\end{itemize}
	Il punto \(c_\lambda(x,y)\) va pensato come il punto a distanza \(\lambda\) da \(x\) e distanza \(1-\lambda\) da \(y\), sul segmento di estremi \(x,y\). Gli omomorfismi \(f \colon (X,c) \to (Y,c')\) tra spazi di convessità sono funzioni \(f \colon X\to Y\) tra i supporti che rispettano le combinazioni convesse: per ogni \(\lambda \in [0,1]\) si ha che \(f(c_\lambda(x,y)) = c'_\lambda(fx, fy)\), o in altre parole (se \(f\times f\) è la funzione ovvia \(X\times X\to Y\times Y\) che manda \((x,y)\mapsto (fx,fy)\)) il quadrato
	\[\xymatrix{
...
\begin{example}\label{example_streams}\index{Categoria!--- di `stream'}
	...
	\begin{itemize}
		...
		\item per frecce \(f \colon A\pto B\) le funzioni della forma
	...
	Le identità sono le funzioni \(\sum_{n=1} A^n\to A\) definite mandando \((\tup an,)\) in \(a_n\); la composizione è data, se \(f \colon A\pto B\) e \(g \colon B\pto C\) dalla regola \(g\cmp f \colon A\pto C\)
...
\begin{example}[La categoria degli insiemi dimensionati]\label{example_dimensionati}\index{Categoria!--- degli insiemi dimensionati}\index{dSet@\(\cate{dSet}\)}
	...
	\begin{itemize}
		\item per oggetti le funzioni suriettive \(d \colon A\to D\) verso un insieme \(D\) che svolge il ruolo di insieme delle unità di misura con cui gli elementi di \(A\) sono etichettati; se, ad esempio \(D \defeq  \{\texttt{m},\texttt{s},\texttt{kg}\}\), gli elementi di \(d^{-1}(m)\) sono le `lunghezze' (cioè si misurano in metri), gli elementi di \(d^{-1}(s)\) sono i tempi, eccetera.
		\item dati due insiemi dimensionati \((d \colon A\to D)\) e \((d', B\to D')\), una freccia tra di loro consiste di una coppia di funzioni \(u,v\) che fanno commutare il seguente quadrato:
		      \[\begin{tikzcd}
				      A\ar[r, "u"]\ar[d,"d"'] & B \ar[d, "d'"]\\
				      D \ar[r, "v"']& D'
			      \end{tikzcd}\]
	\end{itemize}
	Gli assiomi di categoria sono ovviamente soddisfatti: la coppia di identità \((\id_A,\id_D)\) è evidentemente l'identità dell'oggetto \((d \colon A\to D)\) per l'operazione di composizione di funzioni, componente per componente.
\end{example}
\begin{example}[La categoria degli insiemi pesati]\label{example_pesati}\index{Categoria!--- degli insiemi pesati}\index{wSet@\(\cate{wSet}\)}
	...
	\begin{itemize}
		\item per oggetti le coppie \((X,w)\) dove \(X\) è un insieme, e \(w \colon X \to [0,\infty]\) una funzione di `costo' o `peso' che assegna a ogni elemento di \(X\) un numero reale \(w(x)\) (possibilmente infinito; pensando a \(w(x)\) come a un `peso' o `magnitudine' di \(x\), possiamo denotare brevemente \(w(x)=|x|\));
		\item per frecce \(f \colon (X,w) \to (Y,w')\) le \emph{contrazioni} di insiemi pesati, ossia le funzioni \(f \colon X\to Y\) con la proprietà che \(w'\cmp f \le w\) (cioè la funzione \(w\) maggiora punto per punto la composta \(w'\cmp f\): per ogni \(x\in X\) si ha \(|f(x)|_Y \le |x|_X\)).
	\end{itemize}
...
\begin{example}[La categoria degli pseudoinsiemi]\label{ex_cat_pseudoinsiemi}\index{Pseudoinsieme}\index{Categoria!--- degli pseudoinsiemi}\index{Categoria!--- degli insiemi di Bishop|see {--- degli pseudoinsiemi}}\index{Bishop@Errett Bishop}
	...
	\begin{itemize}
		...
		\item un omomorfismo di pseudoinsiemi \(f \colon (S,\sim) \to (T,\asymp)\) consiste di una funzione \(f \colon S\to T\) con la proprietà che se \(s\sim s'\) in \(S\) allora \(fs\asymp fs'\) in \(T\). Equivalentemente, gli omomorfismi di pseudoinsiemi sono le funzioni \(f \colon S\to T\) che `scendono al quoziente' definendo una funzione \(\hat f \colon S/_{\!\sim} \to T/_{\!\asymp}\) tra gli insiemi quoziente.
	\end{itemize}
...
Fin qui, l'osservazione che la classe delle strutture matematiche di un dato tipo, con la definizione appropriata di omomorfismo, formi una categoria, è nient'altro che una banalità della pratica matematica. Una osservazione decisamente meno banale è che alcune strutture matematiche sono \emph{esse stesse} delle categorie.

In più, l'esempio \ref{ex_cat_sigma_strutture} potrebbe far sospettare che gli oggetti di una categoria siano alla fine sempre insiemi dotati di strutture ulteriori (operazioni o relazioni, soggette a equazioni), e per frecce le funzioni che preservano queste operazioni e relazioni. Invece, le categorie che hanno questa proprietà si dicono \emph{concrete}, ed esistono esempi di categorie non concrete (sebbene essi siano sorprendentemente ardui da definire); chi legge e cerca maggiori informazioni può consultare il lavoro di Peter Freyd e Lud\v{e}k Ku\v{c}era \cite{Freyd1973,Kuera1971}.
...
\begin{remark}\label{ord_sonocat_maenriched}\index{Preordine!---i come categorie}
	...
	\begin{itemize}
		...
		\item È essenziale non rimanere confusi da un altro dettaglio: in base a \ref{ex_cat_ordini} esiste una categoria i cui oggetti sono gli insiemi ordinati \((X,\le)\) e le cui frecce sono le funzioni monotòne. In base a \ref{ord_sonocat}, \emph{ogni} insieme ordinato \((X,\le)\) è \emph{esso stesso} una categoria i cui oggetti sono gli elementi di \(X\). Queste due asserzioni sono molto diverse tra loro; sarà la definizione \ref{ex_cat_cat} a rendere preciso in che senso \(\ctPOrd,\ctPos\) e altre sono `categorie di categorie'.
	\end{itemize}
\end{remark}
...
\begin{proof}
	...
	\end{itemize}
\end{proof}
\begin{remark}\label{cor_di_sonopos}
	Per definizione, in una categoria-preordine tra due oggetti \(X,Y\) esiste al più una freccia; corollario immediato di ciò è che fissati \(X\) e \(Y\) tutte le composizioni
	\[\xymatrix@R=0mm{
		X \ar[r] & A_1\ar[r] & A_2 \ar[r]& \cdots \ar[r]& A_n \ar[r] & Y \\
		X \ar[r] & B_1\ar[r] & \cdots \ar[r] & B_m \ar[r] & Y \\
		}\]
	coincidono, perché \emph{tutte} le frecce \(X\to Y\) coincidono.
\end{remark}
...
\begin{theorem}\label{mon_sonocat}\index{Monoide!---i come categorie}
	...
	\begin{itemize}
		\item La categoria \(\susp M\) ha un unico oggetto, che indichiamo con \(\bullet\);\footnote{La notazione è volta a suggerire che precisare questo elemento non sarà mai importante (una scelta canonica, ma che potrebbe causare confusione, è fissare \(\susp M_0 \defeq  \{M\}\), insieme che ovviamente \emph{non} è \(M\)).}
		\item La categoria \(\susp M\) ha una freccia \(m\colon\bullet\to\bullet\) per ogni elemento \(m\in M\); l'unico hom-insieme che dobbiamo specificare, \(\Hom{\susp M}(\bullet,\bullet)\), è cioè uguale a \(M\), e in particolare, l'identità \(\id_\bullet\) è l'elemento neutro \(1\in M\);
...
\begin{esercizi}
	...
	\item Se \(h \colon S\to S\) è una endofunzione di un insieme \(S\), un \emph{punto fisso parametrico} (PFP) per \(h\) è una funzione \(t \colon T\to S\) con la proprietà che \(h\cmp t = t\) (si disegni un diagramma). Siano \(f \colon X\to Y\) e \(g \colon Y \to X\) due funzioni tra insiemi; si mostri che la composizione \(g\cmp f \colon X\to X\) ha un PFP se e solo se la composizione \(f\cmp g \colon Y\to Y\) ha un PFP.
	\item Dimostrare che se \((P,\le)\) è un insieme bene ordinato, ed \(f \colon P\to P\) è una funzione monotòna, allora \(a\le fa\) per ogni \(a\in P\) (cioè ogni endofunzione di un buon ordine è \emph{espansiva}); dimostrare che l'unica biiezione monotòna di un buon ordine è l'identità; più in generale, se esiste un isomorfismo d'ordine \(f \colon P\to Q\) tra due buon ordine, allora esso è unico.
	\item Dare una dimostrazione alternativa di \ref{circ_thm} secondo questa falsariga: definire una funzione di `misura' \(\mu\) sull'insieme delle funzioni Booleane semplici, che manda \(f \colon \bbB^n\to\bbB\) nella cardinalità dell'insieme \(f^\leftarrow(0)\). Mostrare che \(f\) è esprimibile come composizione di AND e NOT nei tre casi:
	\begin{itemize}
		...
		\item se \(\mu(f)\ge 2\), costruire \(f_1,f_2\colon\bbB^n\to \bbB\) tali che \(f(\tup xn,) = f_1(\tup xn,) \land f_2(\tup xn,)\).
	\end{itemize}
	...
	e con la proprietà che \(Z\subseteq X\times Y\) sia un chiuso nello spazio prodotto; ad esempio; \(X=\bbR^3\) e \(Y=\bbP^2(\bbR)\) sono rispettivamente lo spazio vettoriale reale di dimensione 3, e lo spazio proiettivo su di esso. Allora, \(Z\) è la relazione tale che \((x,\ell)\in Z\) se e solo se \(x\) giace sulla retta \(r\). Se su ogni fibra \(r^{-1}(y)\) ora è definita una misura \(d\xi_y\), per esempio come pullback della misura di Lebesgue su \(\bbR^3\), ogni funzione continua \(f \colon X\to\bbR\) ammette una \emph{trasformata di Radon}
	\[R_Zf : y\mapsto \int_{r^{-1}y} f(\xi)d\xi_y.\]
	Un problema centrale della tomografia assiale riguarda il trovare condizioni sufficienti affinché, nota \(R_Zf\), si possa ricostruire \(f\). Sarebbe perciò molto suggestivo rendere la trasformata di Radon una costruzione sulla categoria \(\ctRel(\ctTop)\) i cui oggetti sono spazi topologici e le cui frecce sono relazioni continue. Questa categoria purtroppo non esiste: perché? Quali sono delle ipotesi sufficienti (di separazione, compattezza\dots) da fare sugli spazi per risolvere questo problema e definire una categoria \(\cate{c}\ctRel\) di `relazioni continue'?
\end{esercizi}
