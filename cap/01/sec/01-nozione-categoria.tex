...
\begin{definition}[Categoria]\label{def_categ}\index{Categoria}
	...
	\begin{enumtag}{c}
		...
		\item\label{c_5} Per ogni coppia di frecce \(f\colon X\to Y\) e \(g\colon Y\to Z\), cioè tali che \(\cod{f}=\dom{g}\), esiste una freccia \(g\cmp f:X\to Z\) chiamato \emph{composizione di \(f\) e \(g\)}. Diagrammaticamente: se sono dati
		\[
			\begin{tikzcd}
				X \ar{r}{f}
				& Y & Y \ar{r}{g} & Z
	...
	\begin{enumtag}{p}
		\item \label{cp_1} \emph{Assioma di identità}: per ogni freccia \(f:X\to Y\), valgono le uguaglianze
		\[f\cmp\id_X=f=\id_Y\cmp f.\]
		\item \label{cp_2} \emph{Assioma di associatività}: dati oggetti \(X,Y,Z,W\) e frecce \(f:X\to Y\), \(g:Y\to Z\) e \(h:Z\to W\), vale l'identità
...
\begin{remark}\label{cor_def_categ}\index{Composizione}
	...
	\begin{itemize}
		...
		\item come conseguenza immediata, se \(X\ne X'\) sono oggetti diversi, le identità \(\id_X,\id_{X'}\) sono frecce diverse: si può cioè pensare la corrispondenza \(X\mapsto \id_X : \ctC_0\to\ctC_1\) come una funzione \emph{iniettiva} tra classi;
