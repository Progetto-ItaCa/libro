...
\begin{definition}\label{prelim_def_monoide}\index{Monoide}
	...
	\end{itemize}
\end{definition}
\begin{examples}[Alcuni esempi di monoidi]\leavevmode
	\begin{itemize}
...
\end{definition}
\begin{examples}[Alcuni esempi di insiemi ordinati]\leavevmode
	\begin{itemize}
		...
		Una maniera alternativa di ordinare l'insieme dei naturali è mediante la relazione di \emph{divisibilità}: $m\div n$ se e solo se $n = km$ per qualche $k\in\bbN$. Questa relazione è un ordine parziale, non totale: ad esempio, $2\not\div 3$ e $3\not\div 2$.
		\item Altri insiemi di numeri (interi, razionali, reali) ereditano un ordine totale	da quello dei naturali, e sono anch'essi ordini totali.
		\item L'insieme $\pow S$ di tutti i sottoinsiemi di un insieme $S$ (detto \emph{insieme  delle parti} di $S$) è un poset rispetto all'ordine di inclusione $\subseteq$, cioè per ogni coppia di sottoinsiemi $A,B\subseteq S$, si ha $A\le B$ se e solo se $A\subseteq B$. Questo ordine non è totale.
...
\end{notation}
\fosco{Effettivamente, questa digressione è prematura qui; in attesa di sistemarla meglio la lascio qui, e ne parleremo.}
