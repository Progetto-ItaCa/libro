...
\begin{examples}[Alcuni esempi di insiemi ordinati]
	\Todo{naturali con l'ordine + altri numeri con l'ordine + parti di $A$ + qualcosa che non ha una rel transitiva + qualcosa che è solo un preset}
\end{examples}
Dimostreremo in \ref{mon_sonocat} e in \ref{cat_sonopos} rispettivamente che ogni monoide è una categoria, e ogni insieme preordinato (e a fortiori, ogni insieme parzialmente ordinato) è una categoria. (Gli enunciati in questione renderanno più precise queste ultime affermazioni.) Un primo modo di motivare la definizione di categoria è dunque quello di una simultanea generalizzazione di due strutture matematiche elementari e pervasive. Per procedere, analizziamo degli esempi di strutture che sono categorie, ma non sono né monoidi né insiemi ordinati.
