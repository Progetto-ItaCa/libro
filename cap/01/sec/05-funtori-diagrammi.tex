...
\begin{examples}[Primi esempi di funtori]\label{es_di_funtori}\index{Funtore}
	...
	\begin{enumtag}{ef}
		...
		\item \index{Categoria!--- somma}\label{exfun_4} il funtore di \emph{riduzione} \(\nabla : \ctC+\ctC \fun\ctC\), il cui dominio è la categoria somma di \ref{def_cat_somma}: gli oggetti e le frecce di \(\ctC+\ctC\) sono della forma \((C,\epsilon)\) e \(f : (C,\epsilon) \to (C',\epsilon)\) per \(\epsilon=l,r\), e la riduzione è definita semplicemente da \(\nabla(C,\epsilon)=C\) e \(\nabla(f)=f : C\to C'\).
		...
		\]
		\item \label{exfun_6} Ogni funtore \(F : \ctC\fun\ctD\) definisce il \emph{funtore opposto}
		...
		\[\dmFun{d}{\ctC/C}{\ctC}\]
		che restituisce (appunto) il dominio \(X\) di \(\var XC\); le proprietà di funtorialità sono evidenti. Analogamente, esiste un funtore `codominio' \(c : C/\ctC \to \ctC\) dalla categoria succuba sotto \(C\).
	\end{enumtag}
\end{examples}
\begin{definition}[Funtori pieni, fedeli, pienamente fedeli]\label{classi_di_funtori}\index{Funtore!classi di ---i}\index{Funtore!--- pieno}\index{Funtore!--- fedele}\index{Funtore!--- pienamente fedele}
	È conveniente dare un nome speciale ai funtori \(F : \ctC\fun\ctD\) tali che, nelle stesse notazioni di sopra, ogni funzione \(F_{XY}\) sia rispettivamente iniettiva, suriettiva, e biiettiva: \(F : \ctC\fun\ctD\) si dirà
	\begin{itemize}
...
\end{definition}
\begin{remark}
	Le tre nozioni sono indipendenti: il funtore di inclusione \(\ctC_\cong \subseteq\ctC\) è fedele, ma non pieno se \(\ctC\) non è un gruppoide. Il funtore \(S : \genAArrow \fun \genArrow\) che identifica tra loro le due frecce di \(\genAArrow\) è pieno, ma non fedele.
\end{remark}
...
\begin{example}\label{exa_derivata_funtore}\index{Funtore!differenziale come ---}\index{Differenziale}
	...
	\begin{itemize}
		...
		\item le frecce \((n,x)\to (m,y)\) sono funzioni differenziabili \(f:\bbR^n\to\bbR^m\) tali che \(f(x)=y\).
	\end{itemize}
	Possiamo costruire un funtore \(D:\ctEuc \fun \ctMat[\bbR]\) nel seguente modo:
	\begin{itemize}
		\item sugli oggetti, mappa \((n,x)\) in \(n\);
		\item sulle frecce, assegna alla funzione \(f:(\bbR^n,x)\to (\bbR^m,y)\) il \emph{differenziale} di \(f\) in \(x\), cioè, la mappa lineare \(Df:\bbR^n\to\bbR^m\).
	\end{itemize}
...
\begin{examples}
	...
	\begin{itemize}
		...
		\item Consideriamo l'endofuntore \(\ctP_\bbN\defeq \ctP(\bbN\times\blank):\ctSet\fun\ctSet\) che manda un insieme \(X\) nell'insieme dei sottoinsiemi di \(\bbN\times X\), dove \(\bbN\) è l'insieme dei numeri naturali.
...
\begin{definition}[Bifuntore]\label{def_bifuntore}\index{Bifuntore}\index{Funtore!bi---}
	...
	\begin{itemize}
		...
		\item a ogni coppia di frecce \(f : A\to A', g : B\to B'\) una freccia \(F(f,g) : F(A,B)\to F(A',B')\) in \(\ctC\).
	\end{itemize}
...
\begin{hExample}[Funtore hom co- e controvariante, bifuntore hom]{fund}\label{ex_hom_funtore}\index{Funtore!hom --- covariante}\index{Funtore!hom --- controvariante}
	...
	come segue:
	\begin{itemize}
		\item un oggetto \(Y\) viene mandato nell'insieme \(\Hom{\ctC}(Y,C)\) delle frecce \(u : Y\to C\);
	...
	\begin{itemize}
		...
		\item sulle frecce, il morfismo \((u : X'\to X, v : Y\to Y')\) induce la funzione
...
\begin{examples}[Alcuni esempi di bifuntori]\label{exam_bifuntori}\index{Bifuntore}\index{Funtore!bi---}
	...
	\begin{enumtag}{eb}
		...
		\item\label{eb_2} Esiste un bifuntore \(\blank\times\blank : \ctSet\times\ctSet \fun\ctSet\) che manda un oggetto \((X,Y)\) del prodotto di categorie \(\ctSet\times\ctSet\) nell'insieme \(X\times Y\) e le frecce \((f :A \to X, g : B\to Y)\) in \(f\times g : A\times B \to X\times Y : (a,b)\mapsto (f(a),g(b))\).
...
%         tra categorie incubo, che mandano un oggetto \(\var EA\) in un oggetto \(\var{M_0A}B\) (la corrispondenza \(M_0 = M\cmp d\) è essa stessa funtoriale, di tipo \(\ctSet/A \fun\ctSet/B \fun \ctSet\); si veda \ref{exfun_7} per la notazione \(d : \ctSet/B \to \ctSet\)).
%  \end{itemize}
...
% \end{itemize}
% Ora, dare una famiglia di insiemi \(M_{ab}\) al variare di \(A\times B\) è equivalente a dare un funtore \(\tilde M : A\times B \fun\ctSet\) dal prodotto di due categorie discrete; a sua volta questo è equivalente a dare un funtore \(M : \ctSet/A \fun\ctSet/B\) come sopra, il quale soddisfa una certa proprietà (è un `aggiunto sinistro', si veda il \autoref{cap_aggiunti}) che lo rende univocamente determinato dagli \(M_{ab}\).
\begin{remark}\index{Funtore!--- controvariante}
	Ogni funtore \(F : \ctC\fun\ctD\) definisce un funtore \(F^\op : \ctC^\op \fun\ctD^\op\) che coincide con \(F\) sugli oggetti, e che è definito sulle frecce mandando \(f^\op\in\ctC^\op\) in \(F(f)^\op\in\ctD^\op\). È semplice (e noioso) verificare le proprietà \ref{p_1}, \ref{p_2}.

	Dal momento che \((\ctC^\op)^\op = \ctC\), il dato di un funtore controvariante \(F : \ctC\fun\ctD\), cioè di un funtore \(F : \ctC^\op \fun\ctD\), è esattamente equivalente al dato di un funtore \(F^\op : \ctC \fun \ctD^\op\). È a volte più conveniente considerare l'uno piuttosto che all'altro funtore, riferendosi alla sua controvarianza.
\end{remark}
...
\begin{definition}[La categoria degli elementi di un funtore]\label{elts_F}\index{Categoria!--- degli elementi}\index{aaa_EltsF@\(\Elts{}F\)}
	...
	\begin{itemize}
		...
		\item per frecce \((C,x)\to (C',y)\) le frecce \(f : C\to C'\) di \(\ctC\) tali che la funzione \(Ff : FC\to FC'\) `preserva il punto base', cioè manda \(x\) in \(y\).
	\end{itemize}
...
La costruzione si dà anche per un funtore controvariante \(F : \ctC^\op\fun\ctSet\), la cui categoria degli elementi ha oggetti \((C,x)\) come sopra, e frecce \(f : (C,x)\to (C',y)\) tali che \(Ff : FC'\to FC\) manda \(y\) in \(x\).
...
\begin{hDigression}[Costrutti e loro proprietà]{skip}\index{Costrutto}\index{Funtore!--- fedele}\index{Categoria!--- concreta}\label{def_costrutto}
	...
	Si può definire, da qui, la categoria \(\cate{Cons}\) dei costrutti che ha per frecce \(K : (\ctC,U)\fun (\ctD,V)\) quei funtori \(K : \ctC\fun\ctD\) compatibili con \(U,V\) nel senso ovvio che \(V\cmp K=U\). Più formalmente, al netto di un problema di taglia, si può pensare la categoria \(\cate{Cons}\) come la sottocategoria della comma \(\cate{CAT}/\ctSet\) sui soli oggetti \(\ctC\fun\ctSet\) che sono funtori fedeli. Quel che però è più importante è che la nozione di \emph{isomorfismo} in \(\cate{Cons}\) formalizza l'idea che si possano avere presentazioni differenti di uno stesso costrutto. Ad esempio, esiste una categoria degli spazi topologici, definita in \ref{ex_cat_top}, che è un costrutto rispetto all'ovvio funtore \(U : \ctTop\fun\ctSet : (X,\tau)\mapsto X\) che dimentica la topologia \(\tau\) di uno spazio \((X,\tau)\).
...
\begin{hProposition}{skip}
	...
	Dato uno spazio topologico, la funzione \(j : S \mapsto \bigcup_{\substack{U \text{ aperto,}\\ U\subseteq S}} U\) è un operatore di interno: per la proprietà di stabilità degli aperti rispetto all'unione, \(jS\) è aperto. Per definizione, \(jS\) è il più grande aperto contenuto in \(S\). Inoltre,
	\begin{itemize}
	...
	da categorie opportunamente definite di spazi topologici e di chiusura, e le corrispondenze determinate prima si sollevano a funtori \(K:\ctTop \to \textbf{KSp}\) e \(O:\textbf{KSp} \to\ctTop\):
	\begin{itemize}
		\item Una funzione `continua' \(f : X \to Y\), tale che \(f^{-1}A\) sia aperto per ogni aperto \(A\subseteq Y\), è anche compatibile con gli operatori di interno \(j_X,j_Y\), nel senso che
...
\begin{hRemark}[Cos'è esattamente un diagramma commutativo?]{tech}\label{cos_diag_comm}\index{Diagramma!--- commutativo}
	...
	\begin{quote}
		Un diagramma commutativo è un funtore \(\Phi : \ctJ \fun\ctD\) tale che per ogni coppia di frecce parallele \(\vec a,\vec b\) del suo dominio, ponendo \(a\defeq \cmp(\vec a) = a_n\cmp a_{n-1}\cmp\dots\cmp a_0\) e similmente \(b\defeq \cmp(\vec b) = b_m\cmp b_{m-1}\cmp\dots\cmp b_0\) le rispettive composizioni, si abbia \(\Phi a = \Phi b\) in \(\ctC\).
	\end{quote}
...
\begin{figure}
	\begin{center}
		...
		\end{tikzpicture}
	\end{center}
	\caption{Un diagramma	in catena \(X : \omega^\op \fun\ctSet\) e il suo `albero' associato (sulla destra): \(X(0)\) è l'insieme delle radici dell'albero, da cui si dipartono una serie di rami, tanti quanti sono gli elementi della fibra di \(\varphi_0\) sopra ciascuna radice; analogamente dagli elementi di \(X(1)\) si dipartono tanti rami quanti sono gli elementi della fibra di \(\varphi_1\) sopra ciascun elemento del suo codominio, e così via.}
	\label{fig_alberello}
\end{figure}
...
\begin{definition}[Categoria filtrata]\label{def_cat_filtrata}\index{Categoria!--- filtrata}
	...
	\begin{enumtag}{cf}
		...
		\item \label{cf_2} per ogni \(n\)-upla di frecce parallele \(\tup un, : J\to J'\), esiste una freccia \(k : J'\to K\) tale che \(k\cmp u_1 = k\cmp u_2 = \dots = k \cmp u_n\): cioè il diagramma
		\[\xymatrix{
	...
	\begin{quote}
		Una categoria è filtrata se ogni famiglia finita di oggetti \({\tup Jn,}\) e ogni famiglia finita di frecce \(\tup un, : J\to J'\) ammettono un cocono.
	\end{quote}
...
\begin{examples}\label{contro_esempi_filt}\index{Categoria!--- filtrata}\index{Ordine parziale!--- diretto}
	...
	\begin{itemize}
		...
		\item La categoria \(\bsDelta_\text{inj}\) dei \emph{semisimplessi}, ossia la sottocategoria ottenuta da \(\bsDelta\) prendendo gli stessi oggetti \([n]\) e solo le cofacce \(\delta_n^i\) per frecce, non è filtrata: infatti, nessuna funzione \emph{iniettiva} (le sole che sono ottenibili componendo mappe di cofaccia) \([1] \to [n]\) coequalizza la coppia di frecce \(\delta_1^0,\delta_1^1 : [0]\toto [1]\), che quindi non ammette un cocono.
	\end{itemize}
\end{examples}
...
\begin{proof}
	...
	Viceversa, se \(\ctJ\) è filtrata e \(D : \ctI \fun\ctJ\) un diagramma da una categoria finita, l'insieme finito \(\{\tup Jn,\}=\{DI_1,\dots, DI_n\}\) ammette un cocono \(\pair {k_i}{J^*}\); possiamo porre \(\bar D I \defeq  DI\), ma definire \(\bar D (\infty)\) e l'azione di \(\bar D\) sulle frecce è più complesso: non esiste garanzia che i triangoli della forma
	\[\xymatrix@R=3mm{
		**[l] DI=J\ar[dr]\ar[dd]_{D(I\to I')} \\
		&J^* \\
		**[l] DI'=J'\ar[ur]
		}\]
	siano commutativi. Si possono però definire gli oggetti \(B_0 \defeq  J^*\), e per ogni fissato \(i=\iter n\), e ogni \(f : I_i \to I'\) l'oggetto \(B_i\) come il cocono della famiglia di frecce parallele
	\[\xymatrix@R=1mm{
		&DI'\ar[dr]&\\
...
\begin{proof}
	...
	Però non esiste nessuna freccia \(x : \texttt{0} \to \#\) che coequalizzi le due frecce parallele \(j\cmp \sigma,j\cmp \tau : \texttt{0} \toto \texttt{0}\): l'identità non lo può fare, perché i due idempotenti \(j\cmp \sigma,j\cmp \tau\) sono diversi; né \(\sigma\) né \(\tau\) possono farlo,	perché \(\sigma\cmp j\cmp \sigma = \sigma \neq \tau = \sigma\cmp j\cmp \tau\) e similmente si ragiona per \(\tau\). Quindi \(\genRG\) non è filtrata.
\end{proof}
