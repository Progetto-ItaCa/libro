...
\begin{theorem}\label{filtr_setacciata}\index{Categoria!--- filtrata}
	Ogni categoria filtrata è setacciata; la categoria
		      ...
		      dove (a parte le identità) ci sono tre frecce \(\sigma,\tau,j\) tali che \(\sigma\cmp j = \tau\cmp j = \id_1\) è setacciata ma non filtrata.
\end{theorem}
\begin{proof}
	...
	Però non esiste nessuna freccia $x : \texttt{0} \to \#$ che coequalizzi le due frecce parallele $u\cmp s,u\cmp t : \texttt{0} \toto \texttt{0}$: l'identità non lo può fare, perché i due idempotenti $u\cmp s,u\cmp t$ sono diversi; né $s$ né $t$ possono farlo,	perché $s\cmp u\cmp s = s \neq t = s\cmp u\cmp t$ e similmente si ragiona per $t$. Quindi $\genRG$ non è filtrata.
\end{proof}
