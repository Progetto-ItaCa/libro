...
\begin{terminology}[A proposito del nome `funtore']\index{Funtore!etimologia di `---'}
	...
	Haskell Curry \cite{Curry1961SomeLA} attribuisce più correttamente il termine a Tadeusz Kotarbiński, logico e filosofo polacco, che nel suo testo \emph{Elementy teorii poznania}\footnote{Il titolo completo dell'opera in polacco è \emph{Elementy teorii poznania, logiki formalnej i metodologii nauk}, `Elementi di teoria della conoscenza, logica formale e metodologia delle scienze'; del testo non esiste una traduzione italiana ma varie traduzioni in altre lingue, che sono quasi tutte sconosciute e spesso abbreviano radicalmente il titolo (quello della versione inglese a opera di Olgierd Wojtasiewicz è semplicemente \emph{Gnosiology}, sic). Nel testo, Kotarbiński espone la sua concezione ontologica chiamata `reismo', che unisce elementi di nominalismo radicale e materialismo, e critica l'uso di `onomatoidi' (nomi apparenti) che danno l'illusione di entità reali dietro di essi.} (scritto cinque anni prima di \cite{carnappio}) introduce i funtori come `connettivi sentenziali' all'inizio del suo capitolo II. In \cite{Curry1961SomeLA} l'autore fa uso del concetto di funtore nel suo senso filosofico (più precisamente, in filosofia del linguaggio, dove un funtore è definito come un operatore che agisce sulle frasi di un dato linguaggio, per formarne altre, `preservando l'analisi logica', ossia le relazioni tra le parti della frase) per costruire il linguaggio giocattolo dei \emph{sami}, \emph{teteli} e \emph{tanteti}.

	La traduzione Gnosiology di \emph{Elementy teorii poznania} contiene un'appendice scritta da Kazimierz Ajdukiewicz, altro famoso esponente della scuola polacca di logica. Ajdukiewicz usa in maniera sorprendentemente libera il termine `funtore' in frasi come [traduzione degli autori]
	\begin{quote}
