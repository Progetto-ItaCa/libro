...
\begin{definition}\label{def_cat_setacciata}\index{Categoria!--- setacciata}
	\Todo{categoria setacciata}
\end{definition}
\Todo{forse qualcos'altro: prodotto di setacciate è setacciato, etc?}
\begin{theorem}
	\Todo{Ogni categoria filtrata è setacciata, e la categoria ... è setacciata ma non filtrata}
\end{theorem}
\begin{remark}[Perché categorie `filtrate' e `setacciate'?]
	L'idea è che una categoria filtrata \(\ctJ\)  dispone di un filtro che seleziona un cocono per ogni sottocategoria finita di \(\ctJ\); un setaccio è invece capace, più in generale, di selezionare un cocono per ogni famiglia finita di oggetti, e un filtro è poi un tipo di setaccio particolarmente sottile.
	\begin{figure}
		\begin{center}
			\begin{tikzpicture}[
					x=4em, y=4em,
