...
\begin{definition}\label{def_cat_setacciata}\index{Categoria!--- setacciata}
	Una categoria \(\ctJ\) si dice \emph{setacciata} se per ogni coppia di oggetti $X,Y\in\ctJ$ la categoria delle cospanne $\ctCospan_\ctJ(X,Y)$ che ha 
	\begin{itemize}
		\item per oggetti le cospanne	\(\pair fg : X\xto{f} Z\xot{g} Y\) in \(\ctJ\),
		\item per frecce tra cospanne	\(\pair fg : X\to Z\leftarrow Y\) e \(\pair{f'}{g'} : X\to Z'\leftarrow Y\) le frecce $h : Z\to Z'$ di $\ctJ$ tali che $h\cmp f = f'$ e $h\cmp g=g'$, cioè tali che il diagramma
		\[\xymatrix@R=1mm{
			&Z\ar[dd]_h & \\
		...
		sia commutativo,
	\end{itemize}
	e composizione e identità ovvie, è connessa nel senso di \ref{exe_cpt_conn}.
\end{definition}
...
\begin{proof}
	Se $\ctJ$ è filtrata soddisfa la proprietà di Ore; allora mostriamo che ogni coppia di cospanne della forma  
			...
			è connessa da un cospan di frecce in $\ctJ$, cioè esiste un diagramma
					\[\xymatrix@R=7mm{
			&Z\ar[d]|z & \\
				X \ar[r]|x \ar[dr]_{f'}\ar[ur]^f& K & Y\ar[ul]_g\ar[dl]^{g'}\ar[l]|y\\
				& Z'\ar[u]|{z'}
			}\]
			Evidentemente esistono quadrati commutativi della forma
\[\xymatrix{
...
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	&& Z \\
	X & U & K & V & Y \\
	&& {Z'}
	...
	\arrow["\ell"{description}, from=2-2, to=2-3]
	\arrow["r"{description}, from=2-4, to=2-3]
	\arrow["g"', curve={height=12pt}, from=2-5, to=1-3]
	\arrow["{{g'}}", curve={height=-12pt}, from=2-5, to=3-3]
	\arrow["{{u'}}"', from=3-3, to=2-2]
	\arrow["{{v'}}", from=3-3, to=2-4]
\end{tikzcd}\]
(servono due applicazioni del fatto che $\ctJ$ è filtrata, e una applicazione della proprietà di Ore, per scegliere $K$ in modo che $\ell,r$ siano tali per cui $\ell\cmp u = r\cmp v$ e $\ell\cmp u' = r\cmp v'$ allo stesso tempo); ora basta porre $z = \ell\cmp u = r\cmp v$, $z' =	\ell\cmp u' = r\cmp v'$ e $x = \ell\cmp u\cmp f$, $y = r\cmp v\cmp g$.
\end{proof}
\begin{remark}[Perché categorie `filtrate' e `setacciate'?]
	...
	\begin{figure}
		\begin{center}
			...
			\end{tikzpicture}
		\end{center}
		\caption{In una categoria filtrata $\ctJ$, ogni sottocategoria finita $\ctI$ ammette un cocono; la categoria $\ctJ$ dispone di un filtro sottile abbastanza da selezionare un cocono $J^*$ per $\ctI$, estendendo un diagramma $D : \ctI \fun\ctJ$ a $D^* : \ctI^\rhd \fun \ctJ$ come in \ref{estensione_filtrata}.}\label{fig_filtro_setaccio}
	\end{figure}
\end{remark}
\begin{esercizi}
