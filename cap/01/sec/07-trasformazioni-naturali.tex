...
\begin{example}\label{ex_cat_sset}
	La categoria \(\ctsSet=\presh{\bsDelta}\) degli insiemi simpliciali ha per oggetti i funtori \(\Delta^\op\fun\ctSet\), dove \(\Delta\) è la categoria dei simplessi di \ref{}, e per morfismi le trasformazioni naturali tra questi funtori. Questa categoria è di grande importanza in topologia algebrica, si veda \cite{a,b,c} dato che \(\presh{\bsDelta}\) ammette un funtore di \emph{realizzazione}
	\[\dmFun{r}{\ctsSet}{\ctTop}\]
	verso la categoria \(\ctTop\) degli spazi topologici, che porta un oggetto completamente combinatorio (il funtore \(X : \bsDelta^\op\fun\ctSet\)) in uno spazio topologico che può essere costruito iterativamente attaccando dischi di dimensione via via più alta (quello che in \cite{} è chiamato un CW-complesso; si veda \cite{fritsch1990cellular} per una introduzione elementare e \cite{} per una più indirizzata alle proprietà categoriali di \(\presh{\bsDelta}\) e del funtore \(r\) di realizzazione).
\end{example}
\begin{example}\label{ex_cat_gset}
	\Todo{}
\end{example}
L'interesse per la categoria \(\ctsSet\) degli insiemi simpliciali risiede principalmente nel fatto che ogni categoria piccola \(\ctC\in\ctCat\) induce un insieme simpliciale \(N\ctC\) detto il \emph{nervo} di \(\ctC\), definito nel modo che segue:
\begin{itemize}
...
\begin{proof}
		...
		\item per \(j=n\) si ha
		      \begin{align*}
			      ...
			      d_{n-1}d_i[\tup fn,] & = d_{n-1}[\tup f{i-1}, ,f_{i+1}\cmp f_i, \tup[i+2]f{n-1},] \\
			                           & =[\tup f{i-1}, ,f_{i+1}\cmp f_i, \tup[i+2]f{n-1},];
		      \end{align*}
		\item per \(0<i<n\) si ragiona in maniera del tutto simile. Questo dimostra tutte le equazioni di \eqref{idsim_1}.
	\end{itemize}
	\Todo{}
	...
	\end{itemize}
	L'interazione tra facce e degenerazioni usa gli assiomi di categoria per \(\ctC\); in particolare,
	\begin{align*}
	...
	\begin{align*}
		...
		s_2d_3[\tup f5,] & = s_2[f_1,f_2,f_4\cmp f_3,f_5]       \\
		                 & =[f_1,f_2,\Id[C_2],f_4\cmp f_3,f_5].
	\end{align*}
	Lasciamo a chi legge l'onere di scrivere il caso generale (che non cambia in nulla la struttura di questo argomento), così come l'identità \(d_i\cmp s_j=s_{j-1}\cmp d_i\) quando \(i<j\), che si dimostra con una analoga quantità di tedio; questo conclude la dimostrazione.

	Va ora dimostrato che la corrispondenza \(N : \ctCat \fun \ctsSet\) è un funtore, ovvero che un funtore tra categorie piccole \(F : \ctC\fun\ctD\) induce una trasformazione naturale \(NF: N\ctC\nat N\ctD\) tra i rispettivi nervi, e in più che \emph{ogni} mappa simpliciale \(N\ctC\nat N\ctD\) origina da un (unico) funtore.
	\Todo{}
\end{proof}
La corrispondenza \(N : \ctC\mapsto N\ctC\) diventa un funtore \(N : \ctCat\to\ctsSet\) se si definisce \(NF\) come la trasformazione naturale indotta dall'azione di \(F\) sulle frecce di \(\ctC\).
...
\begin{example}[Co/puntature e co/moltiplicazioni]\index{Funtore!--- co/puntato}
	...
	\end{itemize}
\end{example}
\begin{definition}[Legge distributiva]
...
\begin{example}
	...
	date dall'elidere tutte le occorrenze di \(\bullet\) nell'argomento
\end{example}
...
\begin{definition}[Composizione orizzontale di trasformazioni naturali]\label{def_hcomp}\index{Composizione orizzontale!di trasformazioni naturali}
	Si considerino tre categorie \(\ctC,\ctD,\ctE\), due funtori \(F,G:\ctC\to\ctD\) e due funtori \(F',G':\ctD\to\ctE\). Date due trasformazioni naturali \(\alpha:F\Rightarrow G\) e \(\alpha':F'\Rightarrow G'\), la \emph{trasformazione naturale composta orizzontalmente} \(\alpha'\horcomp\alpha:F'\circ F\Rightarrow G'\circ G\), rappresentata come segue,
	\[
...
\end{proposition}
\begin{definition}[Il nervo di un funtore]\label{def_nervo_funtore}\index{Funtore!nervo di un ---}
...
\end{definition}
Si noti che il nervo di una categoria \(\ctC\), come definito in \ref{}, non è altro che il nervo del funtore canonico
\[\dmFun{\boldsymbol{\Delta}}{\ctFOrd}{\ctCat}\]
che riguarda un insieme totalmente ordinato \([n]\in\ctFOrd\) come la categoria \(\boldsymbol{\Delta}[n] :=\{0\to 1\to\cdots\to n\}\) (a sua volta, il funtore \(\boldsymbol{\Delta}\) è la restrizione \(t|_{\ctFOrd}\), nella terminologia di \ref{ord_come_cat}).
\begin{esercizi}
