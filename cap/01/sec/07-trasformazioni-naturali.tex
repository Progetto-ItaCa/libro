...
Osserviamo poi la condizione di naturalità per \(\alpha^{-1}\) può essere omessa:
...
\begin{definition}[Funtore rappresentabile]\label{def_rappre_fun}\index{Funtore!--- rappresentabile}\index{Rappresentabilità}
	Una famiglia importante di isomorfismi naturali sono quelli che sussistono tra un funtore $F : \ctC \fun\ctSet$ e uno dei \emph{funtori hom} di \ref{ex_hom_funtore}; qualora esista un isomorfismo naturale $F\natIso \Hom\ctC(X_F,\blank)$ per un qualche oggetto $X_F\in\ctC_0$, diciamo che $F$ è \emph{rappresentabile} (da $X_F$), o che $X_F$ è l'\emph{oggetto rappresentante} per $F$. Questa terminologia è indicativa del fatto che la rappresentabilità sia una proprietà di $F$, ossia che se $F$ è rappresentabile, lo sia in modo essenzialmente unico. Questo è vero (ed è il cuore del capitolo \ref{cap_yoneda}, sul lemma di Yoneda), ma bisogna sviluppare più teoria per poterlo dimostrare. Gli esempi di funtori rappresentabili abbondano in algebra, geometria e logica e sono raccolti nelle prime sezioni del capitolo \ref{cap_yoneda}.
\end{definition}
