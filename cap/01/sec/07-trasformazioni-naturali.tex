...
\begin{itemize}
 \item per ogni spanna \((X_1\overset l\leftarrow X_0\overset r\to X_2)\) possiamo considerare la terna \(\underline{\id}=(\id_0 \colon X_0\to X_0,\id_1 \colon X_1\to X_1,\id_2 \colon X_2\to X_2)\);
 \item date \(\underline\alpha=(\alpha_0,\alpha_1,\alpha_2),\underline\beta=(\beta_0,\beta_1,\beta_2)\) che formano due frecce distinte tra spanne, ne esiste una composta \(\underline\beta\cmp\underline\alpha=(\beta_0\cmp\alpha_0,\beta_1\cmp\alpha_1,\beta_2\cmp\alpha_2)\) che è a sua volta un omomorfismo di spanne, dato che l'esterno del diagramma
...
\begin{definition}[Trasformazione naturale]\label{def_nat}\index{Trasformazione naturale}\index{Naturale!trasformazione}\index{Naturalità}
 Date due categorie \(\ctC\) e \(\ctD\) e dati due funtori \(F,G\colon\ctC\to\ctD\), una \emph{trasformazione naturale} \(\alpha\colon F\Rightarrow G\) consiste dei seguenti dati:
 \begin{itemize}
  \item Per ogni oggetto \(X\) di \(\ctC\), una freccia \(\alpha_X\colon FX\to GX\) in \(\ctD\) detto la \emph{componente di \(\alpha\) ad \(X\)};
  \item Per ogni morfismo \(f\colon X\to Y\) di \(\ctD\), il seguente diagramma deve commutare (\emph{condizione di naturalità}):
        \[
         \begin{tikzcd}
...
\begin{definition}[Composizione verticale di trasformazioni naturali]\label{def_vcomp}\index{Composizione verticale!di trasformazioni naturali}
 Consideriamo tre funtori \(F,G,H\colon\ctC\to\ctD\) e due trasformazioni naturali \(\alpha\colon F\Rightarrow G\) e \(\beta\colon G\Rightarrow H\).
 La \emph{composizione verticale} \(\beta\cmp\alpha\colon F\Rightarrow H\) delle transformazioni naturali \(\beta,\alpha\), rappresentate avendo il bordo \(G\) in comune,
 \[\xymatrix{
 ...
 \]
 è definita dalla composizione \(\beta_X\cmp\alpha_X \colon FX\to GX\to HX\).
\end{definition}
L'operazione di composizione verticale definisce una categoria \(\ctCat(\ctC,\ctD)\) la cui classe degli oggetti è fatta dai funtori \(\ctC\fun\ctD\), e dove le frecce \(F\nat G\) sono le trasformazioni naturali come in \ref{def_nat}; evidentemente, la trasformazione naturale identica \(\id_F \colon F\nat F\) ha per componenti \(\id_{FX} \colon FX\to FX\).
\begin{example}\index{Categoria!---e di funtori}
 ...
 \begin{itemize}
  ...
  \item La categoria dei multidigrafi di \ref{ex_cat_grafi} è la categoria dei funtori \(\ctCat((0\toto 1)^\op,\ctSet)\); ciò rende precisa l'osservazione subito successiva alla definizione di \ref{ex_cat_doppiafreccia}. Lasciamo per esercizio a chi legge di mostrare che una trasformazione naturale tra funtori \(F,G\) che assumono rispettivamente per valori delle coppie di funzioni
        ...
        dove \(j\) è una retrazione comune a \(s,t\): \(j\cmp s = j\cmp t = \id_0\). \index{Grafo!--- non diretto} Invece, la categoria dei grafi non diretti di \ref{ex_cat_grafi_nondiretti} consiste della categoria dei funtori (controvarianti) e trasformazioni naturali di forma
        ...
        \begin{itemize}
         \item nel primo caso, un funtore \(G \colon \ctR\ctG^\op\to\ctSet\) consta di una coppia di insiemi \(G_0 = V\) e \(G_1 = E\) che, oltre alle solite funzioni dominio e codominio, hanno una sezione comune \(j' = G(j) \colon V\to E\) che ad ogni vertice \(v\in G_0\) associa un lato \(j'(v)\) di dominio e codominio \(v\);
         \item nel secondo caso, un funtore \(\ctL\ctG^\op\to\ctSet\) consta di una coppia di insiemi \(G_0=V\) e \(G_1=E\) che, oltre alle solite funzioni dominio e codominio, possiedono una involuzione \(\ell = G(l) \colon E\to E\)  dell'insieme dei lati, che scambia tra loro dominio e codominio. Una maniera di rappresentare un grafo non diretto è come un grafo diretto, dove ad ogni lato \(e \colon v\to v'\) corrisponde un lato omonimo \(\ell(e) \colon v'\to v\) --e dunque, a tutti gli effetti al lato \(e\) manca un'orientazione perché `collega \(v\) e \(v'\) in entrambe le direzioni.
        \end{itemize}
  ...
  \item Esiste una categoria \(\ctSpc\) che ha per oggetti le specie combinatorie di \ref{ex_fun_specie}; una freccia in \(\ctSpc\)	è una trasformazione naturale tra funtori \(\ctBij \fun\ctSet\), e dunque consta di una famiglia di funzioni \(\alpha_X \colon F(X)\to G(X)\) che commuta (è equivariante) con le funzioni indotte da ogni funzione \(f\colon X\to Y\).\index{Specie combinatoria}
 \end{itemize}
 Si noti come tutti i vantaggi esposti in \ref{es_fun_repre_vantaggi} dell'avere descritto una struttura come una categoria di `rappresentazioni di forme astratte' rimangono validi: un `(multidi)grafo lineare' consiste di un funtore \(\genArrow^\op \fun \ctVect\), e le mappe di dominio e codominio sono omomorfismi lineari; un `grafo topologico' di un funtore \(\genArrow^\op\fun\ctTop\), e le mappe di dominio e codominio sono continue; similmente per grafi riflessivi e non diretti, lineari, topologici o altro.
\end{example}
...
\begin{hExample}[Trasformazione di Hurewicz in topologia]{tech}\index{Hurewicz!mappa di ---}
 \index{Hurewicz@Witold Hurewicz}
 ...
 \begin{itemize}
  \item se \(X\) è uno spazio topologico, abbiamo definito in \ref{fun_ex_omoto_omolo} il gruppo \(\pi_n(X)\) che ha per elementi le classi di omotopia di funzioni continue \(S^n \to X\);
 ...
 \end{itemize}
 Nel 1935, il topologo polacco Witold Hurewicz ha definito un esempio di trasformazione naturale tra il funtore `\(n\)-esimo gruppo di omotopia' ed `\(n\)-esimo gruppo di omologia singolare', \(h \colon \pi_n \nat H_n\),\footnote{Ha definito, pochi anni prima, anche i funtori \(\pi_n\) di \ref{fun_ex_omoto_omolo}. Hurewicz è un pensatore di fondamentale importanza per la teoria delle categorie, perché è il primo a introdurre la notazione che rappresenta gli omomorfismi come frecce \(f \colon X \to Y\), come viene ricordato anche nelle Note Storiche del capitolo 1 di \cite{working-categories}.} assegnando a un elemento di \(\pi_n(X) = [S^n,X]\), cioè alla classe di omotopia di una certa \(f \colon S^n \to X\), l'elemento \(f_*(\upsilon)\) immagine del generatore sopra fissato mediante \(f_* \colon H_n(S^n) \to H_n(X)\).

 Questa trasformazione è naturale: data una funzione continua \(g \colon X\to Y\) va mostrato che per ogni \(n\ge 0\) il quadrato
 ...
 \[\begin{tikzcd}[ampersand replacement=\&,cramped]
  ...
  \end{tikzcd}\]
 è commutativo (nella categoria dei gruppi, dato che per \(n=1\) non c'è motivo \(\pi_1(X),\pi_1(Y)\) siano abeliani). Questo segue dalla definizione delle mappe coinvolte: come detto, in orizzontale, una classe di omotopia \([f \colon S^n \to X]\) viene mandata in \(H_n(f)(\upsilon)\); in verticale, \([f]\) viene mandata in \([g\cmp f \colon S^n \to X\to Y]\), cosicché la funtorialità di \(H_n\) (e la sua invarianza per omotopia), permette di concludere.
\end{hExample}
\begin{example}\label{ex_cat_sset}\index{Insieme simpliciale}
 La categoria \(\ctsSet=\presh{\bsDelta}\) degli insiemi simpliciali ha per oggetti i funtori \(\bsDelta^\op\fun\ctSet\) definiti in \ref{def_insieme_simpliciale}, dove \(\bsDelta\) è la categoria dei simplessi di \ref{rmk_delta_e_deltaPlus}, e per frecce le trasformazioni naturali tra questi funtori.

 Una trasformazione naturale \(f \colon X_*\nat Y_*\) è detta una \emph{mappa simpliciale}, e consiste di una famiglia di funzioni \(f_n \colon X_n \to Y_n\) tra gli insiemi di \(n\)-simplessi, che sono compatibili con le mappe di faccia e di degenerazione: questo significa che ogni diagramma
...
\begin{remark}[Nervo di una categoria piccola]\label{def_nervo_di_una_cat}\index{Nervo!--- di una categoria}
 ...
 \begin{itemize}
  \item Gli 0-simplessi di \(N\ctC\) sono gli oggetti di \(\ctC\); gli 1-simplessi	sono le frecce di \(\ctC\), e per \(n\ge 2\) gli \(n\)-simplessi di \(N\ctC\) consistono delle \(n\)-uple di frecce componibili di \(\ctC\), cioè
        \[(N\ctC)_n \defeq  \{C_0 \xto{f_1} C_1 \xto{f_2}C_2\xto{f_3}\cdots \xto{f_{n-1}} C_{n-1}\xto{f_n} C_n\mid \tup[0]Cn,\in\ctC_0\}.\]
        Perciò possiamo denotare il generico \(n\)-simplesso di \(N\ctC\) come una \(n\)-upla ordinata \([\tup fn,]\).
  \item Le mappe di faccia \(d_i \colon (N\ctC)_n \to (N\ctC)_{n-1}\) sono definite, a seconda che \(i=0\) o \(i=n\), come l'omissione del primo o dell'ultimo termine della sequenza, e altrimenti come la composizione di due frecce adiacenti:
        \[\begin{cases}
          d_0[\tup fn,] \defeq  [\tup[2]fn,]                                           \\
          d_n[\tup fn,] \defeq  [\tup f{n-1},]                                         \\
          d_i[\tup fn,] \defeq  [\tup f{i-1}, ,f_{i+1}\cmp f_i, \tup[i+2]fn,] & 0<i<n.
         \end{cases}\]
  \item Le mappe di degenerazione \(s_i \colon (N\ctC)_n \to (N\ctC)_{n+1}\)	sono definite come l'inserimento di una copia dell'\(i\)-esimo termine (cioè della sua freccia identica) nella sequenza:
        \[\begin{cases}
          s_0[\tup fn,] \defeq  [\Id[C_0],\tup fn,]                         \\
          s_n[\tup fn,] \defeq  [\tup fn,,\Id[C_n]]                         \\
          s_i[\tup fn,] \defeq  [\tup fi, ,\Id[C_i], \tup[i+1]fn,] & 0<i<n.
         \end{cases}\]
 \end{itemize}
\end{remark}
...
\begin{proof}
 ...
 \medskip
 Va ora dimostrato che la corrispondenza \(N \colon \ctCat \fun \ctsSet\) è un funtore, ovvero che un funtore tra categorie piccole \(F \colon \ctC\fun\ctD\) induce una trasformazione naturale \(NF\colon N\ctC\nat N\ctD\) tra i rispettivi nervi, e in più che \emph{ogni} mappa simpliciale \(f \colon N\ctC\nat N\ctD\) origina da un (unico) funtore.

 Del resto, una tale \(f\) è definita da delle componenti \((f_0,f_1,f_2,\dots)\), e dalla definizione sappiamo che \((N\ctC)_0,(N\ctC)_1,(N\ctC)_2\) sono rispettivamente gli oggetti, le frecce e le coppie di frecce componibili di \(\ctC\); altrettanto è vero per \(\ctD\), e allora \(f_0 \colon \ctC_0\to\ctD_0\) è la candidata funzione sugli oggetti di un funtore, \(f_1 \colon \ctC_1\to\ctD_1\) la funzione sulle frecce. La compatibilità di \(f_0,f_1\) con l'unica degenerazione \(s \colon (N\ctC)_0\to(N\ctC)_1\), definita da \(s[C] = [\Id[C]]\) afferma che \(f_1(\Id[C]) = \Id[f_0C]\). Similmente, la compatibilità di \(f_2\) con la faccia \(d_1\), che consiste della mappa di composizione \(N\ctC_2\to N\ctC_1\), afferma che \(f_1\) preserva anche le composizioni.
\end{proof}
\begin{definition}\label{def_natutra}\index{Trasformazione naturale}\index{homCDFG@\(\Hom{\ctCat(\ctC,\ctD)}(F,G)\)}
 Come visto in \ref{ex_cat_cat} i funtori sono le frecce di una categoria \(\ctCat\) delle categorie (piccole); ma abbiamo appena trovato che anche le trasformazioni naturali tra funtori rendono la classe \(\ctCat(\ctC,\ctD)\) dei funtori \(F \colon \ctC\funto\ctD\) a sua volta una categoria; in più la legge di composizione orizzontale \(\blank\horcomp\blank \colon\ctCat(\ctD,\ctE)\times\ctCat(\ctC,\ctD) \to \ctCat(\ctC,\ctE)\) di \ref{def_hcomp} è un bifuntore (la regola di interscambio di \ref{interchangio} equivale ad affermare questo).

 Allora le trasformazioni naturali \(\alpha \colon F\nat G\) si pensano come ``frecce tra frecce'', o ``frecce di dimensione 2'', nel senso che andiamo a spiegare. La definizione \ref{def_nat} ha introdotto una notazione per le trasformazioni naturali: è comune rappresentarle con delle frecce del tipo \(\To\), a riempire un diagramma di funtori come segue:
 \[
...
\begin{notation}\index{Trasformazione naturale}
 Dovremmo denotare \(\Hom{\ctCat(\ctC,\ctD)}(F,G)\) la collezione delle trasformazioni naturali tra due funtori, cioè delle frecce nella categoria \(\ctCat(\ctC,\ctD)\). Spesso però è conveniente usare una notazione più compatta, specie quando \(\ctC,\ctD\) sono determinate dal contesto: una scelta comune per riferirsi a \(\Hom{\ctCat(\ctC,\ctD)}(F,G)\) è \(\Nat(F,G)\) (ma alcuni autori usano convenzioni ancora più compatte come \([F,G]\)). È conveniente denotare esplicitamente dominio e codominio di \(F,G\), perciò noi introduciamo la notazione
 \[\natHom\ctC\ctD FG \defeq \Hom{\ctCat(\ctC,\ctD)}(F,G)\]
\end{notation}
\begin{remark}\index{Categoria!--- di funtori}
 Sarà spesso utile considerare esempi in cui \(\ctD\) non è una categoria piccola (l'esempio più illustre e pervasivo è \ref{def_yoneda_embeddu}), e sarà altrettanto utile considerare esempi di funtori tra categorie entrambe larghe; però la definizione della `categoria' \(\ctCat(\ctC,\ctD)\) pone un problema di teoria degli insiemi quando \(\ctC\) non è piccola; infatti, in quel caso la classe delle trasformazioni naturali tra due funtori \(F,G\colon\ctC\funto\ctD\) è una sottoclasse di \(\prod_{C\in\ctC_0}\ctD(FC,GC)\), che in generale può non essere un insieme (per esempio, può coincidere con \(\prod_{C\in\ctC_0}\ctD(FC,GC)\), quando?).
\end{remark}
...
\begin{hDefinition}[Isomorfismo naturale]{fund}\label{def_isonat}\index{Isomorfismo!--- naturale}\index{Trasformazione naturale!isomorfismo naturale}
 Dati due funtori \(F,G \colon \ctC\fun\ctD\) tra categorie, un \emph{isomorfismo naturale} consiste di una trasformazione naturale \(\alpha \colon F\nat G\) che possieda una inversa per l'operazione di composizione verticale di \ref{def_vcomp}; in altre parole, \(\alpha\) è un isomorfismo naturale se esiste una trasformazione naturale \(\bar\alpha \colon G\nat F\) tale che \(\alpha\vcmp\bar\alpha = 1_G\) e \(\bar\alpha\vcmp\alpha =1_F\).
\end{hDefinition}
Evidentemente tale \(\bar\alpha\) è unica e si può denotare con \(\alpha^{-1}\). Indichiamo con \(F\natIso G\) la presenza di un isomorfismo naturale (il cui nome \(\alpha\) è sottinteso) tra i funtori \(F\) e \(G\); questo è un modo di suggerire l'idea che l'isomorfismo tra \(F\) e \(G\) (nella categoria \(\Hom{\ctCat}(\ctC,\ctD)\), tra gli oggetti \(F\) e \(G\)) sia fatto di diverse componenti (le \(\alpha_X\)), ciascuna delle quali è un isomorfismo \(\alpha_X \colon FX\iso GX\).

Osserviamo poi che la condizione di naturalità per \(\alpha^{-1}\) può essere omessa:
\begin{lemma}\label{natiso_inversa}\index{Isomorfismo!--- naturale}
 Una trasformazione naturale \(\alpha \colon F\nat G\) è invertibile se e solo se ciascuna delle sue componenti è un isomorfismo di \(\ctD\); infatti, la condizione di naturalità per \(\alpha\) è il fatto che per ogni \(u \colon X\to Y\) si abbia
...
\end{lemma}
\begin{definition}[Funtore rappresentabile]\label{def_rappre_fun}\index{Funtore!--- rappresentabile}\index{Rappresentabilità}
 Una famiglia importante di isomorfismi naturali sono quelli che sussistono tra un funtore \(F \colon \ctC \fun\ctSet\) (covariante o controvariante) e uno dei \emph{funtori hom} (covariante o controvariante) di \ref{ex_hom_funtore};
 \begin{itemize}
  \item Se \(G \colon \ctC^\op\fun\ctSet\) è controvariante, qualora esista un isomorfismo naturale \(G\natIso \Hom{\ctC}(\blank,Y_G)\) per un qualche oggetto \(Y_G\in\ctC_0\), diciamo che \(G\) è \emph{rappresentabile} (da \(Y_G\)), o che \(Y_G\) è l'\emph{oggetto rappresentante} per \(G\).
  \item Dualmente, se \(F \colon \ctC\fun\ctSet\) è covarinte, qualora esista un isomorfismo naturale \(F\natIso \Hom{\ctC}(X_F,\blank)\) per un qualche oggetto \(X_F\in\ctC_0\), diciamo che \(F\) è \emph{corappresentabile} (da \(X_F\)), o che \(X_F\) è l'\emph{oggetto corappresentante} per \(F\).
 \end{itemize}
 Questa terminologia è indicativa del fatto che la (co)rappresentabilità sia una proprietà di \(G\) (di \(F\)), ossia che se \(G\) è rappresentabile, lo sia in modo essenzialmente unico. Questo è vero (ed è il cuore del capitolo \ref{cap_yoneda}, sul lemma di Yoneda), ma bisogna sviluppare più teoria per poterlo dimostrare. Gli esempi di funtori (co)rappresentabili abbondano in algebra, geometria e logica e sono raccolti nelle prime sezioni del capitolo \ref{cap_yoneda}.
\end{definition}
\begin{hLemma}[Presentazione equivalente di trasformazioni naturali]{fund}\index{Categoria!--- freccia generica}\index{Freccia generica}
 Si considerino due categorie \(\ctC,\ctD\). È equivalente dare:
 \begin{enumtag}{tn}
  \item \label{tn_1} una trasformazione naturale \(\alpha \colon F\nat G\), per due funtori \(F,G \colon \ctC\toto\ctD\) paralleli;
  \item \label{tn_2} un funtore \(A \colon \ctC\times \genArrow \fun \ctD\), dove \(\ctC\times \genArrow\) è il prodotto di categorie (si veda \ref{def_cat_prodotto}) tra \(\ctC\) e la freccia generica (si veda \ref{ex_cat_freccia}).
 \end{enumtag}
 Più in particolare, è equivalente dare:
 \begin{enumtag}{in}
  \item \label{in_1} un isomorfismo naturale \(\alpha \colon F\nat G\), per due funtori \(F,G \colon \ctC\toto\ctD\) paralleli;
  \item \label{in_2} un funtore \(J \colon \ctC\times \ctIso \fun \ctD\), dove \(\ctC\times \ctIso\) è il prodotto di categorie (si veda \ref{def_cat_prodotto}) tra \(\ctC\) e l'isomorfismo generico (si veda \ref{ex_cat_iso}).
 \end{enumtag}
 \begin{proof}
  Dato un funtore \(A\) come in \ref{tn_2}, la restrizione \(A|_{\{0\}}\) di \(A\) alla sottocategoria generata da \(\{0\}\subset\genArrow\) è un funtore \(\{0\}\times\ctC\cong \ctC \fun\ctD\); chiamiamolo \(F\); altrettanto è vero per il funtore \(G = A|_{\{1\}} \colon \{1\}\times\ctC\cong \ctC \fun\ctD\). Sia poi \(u \colon 0\to 1\) l'unica freccia non identica; allora \(A(\id_C,u) \colon A(C,0) \to A(C,1)\) è una freccia di \(\ctD\) della forma \(FC \to GC\) per la definizione appena data. Del resto, in \(\ctC\times \genArrow\) esiste un ovvio quadrato commutativo,
  ...
  \[\begin{tikzcd}[ampersand replacement=\&,cramped]
   ...
   \end{tikzcd}\]
  per ogni \(f \colon C \to C'\) (la cui diagonale è \((f,u) \colon (C,0) \to (C',1)\)); la funtorialità di \(A\) manda questo quadrato in un quadrato commutativo della forma
  ...
  \[\begin{tikzcd}[ampersand replacement=\&,cramped]
   ...
   \end{tikzcd}\]
  Cosicché le componenti \(\alpha_{A,C} \defeq  A(\id_C,u)\) definiscono una trasformazione naturale \(\alpha_A \colon F\nat G\).

  Vice versa, una trasformazione naturale \(\alpha \colon F \nat G\) definisce un funtore \(A_\alpha\) ponendo
  \begin{itemize}
   \item \(A_\alpha(X,0) \defeq  FX\) e \(A_\alpha(Y,1)=GY\) per ogni \(X,Y\in\ctC_0\);
   \item \(A_\alpha(f,\id_0) \defeq  Ff\) e \(A_\alpha(g,\id_1)=Gg\) per ogni \(f,g\in\ctC_1\);
   \item \(A_\alpha(C,u) \defeq  \alpha_C \colon FC \to GC\) per la definizione appena data di \(A_\alpha\) sugli oggetti.
  \end{itemize}
...
\begin{example}[Trasformazioni naturali tra mappe monotòne]\label{nat_tra_posets}\index{Trasformazione naturale!--- tra ordini}
 Abbiamo visto in \ref{exa_monotone_funtori} che una funzione monotòna \(f\colon(X,\le)\to(Y,\le)\) si può vedere come un funtore. In questo contesto, date due funzioni monotone \(f,g\colon(X,\le)\to(Y,\le)\), si ha una (unica) trasformazione naturale \(f\Rightarrow g\), o \(f\le g\), se e solo se per ogni \(x\in X\), \(f(x)\le g(x)\). (Quest'ultima disuguaglianza rappresenta la componente della trasformazione nell'oggetto \(x\).)
...
% 	Abbiamo visto nell'Esempio~\ref{exa_azioni_funtori} che dato un gruppo \(G\), un funtore \(\susp G\to\ctSet\) è un insieme \(X\) con un'azione di \(G\) su \(X\). Dati due funtori \(\susp G\to\ctSet\), i.e.\ due insiemi \(X\) e \(Y\) con un'azione di \(G\), una trasformazione naturale tra questi due funtori è una funzione \(f\colon X\to Y\) che commuta con l'azione di \(G\), nel senso che per ogni \(g\in G\), il seguente diagramma commuta,
% 	\[
% 		\begin{tikzcd}
...
\begin{example}\label{doppio_duale}\index{Trasformazione naturale!--- sul biduale}
 ...
 \[\begin{tikzcd}
   ...
   \arrow[from=1-1, to=1-2]
  \end{tikzcd}\]
 dove per ogni \(W\), \(\dual W \defeq \Hom{\ctVect}(W,k)\) è lo spazio vettoriale delle mappe lineari \(W\to k\); \(\epsilon_V(v)\) è la mappa lineare di \emph{valutazione} \(\dual V\to k\) che manda \(\alpha \colon V\to k\) in \(\alpha(v)\). Questo isomorfismo è naturale, dato che una mappa lineare \(f \colon V\to W\) fa commutare il quadrato
 ...
 \[\begin{tikzcd}
  ...
  \end{tikzcd}\]
 (la freccia verticale destra è definita mandando \(\Phi \colon \dual V \to k\) nella mappa \(\Phi \cmp \dual f\), che manda \(\alpha \colon V\to k\) in \(\Phi(\alpha\cmp f)\in k\)).
\end{example}
\begin{warning}\label{il_duale_singolo_no}\index{Spazio duale}
 È altrettanto noto che si può definire un isomorfismo \(\theta_V \colon V\cong \dual V\), ma per farlo è essenziale \emph{scegliere} una base \(\mathcal{B}\) di \(V\) (ossia, l'isomorfismo \(\theta_V^{\mathcal{B}}\) non può essere costruito senza questa scelta, e ne dipende, laddove invece l'isomorfismo di \ref{doppio_duale} è  \emph{polimorfo} in \(V\), cioè è definito sempre alla stessa maniera, uniformemente nel `parametro' \(V\)).
 ...
 \[\begin{tikzcd}
  ...
  \end{tikzcd}\]
 Il problema è più profondo del fatto che, strettamente parlando, le trasformazioni naturali sono state definite tra funtori della stessa varianza;\footnote{Questa restrizione si può aggirare definendo una trasformazione naturale sghemba tra un funtore covariante \(F \colon \ctC\fun\ctD\) e un funtore controvariante \(G \colon \ctC^\op\fun\ctD\) come una famiglia \(\alpha_C \colon FC\to GC\) di frecce di \(\ctD\) con la proprietà che per ogni \(f\in \Hom{\ctC}(X,Y)\) si abbia \(\alpha_X = Gf\cmp\alpha_Y\cmp Ff\). Questi esempi sono relativamente comuni, ma abbiamo mostrato che in questo contesto nemmeno una trasformazione naturale sghemba può esistere.} infatti, se una famiglia \(\theta^{\mathcal{B}}_V\) esistesse, per ogni mappa lineare \(f \colon V\to W\) sarebbe vero che \(\theta_V(v)(v') = \theta_W(fv)(fv')\), cosicché (scegliendo \(f=0\)) \(\theta_V(v)(v')=0\), che significa \(\theta_V=0\), il che è assurdo: abbiamo supposto \(\theta_V\) invertibile.
\end{warning}
\begin{example}\label{il_determinante}\index{Trasformazione naturale!determinante come ---}
 ...
 Il determinante fornisce un esempio di trasformazione naturale \(\det \colon \text{GL}_n\natto (-)^\times\) di componenti
 \[\dmFun{\det_R}{\text{GL}_n(R)}{R^\times}\]
\end{example}
Un funtore \(F \colon \ctC\fun\ctC\) si dice \emph{puntato} (risp., \emph{copuntato}) da una trasformazione naturale \(\eta \colon \id_\ctC \nat F\) (risp., \(\epsilon \colon F \nat \id_\ctC\)).
\begin{example}[Co/puntature e co/moltiplicazioni]\index{Funtore!--- co/puntato}
 ...
 \begin{itemize}
  \item Nella categoria degli insiemi, per un \(A\in\ctSet_0\) fissato, il funtore \(SX \defeq  X^A\times A\) è copuntato dalla mappa di riduzione o \emph{valutazione}
        \[\dmFun{\epsilon_X}{X^A\times A}X\]
        che manda una coppia \(\pair fa\) in \(\epsilon(f,a) = f(a)\); il fatto che le funzioni \(\epsilon_X\) formino una famiglia naturale in \(X\) (ma non in \(A\)) si riduce al fatto che \((u\cmp f)(a) = u\big(f(a)\big)\) per ogni \(u \colon X\to Y\) (oppure usando un formalismo diverso, nel fatto che coincidano le due assegnazioni
        ...
        \[\begin{tikzcd}[ampersand replacement=\&]
         ...
         \end{tikzcd}\]
  \item il funtore potenza covariante \(\ctP \colon \ctSet\fun\ctSet\) di \ref{ex_fun_parti} è puntato dalla famiglia di mappe \(\eta_X \colon X\to \ctP X\) date da \(x\mapsto \{x\}\); la naturalità si traduce nel fatto (ovvio) che \(\ctP_*(f)(\{x\}) = \{f(x)\}\) per ogni \(f \colon X\to Y\). Similmente, assegnare a un insieme \(X\) l'insieme delle liste in \(X\), come in \ref{mongruppi_liberi}, è un funtore puntato \((\blank)^* \colon \ctSet\fun\ctSet\), dalla famiglia \(\theta_X \colon X\to X^* \colon x\mapsto x\cons \emptyList = [x]\), e la naturalità di \(\theta\) si traduce nel fatto (ovvio) che \(f^*[x] = [f(x)]\) per ogni \(f \colon X\to Y\);
 ...
 \end{itemize}
 Oltre a essere puntati o copuntati, i funtori \(S\) e \(\ctP\) di prima hanno associate delle trasformazioni naturali, rispettivamente, di tipo \(\sigma\colon S \natto S\cmp S\) e \(\bigcup\colon\ctP\cmp\ctP \natto \ctP\) definite come segue:
 \begin{itemize}
  ...
  \item Da \ref{nat_tra_posets} sappiamo che una trasformazione naturale \(\alpha \colon f\nat g\) tra mappe monotòne consiste della disuguaglianza \(f(x)\le g(x)\) vera punto per punto nel loro codominio. Allora, una mappa monotòna \(f \colon X\to X\) di un insieme ordinato \((X,\le)\) è un funtore puntato se e solo se è una \emph{espansione}:
        \[\forall x\in X.x\le f(x);\]
        Dualmente, \(g \colon Y\to Y\) di un insieme ordinato \((Y,\le)\) è un funtore copuntato se e solo se è una \emph{contrazione}:
...
\begin{definition}[Legge distributiva]\index{Legge distributiva}\index{Funtore!Legge distributiva tra ---i}
 Sia \(\ctC\) una categoria, e \(F,G \colon \ctC\fun\ctC\) due endofuntori; una \emph{legge distributiva} di \(F\) su \(G\) consta di una trasformazione naturale \(\alpha \colon FG\nat GF\).
\end{definition}
\begin{example}\index{Funtore!--- delle liste}\index{Legge distributiva}
 Esistono due funtori \(F,G \colon \ctSet\fun\ctSet\) definiti come segue:
 \begin{itemize}
 ...
 \end{itemize}
 Esiste una legge distributiva \(\alpha \colon FG\nat GF\) con componenti
...
\begin{hExample}[Un esempio di isomorfismo naturale]{tech}\label{puniv_prodotto_naturale}\index{Trasformazione naturale!isomorfismo naturale}
 ...
 \[\begin{tikzcd}
   ...
   \arrow[from=1-1, to=1-2]
  \end{tikzcd}\]
 mandando una funzione \(u \colon A\to B\times C\) nella coppia \(\pair{\pi_B\cmp u}{\pi_C\cmp u}\), se \(\pi_B \colon \pair bc\mapsto b\) e \(\pi_C \colon \pair bc\mapsto c\) sono le proiezioni sui due fattori del prodotto. Si osservi che
 \begin{itemize}
  ...
  \item la funzione \(\varpi^A_{BC}\) è naturale in tutti i suoi argomenti, ossia \(\varpi^A_{BC}\) sono le componenti di un isomorfismo naturale \emph{di funtori}.
        ...
        Quanto appena detto significa che per ogni terna \(u \colon A'\to A\), \(v \colon B\to B'\) e \(w \colon C\to C'\) di funzioni, il diagramma di funzioni
        ...
        è uguale a\dots{})
 \end{itemize}
 Usando il funtore `diagonale' \(\Delta \colon \ctSet^\op \fun\ctSet^\op\times\ctSet^\op\) di \ref{es_di_funtori} e ricordando da \ref{exam_bifuntori}.\ref{eb_2} che esiste un funtore prodotto \(\blank\times\blank \colon \ctSet\times\ctSet \fun\ctSet\) definito mandando \(X,Y\mapsto X\times Y\) e \(u \colon X\to X',v \colon Y\to Y'\) in \(u\times v \colon \pair xy\mapsto \pair{ux}{vy}\), \(\varpi\) consta di una trasformazione naturale invertibile che `riempie' il diagramma
...
\begin{remark}[Gli isomorfismi dell'aritmetica sono naturali]\label{iso_aritmetici}
 ...
 \begin{enumtag}{as}
  ...
  %
  \item\label{as_4} \(A\times \singleton\cong \singleton \times  A \cong A\) (naturale in \(A\), e \(\singleton\) è un insieme singoletto);
 ...
 \end{enumtag}
 Se \(\ctA,\ctB,\ctC\) sono categorie, isomorfismi del tutto analoghi valgono, naturali in tutti gli argomenti, in \ref{as_1} per la categoria vuota di \ref{ex_cat_vuota} e in \ref{as_4} per la categoria singoletto di \ref{ex_cat_term}.
\end{remark}
\begin{proposition}\index{Gruppo lineare}\index{Yoneda!lemma di ---}
 Si ricordi che in \ref{il_determinante} e \ref{ex_fun_GLn} abbiamo definito un funtore \(\GL_2 \colon \ctcRing\fun\ctGrp\); proviamo ora che esiste un isomorfismo naturale (precisamente, una biiezione naturale di insiemi, che però può essere promossa a un isomorfismo di gruppi) tra \(\GL_2\) e il funtore \(\Hom{\ctcRing}(S(\GL_2),-)\) per l'anello \(S(\GL_2)\) ottenuto dal quoziente
 \[\bbZ[a,b,c,d,y]/(1-(ad-bc)y)\]
 in poche parole, \(S(\GL_2)\) è ottenuto aggiungendo formalmente all'anello dei polinomi \(\bbZ[a,b,c,d]\) un inverso all'elemento \(y=ad-bc\). In virtù del teorema di isomorfismo per anelli, un omomorfismo \(S(\GL_2) \to R\) è determinato da un omomorfismo \(\varphi \colon \bbZ[a,b,c,d,y] \to R\) che contiene l'ideale \(J=(1-(ad-bc)y)\) nel nucleo; ma questo significa esattamente scegliere quattro elementi \(\alpha,\beta,\gamma,\delta\in R\) soggetti alla condizione che \(\alpha\delta-\beta\gamma\) sia un'unità (l'immagine di \(y\) mediante \(\varphi\)), e questo fissa un unico elemento \(\left(\begin{smallmatrix}					\alpha	&	\beta \\					\gamma	&	\delta				\end{smallmatrix}\right)\) di \(\GL_2(R)\).

 La biiezione \(\GL_2(R)\to \ctcRing(S(\GL_2),R)\) così determinata è naturale in \(R\), come è facile vedere dal modo in cui un omomorfismo di anelli \(f \colon R\to R'\) agisce mediante il funtore \(\GL_2\).
 ...
 dove \(\det\) è il polinomio \(\sum_{\sigma\in \fkS(n)}\prod_{i=1}^n x_{i,\sigma(i)}\) nelle indeterminate \(x_{ij}\) (e la somma è indicizzata sugli elementi \(\sigma \in \fkS(n)\) del gruppo simmetrico di ordine \(n\)).
\end{proposition}
...
\begin{definition}[Composizione orizzontale di trasformazioni naturali]\label{def_hcomp}\index{Composizione orizzontale!di trasformazioni naturali}
 Si considerino tre categorie \(\ctC,\ctD,\ctE\), due funtori \(F,G\colon\ctC\to\ctD\) e due funtori \(F',G'\colon\ctD\to\ctE\). Date due trasformazioni naturali \(\alpha\colon F\Rightarrow G\) e \(\alpha'\colon F'\Rightarrow G'\), la \emph{trasformazione naturale composta orizzontalmente} \(\alpha'\horcomp\alpha\colon F'\cmp F\Rightarrow G'\cmp G\), rappresentata come segue,
 \[
...
\begin{definition}[\Whisk di un funtore e una trasformazione naturale]\label{def_whiskering}\index{Innesto}
 ...
 È facile verificare che queste due definizioni dànno trasformazioni naturali (in un caso, \(H\) applicato a ciascun quadrato di naturalità per \(\alpha\) ne preserva la commutatività; nell'altro, stiamo restringendo \(\alpha\) alle sole componenti della forma \(X=KA\)).
\end{definition}
...
\end{proposition}
\begin{proof}
 Diamo due dimostrazioni di questo risultato; chi legge ed è a suo agio con una, dovrebbe cercare di familiarizzare con l'altra. Chi conosce in che cosa consista il \emph{trucco di Eckmann-Hilton}\index{Eckmann@Beno Eckmann}\index{Hilton@Peter Hilton}\index{Eckmann-Hilton!trucco di ---} riconoscerà la seconda dimostrazione come un suo caso particolare.\footnote{Il `trucco di Eckmann-Hilton', che non è un trucco ma un vero e proprio teorema, prende il nome da un matematico svizzero, Beno Eckmann, e uno britannico, Peter Hilton; entrambi hanno dato importanti contributi alla topologia algebrica. L'enunciato è il seguente: sia \(X\) un insieme dotato di due operazioni binarie \(\circ,\star\); si assuma che entrambe \(\circ\) e \(\star\) abbiano un elemento neutro e che valga la regola di interscambio: per \(a,b,c,d\in X\) si ha \((a\star b)\circ(c\star d)=(a\circ c)\star (b\circ d)\); allora
  \begin{itemize}
   \item \(1_\circ=1_\star\);
   \item \(\circ\) e \(\star\) sono entrambe commutative e associative;
   \item per ogni \(x,y\in X\), \(x\circ y = x\star y\) (ossia, le due operazioni coincidono!).
  \end{itemize}
  Tra i corollari di questo (solo apparentemente) innocente risultato, il fatto che i gruppi di omotopia \(\pi_n\) di uno spazio topologico \(A\) sono tutti abeliani per \(n\ge 2\).}
 \begin{itemize}
  \item prima dimostrazione: una trasformazione naturale \(\alpha \colon \id_\ctC\To\id_\ctC\) consiste di una famiglia di mappe \(\alpha_X \colon X\to X\) indicizzata dagli oggetti di \(\ctC\), con la proprietà che \(\alpha_Y \cmp f = f\cmp \alpha_X\) per ogni morfismo \(f \colon X\to Y\) in \(\ctC\); del resto, se \(\beta \colon \id_\ctC\To\id_\ctC\) è un altro elemento di \(\zentrum(\ctC)\), questo vuol dire che \(\alpha_X \cmp \beta_X = \beta_X\cmp \alpha_X\), cosicché \((\alpha\cmp\beta)_X = (\beta\cmp\alpha)_X\), ossia le trasformazioni naturali \(\alpha\cmp\beta\) e \(\beta\cmp\alpha\) coincidono.
...
\begin{hRemark}[A proposito dei diagrammi di trasformazioni naturali]{skip}\index{Trasformazione naturale!incollamento di ---i}
 ...
 \begin{enumerate}
  \item come un diagramma commutativo in una categoria di funtori \(\Hom{\ctCat}(\ctA,\ctB)\), che ha per oggetti i funtori \(\ctA\fun\ctB\) e per frecce le trasformazioni naturali;
 ...
 di frecce nella categoria \(\Hom{\ctCat}(\ctC,\ctE)\) dei funtori di tipo \(\ctC\fun\ctE\).
\end{hRemark}
...
\begin{definition}\label{def_yoneda_embeddu}\index{Yoneda!immersione di ---}
 ...
 \begin{itemize}
  ...
  \item Sulle frecce, manda \(u \colon C\to C'\) nella trasformazione naturale \(\yon_\ctC u \colon \yon_\ctC C \nat \yon_\ctC C'\) la cui componente ad \(X\in\ctC_0\) è la funzione
 ...
 \end{itemize}
 La naturalità di \(\yon_\ctC u\), così come la funtorialità (cioè il fatto che \(\yon_\ctC (v\cmp u)\) è la composizione verticale di \(\yon_\ctC v\) e \(\yon_\ctC u\)) si verificano senza difficoltà (chi legge è invitato a ripetere come esercizio \emph{importante} il ragionamento, eventualmente con notazioni proprie): il fatto che \(\yon_\ctC u\) è naturale significa che data una freccia \(g \colon X\to Y\), il quadrato
 ...
 è commutativo.
\end{definition}
\begin{hRemark}[A proposito del simbolo \(\yon_\ctC\)]{skip}\index{Yoneda@Nobuo Yoneda}\index{aaa_yo@\(\yon_\ctC\)}
 L'immersione di Yoneda è l'unico funtore di questo testo denotato con un carattere non latino o greco. Si legge come lo `\emph{io}' di p\emph{io}ggia, ed è la prima sillaba, nel sistema di trascrizione hiragana degli ideogrammi giapponesi, del nome di famiglia `Yoneda' (\jap{米田 信夫}; in giapponese il cognome precede il nome), di Nobuo Yoneda (\(*\)1930-\(\dag\)1996), matematico e informatico teorico giapponese. È difficile riassumere il profilo e i contributi di Yoneda alla matematica in poche righe: a parte dare il nome al celebre \emph{lemma di Yoneda}, risultato che da solo occupa (almeno) un intero capitolo di questo libro, Yoneda è stato uno dei pionieri nello studio delle categorie abeliane e dei loro usi in algebra omologica \cite{foo}, nonché uno dei primi a verificare un gran numero di casi della congettura di Collatz \cite{foo}.
\end{hRemark}
\begin{remark}\index{Prefascio}\index{Funtore!Prefascio}\index{coprefascio|see {Prefascio}}
 ...
 bisogna però scegliere come frecce \(F\nat G\) tra due funtori \(F,G \colon \ctC\fun\ctSet\) le trasformazioni naturali \(G\nat F\). Questa scelta permette di dualizzare \ref{def_yoneda_embeddu} definendo l'\emph{immersione di coYoneda}
 \[\dmFun{\coyon_\ctC}{\ctC}{\copresh\ctC}\]
 definita mandando \(C\in\ctC_0\) nel funtore hom covariante di \ref{ex_hom_funtore}, e una freccia \(f \colon C\to C'\) nella funzione tra hom insiemi (\emph{ibid.}). Si osservi che allora $\coyon_\ctC = (\yon_{\ctC^\op})^\op$ nella notazione di \ref{es_di_funtori}.\ref{exfun_6}.

 Si osservi che le immersioni di Yoneda e coYoneda si ottengono `saturando' nel primo o nel secondo argomento il bifuntore hom di \ref{ex_hom_funtore}. Questa che sembra una banalità è una importante relazione tra \(\yon_\ctC,\coyon_\ctC\) e il bifuntore hom \(\Hom{\ctC}(\blank,\blank)\).
\end{remark}
...
\begin{definition}[Il nervo di un funtore]\label{def_nervo_funtore}\index{Funtore!nervo di un ---}\index{Nervo!---	di un funtore}
 Ogni funtore \(F \colon \ctC \fun\ctD\) induce un bifuntore
 ...
 (indicato con lo stesso nome) che manda \(D\) nel funtore \(\Hom{\ctD}(F\blank,D) \colon C\mapsto \Hom{\ctD}(FC,D)\)
\end{definition}
...
che riguarda un insieme totalmente ordinato \([n]\in\ctFOrd\) come la categoria \(\bsDelta[n] \defeq \{0\to 1\to\cdots\to n\}\) (a sua volta, il funtore \(\bsDelta\) è la restrizione \(t|_{\ctFOrd}\), nella terminologia di \ref{ord_come_cat}).
\begin{esercizi}
 ...
 sia un bifuntore (nel senso di \ref{def_bifuntore}) quando le tre categorie di funtori hanno per frecce le trasformazioni naturali.
 ...
 Domande analoghe (per chi sa interpretare questa notazione) per la trasformazione naturale \(\theta \colon M\nat M \cmp M\) definita da
 \[\theta_X = \begin{cases}
