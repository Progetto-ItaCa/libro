...
\begin{example}\index{Categoria!---e di funtori}
	...
	\begin{itemize}
		...
		\item Esiste una categoria \(\ctSpc\) che ha per oggetti le specie combinatorie di \ref{ex_fun_specie}; una freccia in \(\ctSpc\)	è una trasformazione naturale tra funtori \(\ctBij \fun\ctSet\), e dunque consta di una famiglia di funzioni \(\alpha_X : F(X)\to G(X)\) che commuta (è equivariante) con le funzioni indotte da ogni funzione \(f:X\to Y\).\index{Specie combinatoria}
	\end{itemize}
...
\begin{definition}[Funtore rappresentabile]\label{def_rappre_fun}\index{Funtore!--- rappresentabile}\index{Rappresentabilità}
	Una famiglia importante di isomorfismi naturali sono quelli che sussistono tra un funtore \(F : \ctC \fun\ctSet\) (covariante o controvariante) e uno dei \emph{funtori hom} (covariante o controvariante) di \ref{ex_hom_funtore};
	\begin{itemize}
		\item Se \(G : \ctC^\op\fun\ctSet\) è controvariante, qualora esista un isomorfismo naturale \(G\natIso \Hom{\ctC}(\blank,Y_G)\) per un qualche oggetto \(Y_G\in\ctC_0\), diciamo che \(G\) è \emph{rappresentabile} (da \(Y_G\)), o che \(Y_G\) è l'\emph{oggetto rappresentante} per \(G\).
		\item Dualmente, se \(F : \ctC\fun\ctSet\) è covarinte, qualora esista un isomorfismo naturale \(F\natIso \Hom{\ctC}(X_F,\blank)\) per un qualche oggetto \(X_F\in\ctC_0\), diciamo che \(F\) è \emph{corappresentabile} (da \(X_F\)), o che \(X_F\) è l'\emph{oggetto corappresentante} per \(F\).
	\end{itemize}
...
\begin{hLemma}[Presentazione equivalente di trasformazioni naturali]{fund}\index{Categoria!--- freccia generica}\index{Freccia generica}
	...
	\begin{proof}
		...
		\[\begin{tikzcd}[ampersand replacement=\&,cramped]
			...
			\end{tikzcd}\]
		Cosicché le componenti \(\alpha_{A,C} \defeq  A(\id_C,u)\) definiscono una trasformazione naturale \(\alpha_A : F\nat G\).
		...
		\begin{itemize}
			...
			\item \(A_\alpha(C,u) \defeq  \alpha_C : FC \to GC\) per la definizione appena data di \(A_\alpha\) sugli oggetti.
		\end{itemize}
...
\begin{example}\label{doppio_duale}\index{Trasformazione naturale!--- sul biduale}
	...
	\[\begin{tikzcd}
			...
			\arrow[from=1-1, to=1-2]
		\end{tikzcd}\]
	dove per ogni \(W\), \(\dual W \defeq \Hom{\ctVect}(W,k)\) è lo spazio vettoriale delle mappe lineari \(W\to k\); \(\epsilon_V(v)\) è la mappa lineare di \emph{valutazione} \(\dual V\to k\) che manda \(\alpha : V\to k\) in \(\alpha(v)\). Questo isomorfismo è naturale, dato che una mappa lineare \(f : V\to W\) fa commutare il quadrato
...
\begin{definition}\label{def_yoneda_embeddu}\index{Yoneda!immersione di ---}
	...
	\begin{itemize}
		...
		\item Sulle frecce, manda \(u : C\to C'\) nella trasformazione naturale \(\yon_\ctC u : \yon_\ctC C \nat \yon_\ctC C'\) la cui componente ad \(X\in\ctC_0\) è la funzione
