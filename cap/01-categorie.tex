...
\begin{example}\label{ex_cat_ordini}
	...
	La categoria \(\ctPre\) ha per oggetti e morfismi gli insiemi preordinati e le funzioni monotòne. Chiaramente, l'identità \(\id_P : (P,\le)\to (P,\le)\) è una funzione monotòna, e la composizione di due funzioni monotòne resta monotòna.
...
\begin{remark}[po, wo e to]\label{po_wo_to}
	...
	Un'altra sottocategoria importante di \(\ctPre\) è quella degli insiemi \emph{totalmente ordinati} o \emph{toset}, dove tutti gli elementi sono confrontabili tra loro:
	\[\forall x,y\in P((x\le y)\lor (y\le x)).\]
	Da ultimo, una ulteriore sottocategoria importante di \(\ctPre\) è quella degli insiemi \emph{bene ordinati} o \emph{woset}, dove ogni sottoinsieme non vuoto ammette un elemento minimo.
\end{remark}
...
\end{definition}
\begin{example}
	La categoria opposta di un monoide
\end{example}
\begin{example}
	La categoria opposta di un poset
\end{example}
\begin{proposition}
	...
	\begin{itemize}
		...
		\item l'operazione di composizione \(\_\circ\_ : \ctC_2 \to \ctC_1\) dove ricordiamo che
		      \[\ctC_2 := \{\}\subseteq\ctC_1\times\ctC_1\]
	\end{itemize}
	tutto ciò soggetto alle equazioni
	\Todo{}
	(che vanno interpretate `in contesto', cioè ad esempio \((g,f) \in \ctC_2\vdash d(g\circ f)=d(f)\) eccetera).

	Il principio di dualità si enuncia come segue.
	Data una formula \(\Sigma\) nel linguaggio \(\mathsf{CT}\), possiamo costruire la formula duale con la sostituzione \(\Sigma[d/c,c/d,\circ/\circ^\op]\)\footnote{\`E chiaro cosa lo scambio di \(d,c\) significhi: prima si sostituisce \(d\) con un simbolo vergine \(\dot c\) e \(c\) con un simbolo vergine \(\dot d\), e poi si sosituisce \(\dot c\) con \(c\) e \(\dot d\) con \(d\):
		\[\Sigma \squig \Sigma[d/\dot c,c/\dot d]\squig \Sigma[\dot c/c,\dot d/d] =:\Sigma^\op\]
		ma questa pedanteria è eccessiva.} dove \(\_\circ^\op\_\) è definita come \(u\circ^\op v = v\circ u\) (e questa è una buona definizione, dopo lo scambio \(d\leftrightarrows c\)).
...
\begin{remark}
	...
	Ad esempio: nell'insieme parzialmente ordinato dei numeri naturali ogni sottoinsieme non vuoto ammette un minimo. Tale ordine parziale è una categoria per \ref{}; il minimo di un insieme ordinato ora si descrive in termini della relazione d'ordine (cioè della classe dei suoi morfismi); del resto, \(\bfN^\op = \{\dots\le 2 \le 1 \le 0\}\) non ha un minimo, bensì un massimo.
\end{remark}
...
\begin{definition}[Incubi e succube sopra un oggetto]\label{def_cat_slice}\index{Categoria!--- incubo}\index{Categoria!--- succuba|see {incubo}}\index{Categoria!--- sopra un oggetto}\index{Categoria!--- sotto un oggetto|see {sopra un oggetto}}
	...
	\begin{itemize}
		...
		\item i morfismi \((A,p)\to(B,q)\) sono morfismi \(f:A\to B\) di \(\ctC\) tali che \(q\circ f=p\), ovvero tali che il seguente diagramma commuta.
		      \[
	...
	\begin{itemize}
		...
		\item per morfismi \((A,p)\to(B,q)\), frecce \(f:A\to B\) di \(\ctC\) tali che \(q=f\circ p\), ovvero tali che il seguente diagramma commuta.
		      \[
			      \begin{tikzcd}[column sep=small]
...
\begin{example}[Intervalli]\label{ex_pos_slices}\index{Categoria!--- intervallo di un poset}
	Se \((\bfR,\le)\) denota l'insieme dei numeri reali linearmente ordinato nel modo usuale, ed \(x\in\bfR\) è un numero reale, la categoria \(\bfR/x\) è il semi-intervallo inferiormente illimitato \((-\infty,t]:=\{t\le x\}\), e analogamente \(x/\bfR\) è il semi-intervallo \([t,+\infty)\). Più in generale, in ogni insieme parzialmente ordinato \((P,\le)\) guardato come categoria, l'incubo \(P/x\) sopra \(x\in P\) consiste dell'intervallo \(\{t\le x\}\) dotato dell'ordine indotto da \(P\), e similmente accade per \(x/P\). (Se \(a,b\in\bfR\) con l'ordine usuale, e \(a\le b\), si noti anche che \((a/\bfR)/(a\le b)\) è esattamente l'intervallo limitato \([a,b]\).)
\end{example}
...
\begin{example}[Implicazioni in contesto]\label{ex_boole_cpc}\index{Categoria!--- del calcolo proposizionale}
	\`E noto dai corsi di logica elementare che l'insieme \(\{\varphi,\psi,\dots\}\) delle formule ben formate del calcolo proposizionale (classico) forma un'algebra di Boole rispetto all'ordine
	...
	questo insieme è solamente \emph{pre}ordinato.) Consideriamo la categoria \(\ctB/\varphi\); in base a \ref{} questa coincide con il segmento \(\{\alpha\le\varphi\}\), il quale in base alla definizione di \(\_\le\_\) si identifica all'insieme degli \(\alpha\) tali che \(\alpha\) implica \(\varphi\), dotato dell'ordine indotto.
...
\begin{enumerate}
	...
	\item Costruisci un esempio in \(\ctTop\) di oggetti \(X\) e \(Y\) non isomorfi, e morfismi \(f:X\to Y\) e \(g:Y\to X\) tali che \(g\cmp f=\id_X\) (ma \(f\cmp g \ne \id_Y\)).
	      ...
	      \item\label{isounico} In una categoria localmente piccola \(\ctC\), dati due oggetti isomorfi \(X\) e \(Y\), dimostra che per ogni oggetto \(A\), gli insiemi \(\Hom{\ctC}(A,X)\) e \(\Hom{\ctC}(A,Y)\) sono in corrispondenza biunivoca.
\end{enumerate}
...
\begin{example}\label{exa_monotone_funtori}\index{Funtore!mappa monotòna come ---}
	Abbiamo visto in \ref{ord_sonocat} che gli insiemi parzialmente ordinati si possono vedere come particolari categorie. 
	Dati due insiemi parzialmente ordinati \((X,\le)\) e \((Y,\le)\), un funtore \((X,\le)\to(Y,\le)\) è precisamente una funzione monotòna nel senso di \ref{ex_cat_ordini}. 
	Vediamo come. Innanzitutto, per ogni oggetto (cioè, in questo caso, elemento) \(x\) di \(X\), un oggetto (elemento) di \(Y\). In altre parole, abbiamo una funzione \(f:X\to Y\). 
	Sulle frecce, per ogni freccia \(x\to x'\) di \(X\) (ce n'è una, ed è unica, se e solo se \(x\le x'\)), una freccia \(f(x)\to f(x')\) (ancora, ce n'è una unica se s solo se \(f(x)\le f(x')\)). In altre parole, \(f\) ha la proprietà che se \(x\le x'\), allora \(f(x)\le f(x')\). Cioè, \emph{\(f\) è monotòna}. 
	L'identità è preservata automaticamente: per ogni \(x\), esiste un'unica freccia \(f(x)\to f(x)\), data dal fatto che l'ordine di \(Y\) è riflessivo. Per cui l'equazione \(f(\id_{x})=\id_{f(x)}\) in questo caso è una tautologia.
...
\begin{example}\label{exa_funtori_da_gruppi}\index{Funtore!omomorfismi di monoidi come ---}
	...
	Dati due monoidi \(M\) e \(N\), un funtore \(\ctB M\to \ctB N\) è precisamente un morfismo di monoidi nel senso di \ref{ex_cat_monoidi}.
	Innanzitutto, un funtore \(\ctB M\to \ctB N\) deve mappare l'unico oggetto di \(\ctB M\) in quello di \(\ctB N\) nell'unico modo possibile, e questo vale per tutti i funtori di questo tipo.
	...
	In questo caso, le condizioni sull'identità e la composizione non sono banali: la preservazione dell'identità ci dice che \(f(1_M)=1_N\), e la preservazione delle composizioni ci dice che per ogni \(m,m'\in M\) abbiamo \(f(m\cdot m')=f(m)\cdot f(m')\). 
\end{example}

\begin{example}\label{exa_azioni_funtori}\index{Funtore!azioni come ---}
	Dato un gruppo \(G\), un funtore \(\ctB G\to\ctSet\) è precisamente un \(G\)-insieme, i.e.\ un insieme con un'azione di \(G\), nel senso di \ref{ex_cat_g_insiemi}. 
	...
	In maniera simile, un funtore \(\ctB G\to\bbR\emdash\ctVect\) è una rappresentazione (lineare) di \(G\) su uno spazio vettoriale.
	
	Le stesse costruzioni si possono fare con un monoide invece di un gruppo.
\end{example}

\begin{example}\label{exa_derivata_funtore}\index{Funtore!differenziale come ---}
	Sia \(\cate{Euc}_*\) la categoria come segue:
	\begin{itemize}
		\item Gli oggetti sono \emph{spazi Euclidei puntati}, cioè coppie \((\bbR^n,x)\) con \(n\in\bbR\) e \(x\in\bbR^n\);
		\item I morfismi \((\bbR^n,x)\to (\bbR^m,y)\) sono funzioni differenziabili \(f:\bbR^n\to\bbR^m\) tali che \(f(x)=y\).
	\end{itemize}
	Possiamo costruire un funtore \(D:\cate{Euc}_*\to\bbR\emdash\ctVect\) nel seguente modo:
	\begin{itemize}
		\item Sugli oggetti, mappa \((\bbR^n,x)\) in \(\bbR^n\);
	...
	\begin{itemize}
		\item Il differenziale della funzione identità, ad ogni punto, è la matrice identità;
		\item Il differenziale della composizione di due funzioni è il prodotto matriciale dei differenziali.
	\end{itemize}
	(Se queste proprietà non fossero chiare, può essere di aiuto scriverle per esteso.)
\end{example}
...
\begin{definition}\label{compo_funtori}
	Dati due funtori \(F:\ctC\to\ctD\) e \(G:\ctD\to\ctE\), il \emph{funtore composto} \(G\circ F:\ctC\to\ctE\) assegna
	\begin{itemize}
	...
	\end{itemize}
	Il fatto che questo dia un funtore (i.e.\ che preservi identità e composizione) segue dal fatto che \(F\) e \(G\) sono funtori:
	\[
	G(F(\id_X)) = G(\id_{FX}) = \id_{GFX} ;
	\]
	\[
	G(F(g\circ f)) = G(Ff\circ Fg) = GFf\circ GFg .
	\]
\end{definition}
...
\begin{definition}[La categoria delle categorie]\label{ex_cat_cat}\index{Categoria!--- delle categorie}
	Come si può verificare, la composizione di funtori come definita sopra è associativa, e con il funtore identità definito sopra, è unitale.
	Per questo le categorie e i funtori formano una categoria. Per essere precisi:
	\begin{itemize}
		\item Chiamiamo \(\ctCat\) la categoria delle categorie piccole e i funtori tra loro;
		\item Chiamiamo \(\cate{CAT}\) la categoria delle categorie localmente piccole e i funtori tra loro.
	\end{itemize}
\end{definition}

\Todo{Va bene la convenzione sopra? Se no cambiatela senza problemi. --Paolo}
