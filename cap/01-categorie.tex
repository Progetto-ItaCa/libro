...
\begin{definition}[Categoria]\label{def_categ}
	...
	\begin{enumtag}{c}
		...
		\item\label{c_3} Ad ogni morfismo \(f\) corrispondono (unici) due oggetti \(\dom{f}\), \(\cod{f}\) chiamati \emph{dominio} e \emph{codominio}. Per denotare il fatto che \(f\) ha dominio \(X\in\ctC_0\) e codominio \(Y\in\ctC_0\), scriveremo \(f\colon X\to Y\), o in \emph{forma diagrammatica},
...
\begin{remark}\label{cor_def_categ}
	...
	\begin{itemize}
		...
		\item la composizione consta di una funzione \emph{parziale} (si noti la diversa freccia)
		      ...
		      Dato che i vari \(\Hom{\ctC}(X,Y)\) sono a due a due disgiunti, la composizione \(\_\cmp\_\) si `spezza' in una classe di funzioni
...
\end{example}
\begin{example}[Turcasso generico]\label{ex_cat_turcasso}
	Sia \(S\) un insieme; la categoria \(\turk\), detta \emph{turcasso generico} (con \(S\) elementi)\footnote{Il nome \emph{turcasso} denota un particolare tipo di faretra in uso agli arcieri turchi, solitamente fissata alla cintura dei soldati anziché portata a tracolla. Si veda \cite{}, 
	\begin{verse}
...
\begin{example}[Categoria dei sistemi dinamici discreti]\label{ex_cat_dyn}\index{Categoria!--- dei sistemi dinamici}\index{Sistema dinamico discreto}\index{Dyn@\(\ctDyn\)}
	...
	\begin{itemize}
		\item per oggetti le coppie \(((X,x_0),f)\) dove \((X,x_0)\) è un insieme puntato nel senso di \ref{}, ed \(f : X\to X\) è una endofunzione di \(X\);
...
\end{example}
\begin{example}[\(\mathrm{C}^*\)-algebre complesse]\index{Categoria!--- delle \(\mathrm{C}^*\)-algebre}\index{Cs-algebra@\(\mathrm{C}^*\)-algebra}
...
\end{example}
\begin{example}[La categoria degli insiemi pesati]\label{example_pesati}
...
\end{proposition}
\begin{remark}
	La definizione di categoria opposta data in \ref{def_cat_opp} Cat opposta per una categoria come forma, o come struttura. Meno quando si pensa a una categoria come universo...
\end{remark}
\begin{remark}
...
\begin{example}[Gruppoide d'azione]\label{action_groupoid}\index{Gruppoide!--- d'azione}
	...
	\end{figure}
\end{example}
\begin{definition}\index{Automorfismo}\index{Morfismo!auto---}
...
\begin{enumtag}{cf}
	\item \label{cf_1} per ogni coppia di elementi \(s,t \in FU\),
	\[(\forall i \in I.r_{U_iU}s = r_{U_iU} t) \,\Rightarrow\, s=t\]
	\item \label{cf_2} per ogni famiglia \(s_i \in FU_i\), tale che \(r_{U_{ij}U_i}s_i = r_{U_{ij}U_j}s_j\), esiste un (unico, in virtù della proprietà precedente) \(s\in FU\) tale che \(r_{U_iU}s=s_i\).
\end{enumtag}
Una corrispondenza del genere si dice un \emph{fascio} sullo spazio \(X\). La teoria delle equazioni differenziali ordinarie conduce naturalmente alla nozione di fascio, \cite[Cap. II]{mac1992sheaves}, \cite{Tennison1975,Borceux1994a,Borceux1989-gf,pedicchiofoundations}, e la geometria algebrica moderna è edificata su questa nozione (anche quando, però, gli spazi topologici considerati possono avere topologie molto diverse da quella di una varietà reale o complessa).
...
\begin{examples}[Alcuni esempi di bifuntori]
	\Todo{}
	Raccogliamo alcuni esempi di funtori di due o più variabili, nelle categorie introdotte in \ref{}.
	\begin{itemize}
		\item il bifuntore hom di \ref{}, che esiste per ogni categoria $\ctC$;
		\item 
		\item Esiste un bifuntore \(\_\times\_ : \ctSet\times\ctSet \fun\ctSet\) che manda un oggetto \((X,Y)\) del prodotto di categorie \(\ctSet\times\ctSet\) nell'insieme \(X\times Y\) e i morfismi \((f :A \to X, g : B\to Y)\) nel morfismo \(f\times g : A\times B \to X\times Y : (a,b)\mapsto (f(a),g(b))\).
		\item Esiste un bifuntore \(\_\oplus\_ : \ctVect\times\ctVect \fun\ctVect\) che, similmente a prima, manda due spazi vettoriali \(V,W\) nella loro \emph{somma diretta}, e due mappe lineari \(f : V\to V',g : W\to W'\) nella mappa lineare \(\left(\begin{smat} f & 0 \\ 0 & g \end{smat}\right) : V\oplus W \to V'\oplus W'\). Questo e il precedente esempio sono casi particolari del \emph{prodotto} di due oggetti rispettivamente nelle categorie \(\ctSet,\ctVect\).
		\end{itemize}
	\end{examples}
%  Diciamo \emph{matrice di insiemi} di taglia $A\times B$ una famiglia $M(a,b)$ di
% \begin{example}[Matrici di insiemi]\label{ex_cat_matrici_insiemi}
	
% \end{example}
% \begin{remark}
% 	Nell'esempio precedente possiamo pensare a $M(a,b) \in \bbN$ come a una cardinalità, e rifrasare la definizione come segue: un \emph{bimodulo} consiste di una funzione
% \end{remark}
\begin{remark}
...
\begin{definition}\label{def_cat_cocomma}
	...
	\[\begin{tikzcd}
		...
		\end{tikzcd}\]
\end{definition}
\begin{remark}[Alcuni esempi di commæ]
...
\end{remark}
\begin{definition}[La categoria degli elementi di un funtore]\label{elts_F}\index{Categoria!--- degli elementi}\index{EltsF@\(\Elts F\)}
...
\end{definition}
Più in generale, per ogni oggetto \(D\in\ctD_0\) e funtore \(F : \ctC\fun\ctD\), definiamo la categoria degli elementi di \(F\) in \(D\) come la categoria degli elementi \(\Elts\ctC{\Hom\ctD(D,F\_)}\) dove \(\Hom\ctD(D,F\_) : C\mapsto\Hom\ctD(D,FC)\); la \(\Elts\ctC F\) precedente si recupera quando \(F\) ha codominio \(\ctSet\), per \(D=\{*\}\) un singoletto.
...
\begin{remark}[Sull'immagine di un funtore]\label{im_non_subcat}\index{Funtore!immagine di un ---}\index{Immagine}
	L'immagine (stretta) di un funtore \(F : \ctC\fun\ctD\) può non essere una sottocategoria nel senso di \ref{}; un controesempio minimo è il seguente: il funtore $F$ definito da $Ff=v,Fg=u$ in
...
\begin{definition}[Immagine essenziale di $F$]\label{def_fun_imess}\index{Immagine essenziale}
	...
	Più formalmente, diciamo che una sottocategoria $\ctA\subseteq\ctD$ è \emph{repleta}\footnote{Dal verbo latino \emph{repleo}; si veda Dante, Purg. XXV, 70--72:
	\begin{verse}
...
\begin{itemize}
	\item un diagramma \emph{discreto} ha per dominio una categoria discreta della forma $A^\delta$, come in \ref{};
	\item un diagramma \emph{connesso} ha per dominio una categoria $\ctJ$ tale che $\pi_0^\ctCat(\ctJ)$ (si veda \ref{}) abbia un singolo elemento;
	\item un diagramma \emph{diretto} ha per dominio un ordinale come in \ref{}, guardato come categoria;
\end{itemize}
...
\end{definition}
\begin{examples}
	Esempi di categorie che sono filtrate: \Todo{}
	Esempi di categorie che non sono filtrate: \Todo{}
\end{examples}
\begin{esercizi}
...
\begin{example}\index{Funtore!--- insieme delle parti}\label{ex_fun_parti}
	Ad ogni insieme possiamo associare il suo \emph{insieme delle parti}, l'insieme \(\pow X = \{U\subseteq X\}\); questo definisce l'azione sugli oggetti di un funtore \(\ctP : \ctSet\fun\ctSet\),	e in effetti, di due:
	\begin{itemize}
		\item il \emph{funtore potenza covariante}, che manda una funzione \(f : X\to Y\) nella funzione
		      \[\dmFun{\ctP_* f}{\pow X}{\pow Y}\]
		      definita mandando \(U\subseteq X\) nell'insieme \(\ctP_* f[U] := \{y\in Y\mid \exists x\in U. (y = fx)\}\).
		\item il \emph{funtore potenza controvariante}, che manda una funzione \(f : X\to Y\) nella funzione
		      \[\dmFun{\ctP^* f}{\pow Y}{\pow X}\]
		      definita mandando \(V\subseteq Y\) nell'insieme \(\ctP^* f[V] := \{x\in X\mid f(x) \in V\}\).
	\end{itemize}
...
\end{example}
\begin{remark}[Ordini come categorie]\label{ord_come_cat}
	In maniera analoga la corrispondenza che assegna a un insieme ordinato \((P,\le)\) la categoria definita in \ref{} è la parte sugli oggetti di un funtore
	\[\dmFun{t}{\ctPos}{\ctCat.}\]
	...
	\`E usuale non denotare in alcun modo particolare questo funtore, cosicché `considerare la categoria \((P,\le)\)' ha come unico significato possibile considerare la categoria \(t(P,\le)\) i cui oggetti sono \(p\in P\) e dove c'è esattamente un morfismo \(p\to p'\) se e solo se \(p\le p'\).
\end{remark}
\index{Funtore!--- pieno}
\index{Funtore!--- fedele}
\index{Funtore!--- pienamente fedele}
\begin{example}[Le componenti connesse di un (multidi)grafo]\index{Funtore!--- componenti connesse di un grafo}\index{pi0@\(\pi_0\)|see {componenti connesse}}\index{Componenti connesse!--- di un grafo}\label{ex_fun_cpt_conn}
...
\end{definition}
\begin{notation}
	Coerentemente con la notazione usata finora, dovremmo denotare \(\Hom{\ctCat(\ctC,\ctD)}(F,G)\) la collezione delle trasformazioni naturali tra due funtori, cioè dei morfismi nella categoria \(\ctCat(\ctC,\ctD)\). \`E però più conveniente usare una notazione più compatta, specie quando \(\ctC,\ctD\) sono univocamente determinate dal contesto: una scelta comune è \(\Nat(F,G)\) (ma alcuni autori usano convenzioni ancora più compatte). \`E conveniente denotare esplicitamente dominio e codominio di \(F,G\), perciò noi introduciamo la notazione
...
\begin{example}[Co/puntature e co/moltiplicazioni]
	\Todo{Raccogliamo alcuni esempi di funtori puntati e copuntati:
		\begin{itemize}
			\item Nella categoria degli insiemi, per un $A\in\ctSet_0$ fissato, il funtore $FX := X^A\times A$ è copuntato dalla mappa di riduzione o \emph{valutazione}
			\[\dmFun{\epsilon_X}{X^A\times A}X\]
			che manda una coppia $\pair fa$ in $\epsilon(f,a) = f(a)$; il fatto che le funzioni $\epsilon_X$ formino una famiglia naturale in $X$ (ma non in $A$, si veda \ref{} per approfondire) si riduce al fatto che $(u\cmp f)(a) = u\big(f(a)\big)$ per ogni $u : X\to Y$ (oppure usando un formalismo diverso, nel fatto che coincidano le due assegnazioni 
...
\end{tikzcd}\]
			\item il funtore potenza covariante $\ctP : \ctSet\fun\ctSet$ di \ref{ex_fun_parti} è puntato dalla famiglia di mappe $\eta_X : X\to \ctP X$ date da $x\mapsto \{x\}$; la naturalità si traduce nel fatto (ovvio) che $\ctP_*(f)(\{x\}) = \{f(x)\}$ per ogni $f : X\to Y$. Similmente, assegnare a un insieme $X$ l'insieme delle liste in $X$, come in \ref{}, è un funtore puntato $(\_)^* : \ctSet\fun\ctSet$, dalla famiglia $\theta_X : X\to X^* : x\mapsto x\cons \emptyList = [x]$, e la naturalità di $\theta$ si traduce nel fatto (ovvio) che $f^*[x] = [f(x)]$ per ogni $f : X\to Y$;
			\item mandare una lista \emph{non vuota} nella sua testa $h_X(x\cons xs) =x$ è una trasformazione naturale dal funtore $(\_)^+$ delle liste \emph{non vuote} all'identità.
		\end{itemize}
		In più, possiamo trovare
		\begin{itemize}
			\item comoltiplicazione di \(X\times (-)^X\)
			\item unione come moltiplicazione di \(P\)
			\item ... % https://mathoverflow.net/questions/328513/big-list-of-comonads
			\item % https://ieeexplore.ieee.org/document/8005129
			\item % https://arxiv.org/pdf/1012.1220
		\end{itemize}}
\end{example}
\begin{example}
...
\end{remark}
\begin{hDefinition}[Invertitore]{skip}
...
\begin{esercizi}
	...
	\[\theta_X = \begin{cases}
			...
			(x\cons xs) & \mapsto \texttt{map}\kern.5em (x\cons\_)\kern.5em \theta_X (xs).
		\end{cases}
	\]
	...
	\begin{itemize}
		...
		\item Studiare la struttura di autogràfo \(X^\delta\) corrisponde all'azione banale \(s.x=x=t.x\) su un insieme \(X\) di vertici (che sono anche lati, cosicché l'autografo \(X^\delta\) ha per lati degli \(x\) della forma \(\xymatrix@C=4mm{x \ar@(ur,dr)[]^x}\)\dots).
	\end{itemize}
\end{esercizi}
...
\end{notation}
\begin{remark}
	Tra i funtori finora introdotti, quello dell'esempio \ref{exfun_4} è pieno; quelli degli esempi \ref{exfun_3}, \ref{dom_e_cod}, \ref{liberi_forgetti}, sono fedeli; quelli degli esempi \ref{ex_fun_co_disc}, \ref{funtore_mon_to_cat}, \ref{ord_come_cat}, sono pienamente fedeli.
\end{remark}
...
\end{definition}
Gli isomorfismi di categorie sono relativamente rari; è più comune trovarsi nella condizione in cui esistono due funtori \(F : \ctC \leftrightarrows \ctD: G\) con la proprietà che esistano \emph{isomorfismi naturali} \(\epsilon : F\cmp G \cong \Id[\ctD]\) ed \(\eta : \Id[\ctC]\cong G\cmp F\). Questa definizione più rilassata di identificazione tra due categorie si dice\emph{equivalenza}.
\begin{definition}[Equivalenza di categorie]\label{funtore_equcat}\index{Equivalenza}
	\Todo{Equivalenza di categorie }
	Una equivalenza di categorie \(\ctC\simeq\ctD\) consiste di una quadrupla \((F,G,\eta,\epsilon)\), dove
	\begin{itemize}
		\item
		\item
		\item
...
\begin{definition}[Funtori iniziali e finali]\label{def_fun_initfin}\index{Funtore!--- iniziale}\index{Funtore!--- finale|see {Funtore iniziale}}
	...
	\begin{itemize}
		...
		\item date due frecce \(D\to FX, D\to FY\) in \(\ctD\), esiste una successione finita di oggetti \(\tup An,\) e morfismi \(A_i \leftrightarrows A_{i+1}\) (in una delle due direzioni) tali che ogni triangolo nel diagramma
...
\begin{remark}[Condizione di Ore]\label{condizione_ore}\index{Funtore!condizione di Ore per un ---}\index{Funtore!--- iniziale}
	Una condizione più forte dell'inizialità, molto comune e più facile da verificare, è la \emph{condizione di Ore}: \(F : \ctC\fun\ctD\) soddisfa la condizione di Ore se 
	\begin{itemize}
		\item per ogni \(D\in\ctD\), l'insieme \(\sum_X\Hom\ctD(D,FX)\) è non vuoto;
...
\end{remark}
\begin{hTheorem}{skip}
	Per ogni categoria $\ctD$ (piccola e) filtrata nel senso di \ref{}, esiste un funtore iniziale $K : P_\ctD\fun\ctD$, di dominio un insieme parzialmente ordinato.
\end{hTheorem}
...
\begin{esercizi}
	...
	\item \label{ex_monepi_3} Costruire un'equivalenza di categorie tra la categoria dei sistemi dinamici discreti di \ref{ex_cat_dyn} e la categoria delle rappresentazioni, \ref{es_fun_repre}, del monoide \((\bbN,+,0)\); è un isomorfismo? Mostrare che \(\bbN\) è in maniera naturale un oggetto \(\bsN=(* \xto 0 \bbN \xto{\_+1} \bbN)\) di \(\ctDyn\), che soddisfa la proprietà (\emph{universale}) seguente:
	\begin{quote}
