...
\end{definition}
\begin{definition}
	\Todo{Join di categorie: come lo chiamiamo? Join o traduciamo? A me piace `giunto' di due categorie.}
\end{definition}
...
\begin{enumerate}
	...
	\item
	\item
  \item Determinare il join di due catene generiche $\Delta[n] * \Delta[m]$ come in \ref{ex_cat_catena} e di due cubi $P[n] * P[m]$ come in \ref{ex_quadcuboncubo}, e più in generale di due insiemi parzialmente ordinati $P,Q$. Determinare il join di due categorie discrete $A^\delta * B^\delta$ come in \ref{ex_cat_discreta} e di due categorie codiscrete $A^\chi * B^\chi$: il risultato è ancora codiscreto? Più in generale, determinare il join di due gruppoidi $\ctG * \ctG'$; il risultato è ancora un gruppoide? Determinare il join $\ctB(\bbN,+) * \ctB(\bbN,+)$ del monoide dei numeri naturali con la somma, e $\ctB(\bbN,\max) * \ctB(\bbN,\max)$ del monoide dove la moltiplicazione è data da $n\land m \defeq \max\{n,m\}$.
\end{enumerate}
