...
\begin{example}\index{Funtore!--- di compattificazione}
	...
	\begin{itemize}
		...
		\end{tikzcd}\]
		che manda $x\mapsto fx$ e $\infty_X$ in $\infty_Y$. La continuità sugli aperti della topologia di $Y$ è ovvia, e va verificata solo sugli aperti di $Y_\infty$ della forma $V=K^c\cup\{\infty_Y\}$ con $K$ compatto e chiuso in $Y$; del resto, ora, $f^{-1}V = (f^{-1}K)^c \cup \{\infty_X\}$ che è un intorno aperto di $\infty_X$.
	\end{itemize}
\end{example}
\begin{example}\index{Funtore!--- omotopia}\index{Funtore!--- omologia}
