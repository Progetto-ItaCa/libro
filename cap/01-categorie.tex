...
Questi ultimi esempi lasciano supporre che \emph{ogni} grafo \(\ctG\), fatto di un insieme di vertici (o `oggetti') \(V\) e di un insieme di lati (o `morfismi') \(E\) generi una categoria \(\bfF\ctG\). \`E effettivamente così, e la seguente costruzione in forma di esempio formalizza questa idea.
\index{Grafo}
...
\begin{example}[Co/puntature e co/moltiplicazioni]
	...
	\begin{itemize}
		\item Nella categoria degli insiemi, per un \(A\in\ctSet_0\) fissato, il funtore \(SX := X^A\times A\) è copuntato dalla mappa di riduzione o \emph{valutazione}
		      ...
		      che manda una coppia \(\pair fa\) in \(\epsilon(f,a) = f(a)\); il fatto che le funzioni \(\epsilon_X\) formino una famiglia naturale in \(X\) (ma non in \(A\), si veda \ref{def_dinaturali} per approfondire) si riduce al fatto che \((u\cmp f)(a) = u\big(f(a)\big)\) per ogni \(u : X\to Y\) (oppure usando un formalismo diverso, nel fatto che coincidano le due assegnazioni
		      ...
		      \[\begin{tikzcd}[ampersand replacement=\&]
			      ...
			      \end{tikzcd}\]
		\item il funtore potenza covariante \(\ctP : \ctSet\fun\ctSet\) di \ref{ex_fun_parti} è puntato dalla famiglia di mappe \(\eta_X : X\to \ctP X\) date da \(x\mapsto \{x\}\); la naturalità si traduce nel fatto (ovvio) che \(\ctP_*(f)(\{x\}) = \{f(x)\}\) per ogni \(f : X\to Y\). Similmente, assegnare a un insieme \(X\) l'insieme delle liste in \(X\), come in \ref{mongruppi_liberi}, è un funtore puntato \((\_)^* : \ctSet\fun\ctSet\), dalla famiglia \(\theta_X : X\to X^* : x\mapsto x\cons \emptyList = [x]\), e la naturalità di \(\theta\) si traduce nel fatto (ovvio) che \(f^*[x] = [f(x)]\) per ogni \(f : X\to Y\);
	...
	\[\xymatrix{
			S \ar@{=>}[r]^-\sigma & S\cmp S & \ctP\cmp\ctP \ar@{=>}[r]^-\cup & \ctP
		}\] definite come segue:
	\begin{itemize}
		...
		\item \(\cup_X\) è la mappa di `unione' \(\dmFun{\cup_X}{\pow{(\pow X)}}{\pow X}\)
...
\begin{esercizi}
	...
	dove \(\times\) indica il prodotto cartesiano, e \(+\) la somma (unione disgiunta) di insiemi. Costruire una biiezione esplicita, e dimostrare la naturalità in \(A,B,C\) separatamente (cosicché a essere isomorfi siano i `trifuntori' \(({\_}_1\times {\_}_2)+({\_}_1\times {\_}_3)\) e \({\_}_1\times ({\_}_2+{\_}_3)\)).
...
\begin{definition}[Equivalenza di categorie]\label{funtore_equcat}\index{Equivalenza}
	...
	\begin{itemize}
		...
		\item valgono le seguenti equazioni:
		      \begin{itemize}
			      \item (equazione zig) \(...\) è uguale alla trasformazione naturale identica di \(F\);
			      \item (equazione zag) \(...\) è uguale alla trasformazione naturale identica di \(G\).
		      \end{itemize}
	\end{itemize}
...
\end{remark}
\begin{hTheorem}{skip}
	Per ogni categoria \(\ctD\) (piccola e) filtrata nel senso di \ref{def_cat_filtrata}, esiste un funtore iniziale \(K : P_\ctD\fun\ctD\), di dominio un insieme parzialmente ordinato.
\end{hTheorem}
