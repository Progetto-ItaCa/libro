...
\begin{definition}[Categoria]\label{def_categ}
	...
	\begin{enumtag}{c}
		...
		\item\label{c_4} Ogni oggetto \(X\) ha un morfismo distinto \(\id_X\colon X\to X\) chiamato \emph{identità}.
...
\end{tikzcd}
\]
per la composizione che appare in \ref{c_5} (e fare altrettanto per diagrammi più complessi): ora la composizione di $g$ e di $f$ è effettivamente $g\cmp f$; questo sposta semplicemente il problema, dato che ora le frecce scorrono in senso contrario al verso di lettura...
    % Il modello mentale che è utile avere in mente per comprendere la definizione di categoria è dato dall'Esempio \ref{ex_cat_insiemi};\footnote{Ma (avremo modo di ripeterlo diverse volte) \emph{esistono categorie i cui morfismi non sono funzioni tra insiemi}.} la notazione per la composizione di morfismi `$\cmp$' è infatti la stessa usata in algebra per denotare la composizione di funzioni, che applica la funzione `$f$' all'argomento `$x$' scrivendo $f(x)$, e compone $g$ con $f$ scrivendo $(g\cmp f)(x):=g(f(x))$.
\end{remark}
\begin{definition}[Categoria piccola, categoria localmente piccola]
 ...
 una categoria che non è piccola si dice \emph{grande}, \emph{larga} o simili.
	...
	Invece, \(\ctC\) si dice \emph{localmente piccola} se dati ogni due oggetti \(X\) e \(Y\), la classe \(\Hom{\ctC}(X,Y)\) di morfismi da \(X\) a \(Y\) è un insieme. 
\end{definition}
 ...
 La categoria di \emph{tutti} gli insiemi finiti non è, strettamente parlando, piccola (perché c'è una classe propria anche solo di insiemi con un singolo elemento: \(\{\varnothing\},\{\{\varnothing\}\}, \{\{\{\varnothing\}\}\}\), eccetera), ma è localmente piccola, perché fissati due insiemi \(X,Y\), la collezione delle funzioni \(f : X\to Y\) è contenuta (stessa idea di prima) in \(2^{X\times Y}\), e quest'ultimo è un insieme (per assioma). La differenza tra le due situazioni è che l'unione \(\bigcup_{X,Y}2^{X\times Y}\) è indiciata su una classe propria, e quindi è una classe propria.
\begin{remark}
...
\begin{example}\label{ex_cat_freccia}
	La categoria `freccia generica' ha due oggetti \(0,1\) e un unico morfismo non identità \(u : 0\to 1\) (che di solito non ha nome). La composizione è forzata a essere definita solamente tra le identità ed \(u\), nell'unico modo ovvio, e tutti gli assiomi di categoria sono banalmente soddisfatti.
\end{example}
	...
	\begin{figure}[h]
		\begin{center}
			\begin{tikzpicture}
				...
				\draw[-latex] (0'') -- (2'');
				%--
				\fill (9,0) circle (1pt) node[below] {\(0\)};
				\node (0''') at (9,0) {};
				\fill (9.5,0.866) circle (1pt) node[above] {\(2\)};
				\node (2''') at (9.5,0.866) {};
				\fill (10,0) circle (1pt) node[below] {\(1\)};
				\node (1''') at (10,0) {};
				\fill (10.5,0.866) circle (1pt) node[above] {\(3\)};
				\node (3''') at (10.5,0.866) {};
				\draw[-latex] (0''') -- (1''');
				\draw[-latex] (1''') -- (2''');
				\draw[-latex] (0''') -- (2''');
...
\begin{example}\label{ex_cat_doppiafreccia}
	La categoria `doppia freccia generica' ha due oggetti \(0,1\) ed esattamente due morfismi non identità e paralleli \(s,t : 0\to 1\)%
...
\begin{example}[Spanne e cospanne generiche]\label{ex_spancospan}
	...
	Dualmente, la categoria `cospanna generica' \(\Lambda^2_2\) ha tre oggetti \(0,1,2\) ma i due morfismi non identità puntano verso \(2\) invece che uscire da \(0\):
	\[\begin{tikzcd}
		0\ar[r] & 2 & 1\ar[l]
	\end{tikzcd}\]
...
\end{tikzpicture}
     \end{center}
     \caption{Concatenazione di due sequenze $[\optup[0] e4;]$ e $[\optup[4]f7;]$ di morfismi in $F\ctG$.}
...
\begin{quote}
	Non tutte le categorie larghe astratte sono categorie di insiemi strutturati. Ad esempio, la categoria di \ref{ex_cat_hotop} non ha la proprietà di essere \emph{concreta} (si veda \ref{ssec:categorie_strutture}).
\end{quote}
...
\begin{example}\label{ex_cat_rel_equiv}
	...
	Gli assiomi di identità e associatività sono automaticamente soddisfatti: per esempio, la composizione \((w\to x\to y)\to z\) e la composizione \(w\to (x\to y\to z)\) coincidono, constando dell'unico morfismo \(w\to z\).
\end{example}
...
\begin{theorem}\label{mon_sonocat}
	Dato un monoide \((M,\cdot,1)\), definiamo la seguente categoria, che indichiamo con \(\ctB M\).\footnote{A volte la categoria definita in questo modo si indica con \(\susp(M,\cdot,1)\), a volte (quando l'operazione su \(M\) è ovvia dal contesto) semplicemente con \(\ctB M\).}
	\begin{itemize}
...
\end{example}
\begin{remark}[A proposito del nome `funtore']
	\`E sorprendentemente difficile attribuire la creazione del termine `funtore' a un singolo individuo.
	...
	Nel suo testo \cite{kromer}, Ralf Kr\"omer osserva che la terminologia di Carnap è stata, da quel momento in poi, largamente utilizzata; del resto a Carnap non va nemmeno il merito di avere coniato per primo la parola. 
	
	Haskell Curry \cite{Curry1961SomeLA} attribuisce invece più correttamente il termine a Tadeusz Kotarbiński, logico e filosofo polacco, che nel suo testo \emph{Elementy teorji poznania} (scritto cinque anni prima di \cite{carnappio}) introduce i funtori come `connettivi sentenziali' all'inizio del suo capitolo II. In \cite{Curry1961SomeLA} l'autore fa uso del concetto di funtore nel suo senso filosofico (più precisamente, in filosofia del linguaggio, dove un funtore è definito come un operatore che agisce sulle frasi di un dato linguaggio, per formarne altre, `preservando l'analisi logica', ossia le relazioni tra le parti della frase) per costruire il linguaggio giocattolo dei sami, teteli e tanteti.
...
che rendono commutativi i diagrammi in \ref{mor_spanne}. Riguardando le spanne $X_\bullet,Y_\bullet$ come diagrammi di forma $\Lambda^2_0 \to\ctC$, la terna $(\alpha_0,\alpha_1,\alpha_2)$ si può vedere come una freccia $X_\bullet \nat Y_\bullet$, nel senso che\Todo{}
% Ciò significa che c'è una qualche nozione di morfismo tra funtori, almeno in alcuni contesti. 

La nozione di \emph{trasformazione naturale} tra funtori rende precisa questa congettura.
...
Siccome i funtori si possono considerare a loro volta frecce, le trasformazioni naturali si possono vedere come ``frecce tra frecce'', o ``frecce di secondo ordine''. È comune rappresentarle con delle doppie frecce, come segue:
\[
...
\begin{example}\index{Equivarianza}
	Abbiamo visto nell'Esempio~\ref{exa_azioni_funtori} che dato un gruppo \(G\), un funtore \(\ctB G\to\ctSet\) è un insieme \(X\) con un'azione di \(G\) su \(X\). Dati due funtori \(\ctB G\to\ctSet\), i.e.\ due insiemi \(X\) e \(Y\) con un'azione di \(G\), una trasformazione naturale tra questi due funtori è una funzione \(f:X\to Y\) che commuta con l'azione di \(G\), nel senso che per ogni \(g\in G\), il seguente diagramma commuta:
	\[
	\begin{tikzcd}
...
\begin{definition}[Composizione verticale di trasformazioni naturali]\label{def_vcomp}\index{Composizione verticale!di trasformazioni naturali}
	...
	La \emph{trasformazione naturale composta} \(\beta\circ\alpha:F\Rightarrow H\),
	...
	\[
	\begin{tikzcd}
		FX \ar{r}{\alpha_X} & GX \ar{r}{\beta_X} & HX
	\end{tikzcd}
	\]
\end{definition}
...
\begin{definition}[Composizione orizzontale di trasformazioni naturali]\label{def_hcomp}\index{Composizione orizzontale!di trasformazioni naturali}
	Si considerino tre categorie \(\ctC,\ctD,\ctE\), due funtori \(F,G:\ctC\to\ctD\) e due funtori \(F',G':\ctD\to\ctE\). Date tue trasformazioni naturali \(\alpha:F\Rightarrow G\) e \(\alpha':F'\Rightarrow G'\), la \emph{trasformazione naturale composta orizzontalmente} \(\alpha'\alpha:F'\circ F\Rightarrow G'\circ G\), rappresentata come segue,
	\[
	\begin{tikzcd}[sep=large]
	...
	\end{tikzcd}
	\]
	è definita in componenti da uno dei due percorsi equivalenti nel seguente diagramma in \(\ctE\),
	\[
	\begin{tikzcd}
