...
\begin{esercizi}
	...
	\item \label{jgdasoh_2} Descrivere in cosa consiste una struttura che soddisfa tutti gli assiomi di \ref{def_categ}, a parte il fatto che \(d,c : \ctC_1\to\ctC_0\) sono funzioni parziali: in una `categoria bucata', esistono frecce \(\xymatrix{{} \ar[r]^f & Y}\) di \(\ctC\) che non hanno dominio, e alcune \(\xymatrix{X \ar[r]^g & {}}\) che non hanno codominio (possono esistere anche frecce \(\xymatrix{{} \ar[r]^p & {}}\)? Quante?).
...
\end{notation}
\begin{remark}
	Tra i funtori finora introdotti, quello dell'esempio \ref{exfun_4} è pieno; quelli degli esempi \ref{exfun_3}, \ref{dom_e_cod}, \ref{liberi_forgetti} sono fedeli; quelli degli esempi \ref{ex_fun_co_disc}, \ref{exa_monotone_funtori}, \ref{exa_funtori_da_gruppi} sono pienamente fedeli.
\end{remark}
...
\begin{esercizi}
	...
	\item \label{ex_monepi_5} Esiste un unico funtore \(u_M : \ctTerm\fun\susp M\) quando guardiamo un monoide \((M,\cdot,1)\) come una categoria con un solo oggetto, al modo di \ref{mon_sonocat}; in quale proprietà di \(M\) si traduce il fatto che \(u_M\) è un funtore iniziale?
\end{esercizi}