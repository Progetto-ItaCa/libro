...
\begin{remark}\label{klext_delle_relazioni}
	...
	dal momento che il lato sinistro è equivalente a \(\forall v\in V.\forall u\in U.(v,u)\in R\) e il lato destro a \(\forall u\in U.\forall v\in V.(v,u)\in R\). Nel capitolo \ref{cap_aggiunti} e in particolare in \ref{rel_danno_aggiunti} questo sarà un esempio elementare di una nozione importante, quella di \emph{aggiunzione} tra due categorie.
\end{remark}
...
\begin{remark}\label{varie_categorie_nella_pratica}
	Categorie di modelli come in \ref{ex_cat_sigma_strutture} sono tra le più comuni da considerare nella pratica matematica: i monoidi e i loro omomorfismi, i gruppi e i loro omomorfismi (tra essi, la sottocategoria dei gruppi abeliani), gli spazi vettoriali su un campo $k$ e le funzioni $k$-lineari, formano tutte esempi di categoria, che denotiamo con $\ctMon$, $\ctGrp$, $\ctAb$, $\ctVect_k$,\dots
\end{remark}
...
\begin{example}\label{rel_autoduale}
	Un esempio non banale di categoria autoduale è $\ctRel$.\Todo{}
\end{example}
...
\end{remark}
\begin{definition}[Funtori pieni, fedeli, essenzialmente suriettivi]
...
\begin{definition}\label{compo_funtori}
	Dati due funtori \(F:\ctC\to\ctD\) e \(G:\ctD\to\ctE\), il \emph{funtore composto} \(G\cmp F:\ctC\fun\ctE\) assegna
	\begin{itemize}
...
Osserviamo che dalla definizione di composizione orizzontale in \ref{} la composizione $\cmp$ è un bifuntore (si veda \ref{def_bifuntore}): da questo segue
...
\begin{example}
	...
	e i cui morfismi $\alpha : F\nat G$ sono le trasformazioni naturali, si chiama la categoria	dei \emph{prefasci} su $\ctC$.

	Varie notazioni sono utilizzate per indicarla: tra le più comuni:
	\[
	\widehat{\ctC},\quad \PSh(\ctC),\quad \presh\ctC
	\]
	(delle tre, l'ultima è la meno ambigua: è quella che impiegheremo noi).
\end{example}
Una colonna portante del pensiero moderno:
...
\end{hRemark}
Un risultato apparentemente semplice da dimostrare, ma dalle conseguenze molto profonde, la cui dimostrazione riposa sul risultato centrale del capitolo \ref{cap_yoneda}, è la seguente:
\begin{proposition}
	Per ogni categoria piccola $\ctC$, l'immersione di Yoneda $\yon_\ctC$ di \ref{def_yoneda_embeddu} è un funtore pienamente fedele.
\end{proposition}
(Nel capitolo \ref{cap_yoneda} deriveremo questo da un risultato ancora più generale. Rimandiamo chi legge a \ref{lemma_yoneda} e \ref{corollario_yo_ff})
\subsubsection*{Esercizi}
\begin{enumerate}
...
\end{tikzcd}\]
    ...
    \begin{itemize}
        ...
        \item Mostrare che $X\mapsto\ker(\alpha_X)$ definisce un funtore $K : \ctC\fun\ctAb$, e che $\kappa_X : \ker(\alpha_X) \to FX$ è naturale, e un monomorfismo in $[\ctC,\ctAb]$ (si veda \ref{}); a questa maniera, esiste una successione esatta \emph{di trasformazioni naturali} (cioè una successione di trasformazioni naturali, esatta quando saturata in ogni componente) della forma
...
Rammentiamo le definizioni date in \ref{}.
...
\end{example}
\begin{example}
	La categoria delle matrici di \ref{} è equivalente, senza essere isomorfa, a Vect.
\end{example}
\Todo{...qualcosa dopo...}
\begin{proposition}
	La classe dei monomorfismi di $\ctCat$ consiste dei funtori $F : \ctC\fun\ctD$ che sono fedeli e iniettivi sugli oggetti (cioè $F_0$ e tutte le $F_{XY}$ in \ref{} sono entrambe funzioni iniettive).
\end{proposition}
\begin{proof}
	\Todo{...}
\end{proof}
\subsubsection*{Esercizi}
\begin{enumerate}
	\item In \ref{} abbiamo dimostrato che i monomorfismi in $\ctCat$ sono i funtori iniettivi \emph{sugli oggetti e sui morfismi}. Trovare un funtore che non è iniettivo sugli oggetti, ma lo è sui morfismi, e mostrare che non è un mono. Idem per un funtore che non è iniettivo sui morfismi, ma lo è sugli oggetti.
	\item Mostrare che $\ctMat$ e $\ctVect$ \emph{non possono essere} categorie isomorfe (suggerimento: si veda la definizione di \emph{anima} di una categoria in \ref{def_cuore}: $\ctC^\text{an} := \sum_{C\in\ctC_0} \ctC_\cong(C,C)$; mostrare che due categorie isomorfe hanno anime isomorfe; mostrare che l'anima di $\ctMat$ e quella di $\ctVect$ non sono isomorfe.)
	\item
	\item
	\item
\end{enumerate}