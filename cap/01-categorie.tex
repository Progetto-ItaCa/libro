...
\end{definition}
Un esempio importante di rappresentazione di $G$ in una categoria è dato dai gruppi a un parametro di diffeomorfismi\footnote{Il nome `gruppo a un parametro' è un relitto del tempo antico quando la nozione di fibrato e di funtore erano le ultime innovazioni tecnologiche; esso è dovuto al fatto che si pensava a un'azione di $\bbR$ su una varietà $X$ come a una famiglia di diffeomorfismi $\varphi_t : X\to X$ dipendenti --appunto-- da un parametro reale $t\in\bbR$. Chiaramente, $\varphi_t = \varphi(t,-)$ per un'azione $\varphi : \bbR\times X\to X$ mediante diffeomorfismi.}
...
\begin{example}
	\Todo{Mandare un insieme nella sua categoria (co)discreta}
\end{example}
\begin{example}
