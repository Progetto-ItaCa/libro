...
\end{example}
\begin{remark}
	La categoria degli spazi vettoriali e mappe lineari, introdotta indirettamente in \ref{varie_categorie_nella_pratica}, e la categoria \(\bbF\emdash\ctMat\) ora definita, sono collegate in un senso preciso che sarà chiaro introducendo la nozione di \emph{scheletro} di una categoria in \ref{def_scheletro}: \(\bbF\emdash\ctMat\) si ottiene da \(\bbF\emdash\ctVect\) scegliendo un solo oggetto per ogni dimensione (dato che ogni \(\bbF\)-spazio vettoriale \(V\) di dimensione finita su \(\bbF\) è isomorfo ad un oggetto di \(\bbF\emdash\ctMat\)).
\end{remark}
...
\begin{examples}[Primi esempi di funtori]\label{es_di_funtori} Raccogliamo alcuni esempi elementari di funtori; la verifica che essi soddisfano le proprietà \ref{p_1} e \ref{p_2} è del tutto elementare e lasciata per esercizio.
	\begin{itemize}
		...
		\item \label{exfun_2} costante
		\item \label{exfun_3} diagonale \(\Delta : \ctC \fun\ctC\times\ctC\)
		\item \label{exfun_4} riduzione \(\nabla : \ctC+\ctC \fun\ctC\)
...
\end{example}
In maniera simile, un funtore \(\susp G\fun\bbR\emdash\ctVect\) consiste precisamente di una rappresentazione (lineare) di \(G\) su uno spazio vettoriale nel senso che è noto dall'algebra o dalla fisica matematica; le stesse costruzioni si possono fare con un monoide invece di un gruppo, e ciò giustifica la definizione seguente.
...
\begin{example}\label{exa_derivata_funtore}\index{Funtore!differenziale come ---}
	...
	\end{itemize}
	Possiamo costruire un funtore \(D:\ctEuc\fun\bbR\emdash\ctMat\) nel seguente modo:
	\begin{itemize}
...
\begin{example}\label{ex_hom_funtore}\index{Funtore!hom --- covariante}\index{Funtore!hom --- controvariante}
	L'esempio prototipico di \emph{funtore hom} controvariante, definito come segue. Sia \(\ctC\) una categoria, e \(X\) un suo oggetto. Definiamo un funtore
	\[\dmFun{\Hom{\ctC}(-,X)}{\ctC^\op}{\ctSet}\]
	come segue:
	\begin{itemize}
	...
	\end{itemize}
	Con questa definizione, è evidente che se \(f : A\to B\) è un morfismo di \(\ctC\), \(\hom(f,X)\) agisce come \(\hom(f,\id_X)\) e \(\hom(Y,f)\) agisce come \(\hom(\id_Y,f)\); ma allora, quando sia definito, \(\hom(f,g)\) agisce come \(\hom(f,\id_\bullet)\cmp\hom(\id_\bullet,g)\), cioè come \(\hom(\id_\bullet,g)\cmp\hom(f,\id_\bullet)\).
\end{example}
\begin{remark}
...
\begin{example}[Realizzare \(\bbF\emdash\ctMat\) in \(\bbF\emdash\ctVect\)]\index{Funtore!--- matrici reali}
	La categoria delle matrici \(\bbF\emdash\ctMat\) è stata definita in \ref{}; la categoria \(\bbF\emdash\ctVect\) è stata definita in \ref{}. Abbiamo già osservato che \(\bbF\emdash\ctMat\) si ottiene da \(\bbF\emdash\ctVect\) scegliendo un solo oggetto per ogni dimensione, e questo definisce un funtore ovvio \(\bbF\emdash\ctMat \to \bbF\emdash\ctVect\) (mandando \(n\) in \(\bbF^n\) e una matrice \(A\) nell'applicazione lineare che agisce come \(A\) nella base canonica). Esiste anche un funtore nella direzione opposta,
	\[\dmFun{Q}{\bbF\emdash\ctVect}{\bbF\emdash\ctMat}\]
...
\begin{hExample}[Un esempio di isomorfismo naturale]{tech}\index{Trasformazione naturale!isomorfismo naturale}
	(La natura di questo esempio sarà chiarita dal capitolo \ref{}, sulle proprietà universali, e dal capitolo \ref{}, sui funtori aggiunti.)
...
\begin{definition}[\Whisk di un funtore e una trasformazione naturale]
	...
	\[(H*\alpha)_B = H(\alpha_B) : H(FB) \to H(GB)\]
	...
	\[(\alpha * K)_A = \alpha_{KA} : FKA \to GKA.\]
...
\[(H*\alpha)*K=H*(\alpha*K)\]
(cioè su ogni componente si ha l'uguaglianza \(((H*\alpha)*K)_A= H(\alpha_{KA})=(H*(\alpha*K))_A\)) la quale rende la scrittura \(H * \alpha * K\) univocamente definita.
...
\end{remark}
\begin{remark}
	\Todo{A proposito di \whisk e iso naturali...}
\end{remark}
...
\end{example}
\begin{remark}
	\Todo{Definire anche \(\copresh\ctC\) e studiare le relazioni tra \(\presh-\) e \(\copresh-\).}
\end{remark}
...
\begin{proposition}
	Per ogni categoria piccola \(\ctC\), l'immersione di Yoneda \(\yon_\ctC\) di \ref{def_yoneda_embeddu} è un funtore pienamente fedele.
\end{proposition}
\subsubsection*{Esercizi}
\begin{enumerate}
...
\begin{example}
	...
	\Todo{ci è equivalente, se non ricordo male; ma non voglio entrare troppo nel dettaglio.}
...
\end{proposition}
\begin{proof}
Se $F : \ctC \fun\ctD$ è un mono in $\ctCat$, quando $X,Y \in\ctC_0$ si rappresentano come funtori $X,Y : \ctTerm \fun \ctC$, è evidente che $F\cmp X,F\cmp Y : \ctTm\to\ctD$, e se $F\cmp X$ è l'oggetto $F_0X$ (risp., $F\cmp Y$ è $F_0 Y$): se questi ultimi sono uguali, allora $X=Y$. In maniera simile, è possibile mostrare che ogni $F_{XY}$ è iniettiva.
\end{proof}
\subsubsection*{Esercizi}
\begin{enumerate}
	...
	\item Fissato un campo \(\bbF\), mostrare che \(\bbF\emdash\ctMat\) e \(\bbF\emdash\ctVect\) \emph{non possono essere} categorie isomorfe (suggerimento: si veda la definizione di \emph{anima} di una categoria in \ref{def_cuore}: \(\ctC^\text{an} := \sum_{C\in\ctC_0} \ctC_\cong(C,C)\); mostrare che due categorie isomorfe hanno anime isomorfe; mostrare che l'anima di \(\bbF\emdash\ctMat\) e quella di \(\bbF\emdash\ctVect\) non sono isomorfe.)
	\item
	\item
	\item
\end{enumerate}