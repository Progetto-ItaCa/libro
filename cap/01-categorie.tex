...
\end{definition}
\begin{remark}
Come è evidente, gli esempi di diagrammi delle forme più svariate abbondano: fissata una categoria $\ctC$, uno span in $\ctC$ come in \ref{def_span_e_cospan} è un diagramma di forma $\Lambda^2_0$ e un cospan è un diagramma di forma $\Lambda^2_2$. Un quadrato in $\ctC$ è un diagramma di forma $P[2]$, e un cubo $n$-dimensionale di forma $P[n]$. Una singola freccia $f : X\to Y$ in $\ctC$ si può vedere come un diagramma di tipo $\Delta[1]$, e più in generale una catena 
\[\begin{tikzcd}
