...
\end{definition}
\begin{remark}
	Come è evidente, gli esempi di diagrammi abbondano tra gli esempi fatti in precedenza: fissata una categoria \(\ctC\), una spanna in \(\ctC\) come in \ref{def_span_e_cospan} è un diagramma di forma \(\Lambda^2_0\) e una cospanna è un diagramma di forma \(\Lambda^2_2\). Un quadrato in \(\ctC\) è un diagramma di forma \(P[2]\), e un cubo \(n\)-dimensionale di forma \(P[n]\). Un singolo oggetto di \(\ctC\) si può vedere come un diagramma di forma \(\Delta[0]=\ctTerm\), e più in generale un insieme di cardinalità $|A|$ di oggetti di $\ctC$ è un diagramma di forma $A^\delta$, come in \ref{ex_cat_discreta}; una singola freccia \(f : X\to Y\) in \(\ctC\) si può vedere come un diagramma di tipo \(\Delta[1]\), e più in generale una catena
	\[\begin{tikzcd}
...
In \ref{} abbiamo osservato che le spanne e cospanne in $\ctC$ si possono vedere come funtori; del resto, in \ref{} abbiamo costruito una \emph{categoria} di spanne in $\ctC$. Ciò significa che c'è una qualche nozione di morfismo tra funtori, almeno in alcuni contesti. Vorremmo rendere precisa questa congettura.
...
\begin{example}\index{Equivarianza}
	...
	\[
	\begin{tikzcd}
		X \ar{d}{f} \ar{r}{g\cdot} & X \ar{d}{f} \\
		Y \ar{r}{g\cdot} & Y
	\end{tikzcd}
	\]
	In teoria delle rappresentazioni, questa condizione su \(f\) si chiama \emph{equivarianza}, ed è esattamente la condizione di naturalità istanziata in questo caso.
\end{example}

\begin{example}[Doppio duale]
\end{example}

\begin{example}[Mappa di Hurewicz]
\end{example}
...
\begin{definition}[Composizione verticale di trasformazioni naturali]\label{def_vcomp}\index{Composizione verticale!di trasformazioni naturali}
	...
	\[
	\begin{tikzcd}[sep=large]
		\ctC \ar[bend left=60, ""{name=F,below}]{r}{F} 
		\ar[""{name=Gup,above}, ""{name=Gdown,below}]{r}[near start]{G} 
		\ar[bend right=60, ""{name=H,above}]{r}[swap]{H} & \ctD
...
\begin{definition}\label{funtore_essurj}
  ...
  \Todo{Funtore essenzialmente suriettivo (nome alternativo: iso-denso)}
\end{definition}
...
Esercizio: i mono in $\ctCat$ sono i funtori iniettivi \emph{sugli oggetti e sui morfismi}. C'è un funtore che non è iniettivo sugli oggetti, ma lo è sui morfismi, e mostrare che non è un mono. Idem per un funtore che non è iniettivo sui morfismi, ma lo è sugli oggetti. 

Mostrare che Mat e Vect \emph{non possono essere} isomorfe (hint: costruire il gruppoide dei soli automorfismi di $\ctC$; il `nocciolo' di $\ctC$; mostrare che due categorie isomorfe hanno noccioli isomorfi. Mostrare che il nocciolo di Mat e quello di Vect non sono isomorfi.)
\Todo{Alcune parole sul fatto che in $\ctCat$ le equivalenze sono `meglio' degli isomorfismi.}