...
\begin{esercizi}
	...
	\item \label{jgdasoh_2} Descrivere in cosa consiste una categoria che soddisfa tutti gli assiomi di \ref{def_categ}, a parte il fatto che \(d,c : \ctC_1\to\ctC_0\) sono funzioni parziali: in una categoria `bucata', esistono frecce \(\xymatrix{{} \ar[r]^f & Y}\) di \(\ctC\) che non hanno dominio, e alcune \(\xymatrix{X \ar[r]^g & {}}\) che non hanno codominio (possono esistere anche frecce \(\xymatrix{{} \ar[r]^p & {}}\)? Quante?).
	\item \label{jgdasoh_3} \Todo{categoria dove s,t sono relazioni?}
	\item \label{jgdasoh_4} Verificare gli assiomi di categoria per \(\ctFin\) e \(\ctVect\) come sono state presentate in \ref{}.
...
\begin{example}[Categoria dei sistemi dinamici discreti]\label{ex_cat_dyn}\index{Categoria!--- dei sistemi dinamici}\index{Sistema dinamico discreto}\index{Dyn@$\ctDyn$}
	\Todo{Sistemi dinamici discreti}
\end{example}
...
\begin{esercizi}
	...
	\item Data una categoria \(\ctC\), possiamo definire una relazione sulla classe dei suoi oggetti ponendo
	\[A\preceq B \iff \Hom\ctC(A,B)\ne\varnothing\]
	Mostrare che \(\preceq\) è riflessiva e transitiva; è antisimmetrica? A che proprietà di \(\ctC\) corrisponde il fatto che il preordine \(\ctC^\bfP=(\ctC_0,\preceq)\) (detto la \emph{riflessione ordinata} di \(\ctC\)) ha un elemento minimo? A che oggetti corrispondono gli elementi \emph{minimali} di \(\ctC^\bfP\)?
...
\begin{definition}[Incubi e succube sopra un oggetto]\label{def_cat_slice}\index{Categoria!--- incubo}\index{Categoria!--- succuba|see {incubo}}\index{Categoria!--- sopra un oggetto}\index{Categoria!--- sotto un oggetto|see {sopra un oggetto}}
	...
	La \emph{categoria incubo} sopra \(X\), denotata \(\ctC/X\), o semplicemente \emph{categoria sopra} \(X\) è definita come segue:\footnote{Nel folklore medievale un \emph{incubo} è un demone maschile, che premendo sul petto dell'addormentato causa le paralisi notturne.

		Una succuba, invece, è una entità demoniaca femminile, che appare nei sogni soggiogando il sognatore e lo obbliga a un rapporto sessuale.}
	\begin{itemize}
...
\begin{esercizi}
	...
	\item Scrivere in cosa consiste un prefascio, nel senso di \ref{exa_funtori_da_poset}, quando \(P=2^X\) è l'insieme delle parti di un insieme \(X\), ordinato dall'inclusione. In cosa consiste un \emph{fascio} \(F : (2^X)^\op\to\ctSet\), cioè un funtore che soddisfa le due condizioni in \ref{cf_1}, \ref{cf_2}? (Suggerimento: \(2^X\) è generato dai suoi elementi minimali, ossia \(U\subseteq X\) risulta, ovviamente, da \(\bigcup_{x\in U} \{x\}\).)
\end{esercizi}
...
\begin{example}\index{Funtore!--- insieme delle parti}\label{ex_fun_parti}
	\Todo{funtore potenza covariante}
	\Todo{funtore potenza controvariante}
\end{example}
...
\end{example}
\begin{example}[Le componenti connesse di un (multidi)grafo]\index{Funtore!--- componenti connesse di un grafo}\index{pi0@\(\pi_0\)|see {componenti connesse}}\index{Componenti connesse!--- di un grafo}
...
\begin{remark}[Alcuni esempi di commæ]
	Importanti esempi di categorie sono particolari categorie commæ per opportune scelte di \(F\) o \(G\); l'esercizio \ref{} invita chi legge a esplicitare, tra le altre, \((F/E)\) e \((E/G)\) dove \(E\) è un oggetto guardato come funtore \(E : \ctTerm \fun\ctE\) alla maniera dell'esercizio \ref{}.
\end{remark}
...
\begin{esercizi}
	...
	\item \label{gfdpgubai_4} Si costruisca, tramite il differenziale, un funtore simile a quello di \ref{exa_derivata_funtore}, ma sulla categoria \(\cate{Mfd}\) delle varietà e funzioni lisce, che sugli oggetti assegna ad una varietà il suo fibrato tangente.
...
\begin{definition}\label{def_natutra}
	Come visto in \ref{} i funtori sono i morfismi di una categoria \(\ctCat\) delle categorie (piccole); ma abbiamo appena trovato che anche le trasformazioni naturali tra funtori rendono la classe \(\ctCat(\ctC,\ctD)\) dei funtori \(F : \ctC\funto\ctD\) a sua volta una categoria, dove le trasformazioni naturali \(\alpha : F\nat G\) si pensano come ``frecce tra frecce'', o ``frecce di dimensione 2'', nel senso che andiamo a spiegare. La definizione \ref{def_nat} ha introdotto una notazione per le trasformazioni naturali: è comune rappresentarle con delle frecce del tipo \(\To\), a riempire un diagramma di funtori come segue:
	\[
...
\begin{example}[Co/puntature e co/moltiplicazioni]
	\Todo{Raccogliamo alcuni esempi di funtori puntati e copuntati:
		\begin{itemize}
			\item valutazione
			\item insieme potenza come in \ref{ex_fun_parti}
			\item testa di liste non vuote
		\end{itemize}
...
\begin{esercizi}
	...
	\item Si denoti con $\{s,t\}$ un insieme con due elementi, ed $M =\{s,t\}^*$ il monoide libero su un insieme con due elementi, seguendo \ref{}; la categoria degli \emph{autografi} consiste della categoria delle rappresentazioni di $M$, nel senso di \ref{}. 
	\begin{itemize}
		\item Spiegare in che senso un autografo si descrive come un (multidi)grafo tale che `l'insieme dei lati coincide con l'insieme dei vertici';
		\item \Todo{}
	\end{itemize}
\end{esercizi}
...
\end{notation}
\begin{remark}
	Tra i funtori finora introdotti, quelli degli esempi \ref{exfun_4}, \ref{}, \ref{} sono pieni; quelli degli esempi \ref{exfun_3}, \ref{dom_e_cod}, \ref{liberi_forgetti} sono fedeli; quelli degli esempi \ref{ex_fun_co_disc}, \ref{exa_monotone_funtori}, \ref{exa_funtori_da_gruppi} sono pienamente fedeli; quelli degli esempi \ref{}, \ref{}, \ref{} sono iso-densi.
\end{remark}
...
\end{proof}
\begin{definition}
	\Todo{Funtore iniziale/finale (def con le comma)}
\end{definition}
\begin{esercizi}
