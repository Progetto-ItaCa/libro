...
\begin{itemize}
    ...
    \item date $\underline\alpha=(\alpha_0,\alpha_1,\alpha_2),\underline\beta=(\beta_0,\beta_1,\beta_2)$ che formano due distinti morfismi tra spanne, ne esiste una composta $\beta\cmp\alpha$ che è a sua volta un morfismo di spanne.
\end{itemize}
...
\end{example}
\begin{example}\Todo{Morfismi tra rappresentazioni di gruppi}
\end{example}
...
\begin{definition}[Composizione verticale di trasformazioni naturali]\label{def_vcomp}\index{Composizione verticale!di trasformazioni naturali}
	...
	La \emph{composizione verticale} \(\beta\circ\alpha:F\Rightarrow H\) delle transformazioni naturali $\beta,\alpha$, rappresentata come 
	\[
	\begin{tikzcd}[sep=large]
...
\end{tikzcd}\]
    ...
    \begin{itemize}
        ...
        \[(\ker\alpha)_C := \ker(\alpha_C : FC\to GC).\]
        \item Mostrare che $X\mapsto\ker(\alpha_X)$ definisce un funtore $K : \ctC\funto\ctAb$, e che $\kappa_X : \ker(\alpha_X) \to FX$ è naturale, e un monomorfismo in $[\ctC,\ctAb]$ (si veda \ref{}); a questa maniera, esiste una successione esatta \emph{di trasformazioni naturali} 
        \[0\natto\ker\alpha \overset\kappa\natto F \overset\alpha\natto G\]
        dove chiaramente `$0$' è il funtore costante nel gruppo abeliano nullo.
        \item Dualizzare e definire il conucleo di una trasformazione naturale; costruire la successione esatta
        \[\begin{tikzcd}
