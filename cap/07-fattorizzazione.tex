...
\begin{proposition}\label{ort_to_terminale}
	...
	\begin{enumtag}{ot}
		...
		\item\label{ot_2} il funtore \(\varHom{\ctC}(\blank,A)\) manda la freccia \(f\) in una biiezione
		\[\begin{tikzcd}
				\varHom{\ctC}(Y,A) \ar[r, "{g\cmp \blank}"] & \varHom{\ctC}(X,A).
			\end{tikzcd}\]
	\end{enumtag}
	...
	\begin{enumtag}{oi}
		...
		\item \label{oi_2} il funtore \(\varHom{\ctC}(B,\blank)\) manda la freccia \(g\) in una biiezione
		\[\begin{tikzcd}
				\varHom{\ctC}(Y,A) \ar[r, "{\blank\cmp f}"] & \varHom{\ctC}(Y,B).
			\end{tikzcd}\]
	\end{enumtag}
\end{proposition}
\begin{proof}
...
\begin{definition}[Prodotto di Leibniz]
	Siano \(f : A \to B\) e \(g : X\to Y\) due frecce di una categoria \(\ctC\). Il \emph{prodotto di Leibniz} di \(f\) e \(g\), denotato \(f\oast g\), si costruisce a partire dal prodotto fibrato
	\[
		\begin{tikzcd}
			\pull {\varHom{\ctC}(A,X)}{\varHom{\ctC}(B,Y)}{\varHom{\ctC}(A,Y)} \ar[r]\ar[d] & \varHom{\ctC}(B,Y) \ar[d, "\blank\cmp f"]\\
			\varHom{\ctC}(A,X) \ar[r, "g\cmp\blank"'] & \varHom{\ctC}(A,Y)
		\end{tikzcd}
	\]
	e consiste della mappa di confronto \(f\oast g : \varHom{\ctC}(B,X) \to \pull {\varHom{\ctC}(A,X)}{\varHom{\ctC}(B,Y)}{\varHom{\ctC}(A,Y)}\) indotta dalla proprietà universale.
	...
	\[
		\begin{tikzcd}
			\varHom{\ctC}(B,X) \ar[d]\ar[r,"g\cmp\blank"]& \varHom{\ctC}(B,Y)\ar[d, "\blank\cmp f"]\\
			\varHom{\ctC}(A,X) \ar[r, "g\cmp\blank"'] & \varHom{\ctC}(A,Y)
		\end{tikzcd}
	\]
	è un prodotto fibrato.
\end{definition}
Per orientare chi legge, è utile rendere esplicita la definizione del prodotto di Leibniz \(f\oast g\): una freccia \(u : B\to X\) viene mandata nella coppia \((u\cmp f, g\cmp u)\), che appartiene al prodotto fibrato in questione vista l'associatività della composizione: \(g\cmp (u\cmp f) = (g\cmp u)\cmp f\).
...
\end{quote}
\begin{proposition}
	Due frecce \(f,g\) sono ortogonali (cioè \(f\perp g\) nel senso di \ref{def_ortogona}) se e solo se il prodotto di Leibniz \(f\oast g\) è esatto.
\end{proposition}
\begin{proof}
