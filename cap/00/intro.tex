...
Fermarsi qui a raccontare la teoria delle categorie però tradirebbe parte della sua storia. Se questo testo parlasse, diciamo, di equazioni differenziali, partiremmo a contestualizzare il suo contenuto con il difficilissimo problema dei tre corpi introdotto da Poincaré, con la necessità di descrivere fenomeni elusivi attraverso un linguaggio generale. 

Con la teoria delle categorie è più difficile partire: che problema risolve? Da dove viene? 

Qualsiasi cosa se ne dica nei corridoi delle università, anche la teoria delle categorie ha un intento concreto e squisitamente \emph{pratico}, che si può riassumere nel desiderio, da parte di chi l'ha inventata, di astrarre ad una definizione generale una situazione come la seguente.
\begin{example}
...
La specifica delle funzioni `determinante' è \emph{polimorfa nel parametro} \(A\), ossia si può pensare (in modo impreciso, ma evocativo) come una funzione che ad \(A\) assegna una funzione \(\det_A\) di tipo \(M(A,2) \to A\). L'anello cambia, ma la funzione è definita sempre allo stesso modo, \emph{mutatis mutandis}.
