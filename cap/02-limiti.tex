...
\begin{hExamples}[Alcuni esempi di oggetti iniziali e terminali]{fund}
	...
	\end{enumtag}
\end{hExamples}
Questi esempi sono importanti per fissare l'idea in chi legge che un oggetto iniziale o terminale può essere `grande' o `complicato' e che sebbene la sua definizione sia semplice, in molti casi concreti la sua esistenza è un fatto altamente non banale.
\begin{example}
	\Todo{oggetto iniziale/terminale di una categoria degli elementi}
\end{example}
\begin{example}
	\Todo{oggetto terminale di coalgebre per $1+A\times\blank$}
\end{example}
% \color{red}
