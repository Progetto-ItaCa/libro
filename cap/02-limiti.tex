...
\begin{proposition}\label{term_coalgebra_maybe}
	...
	\[\xi_!(\emptyList) := \bullet \in 1 \qquad \xi_!(a\cons as) = (a, as) \in A\times A^\omega.\]
\end{proposition}
\begin{figure}
	\begin{center}
		\begin{tikzpicture}[
			...
			\node[dot] (x0) at (-2.5,1) {};
			\node[font=\tiny] (x1) at (-.5,1) {$(a_1,x_1)$};
			\node[font=\tiny] (x2) at (.5,1.5) {$(a_2,x_2)$};
			\node[font=\tiny] (x3) at (1.35,.275) {$(a_3,x_3)$};
			\node[font=\tiny] (x4) at (2.5,.725) {$(a_4,x_4)$};
			\draw[-latex] (x0) -- (x1)
			(x1) -- (x2)
			(x2) -- (x3)
			(x3) -- (x4);
...
\begin{proof}[Dimostrazione di \ref{term_coalgebra_maybe}]
	...
	\[\begin{cases}
			...
			x \overset \leadsto \bullet & \text{ se } \alpha(x) = \bullet.
		\end{cases}\]
...
\end{proof}
\begin{remark}
	Il nome `comportamento' per la freccia universale \(b_{(X,\alpha)}\) è dovuto al fatto che si può pensare alla successione \eqref{iter_coalgebra} come a una vera e propria traiettoria nello `spazio' \(X\times A\), tale che
	\[\xymatrix@R=0cm{
		x \ar@{|->}[r]& a_1\\&x_1 \ar@{|->}[r]& a_2\\&&x_2 \ar@{|->}[r]& \\&&&\dots &
		}\]
	La Figura \ref{} chiarisce questa idea intuitiva.
\end{remark}
% \color{red}
