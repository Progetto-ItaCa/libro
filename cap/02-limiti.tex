...
\begin{itemize}
 \item Il limite del diagramma \(D : \ctJ\fun\ctC\) è il `massimo dei minoranti' tra diagrammi della forma \eqref{}; in effetti, in un insieme ordinato (che sappiamo essere una categoria per \ref{ord_sonocat}), l'estremo inferiore di un sottoinsieme $S\subseteq P$ è un elemento $\inf S$ con la proprietà 
	\Todo{}
 \item Dualmente, il colimite del diagramma \(D : \ctJ\fun\ctC\) è il `minimo dei maggioranti' tra diagrammi della forma \eqref{}; in effetti, in un insieme ordinato l'estremo superiore di un sottoinsieme $S\subseteq P$ è un elemento $\sup S$ con la proprietà
 \Todo{}
\end{itemize}
In ogni insieme ordinato, quindi, la proprietà di essere estremo superiore	o inferiore si esprime come una proprietà universale, e si riduce alla nozione di colimite o limite in una categoria-preordine. La descrizione di limiti e colimiti in una categoria-preordine nel senso generale verrà delineata più in dettaglio in \ref{}, ma invitiamo già chi legge a immaginare come essa si possa sviluppare.\footnote{Un semplice esercizio per cui non serve alcun prerequisito	non elementare, è mostrare i fatti seguenti: se $\le$ è un preordine su $P$, un oggetto iniziale della categoria $(P,\le)$ è un elemento $\le$-minimo di $P$ o equivalentemente $\sup \varnothing$; dualmente, un oggetto terminale di $(P,\le)$ è un elemento $\le$-massimo di $P$, o equivalentemente $\inf\varnothing$; se l'ordine è antisimmetrico, poi, $\sup\varnothing$ e $\inf\varnothing$ sono \emph{strettamente} unici quando esistono.}

La teoria dei limiti e dei colimiti si può vedere come la teoria di estremi superiori e inferiori, per categorie che non sono preordini (cioè che non sono nell'immagine del funtore di \ref{ord_come_cat}).
...
\begin{hExamples}[Alcuni esempi di oggetti iniziali e terminali]{fund}
 ...
 \begin{itemize}
  ...
  \item Nella categoria degli anelli unitari \(\ctRing_1\) di \ref{varie_categorie_nella_pratica} (in cui $0\ne 1$ e gli omomorfismi rispettano entrambi, di modo che non esiste l'anello $\{0\}$), l'oggetto iniziale è l'anello degli interi \(\bbZ\): dato un anello unitario \(R\), un omomorfismo di anelli \(\varphi : \bbZ \to R\) deve mandare \(0\) in \(0_R\) e \(1\) in \(1_R\); del resto deve anche essere tale che \(\varphi(n) = \varphi(n\cdot 1) = n \cdot 1_R = \iter[1][+]1\) ed è quindi definito su tutti gli altri interi. Nella sottocategoria degli anelli con divisione, però, un oggetto iniziale non esiste: per quale motivo? (sugg.: quale dovrebbe essere la caratteristica di $\init_\ctFld$?)
  \item Nella categoria delle algebre di Boole di \ref{varie_categorie_nella_pratica}, l'oggetto iniziale è l'algebra con due elementi \(\bbB=\{0 < 1\}\) (con le operazioni di congiunzione, disgiunzione e complemento usuali): se $H$ è un'altra algebra di Boole, dato che un omomorfismo di algebre di Boole/Heyting deve mandare \(0\) in \(0\) e \(1\) in \(1\), deve esistere un unico $\bang_H : \bbB \to H$.
  \item Nella categoria dei sistemi dinamici di \ref{ex_cat_dyn} succede qualcosa di interessante, osservato nell'esercizio \ref{ex_monepi_3}: \Todo{}
...
\end{proposition}
Si noti l'aggettivo \emph{unico} nell'enunciato: se tra \(\term\) e \(\term'\) esiste uno e un solo isomorfismo, essi non sono uguali, ma (per così dire) `unici in un unico modo'. Non va però frainteso questo punto: l'isomorfismo è unico \emph{solo una volta fissati \(\term\) e \(\term'\)} e varia insieme ad essi.

La dimostrazione ricorda, in spirito, l'argomento che mostra che l'elemento neutro in un monoide (o l'identità \(\id_X\) di un oggetto in una categoria) è unico. 
\begin{proof}
...
\]
specificata insieme alle funzioni \(\pi_1,\pi_2\) di \emph{proiezione} sui due fattori \(A\) e \(B\), definite in \ref{}.

Anche il prodotto cartesiano di insiemi può essere caratterizzato da una proprietà che non riguarda direttamente i suoi elementi, ma li caratterizza in termini delle funzioni verso \(A\times B\). Per chiarire questo aspetto, consideriamo una funzione che ha \(A\times B\) come codominio,
\[\label{prodotti_h}
 h\colon X\to A\times B
\]
Essa può essere composta con le proiezioni \(\pi_1,\pi_2\) per ottenere due funzioni
\[\label{prodotti_f_e_g}
 \pi_1\cmp h\colon X\to A \qquad \pi_2\cmp h\colon X\to B
\]
...
\begin{itemize}
 \item grazie a \ref{eq_comprule} l'immagine di \(h(x)\) può essere \emph{ridotta} alla coppia ordinata
       \[\pair{(\pi_1\cmp h)(x)}{(\pi_2\cmp h)(x)};\]
 \item comunque siano date due funzioni \(f : X\to A\) e \(g : X \to B\), è possibile \emph{costruire} la coppia ordinata \(\pair{f(x)}{g(x)}\) e la funzione \(f\pmap g\) che abbiamo già introdotto in \ref{puniv_prodotto_naturale} che ne risulta è l'immagine di \(\pair fg\), nell'insieme a sinistra, rispetto alla biiezione natuarle (già dimostrata)
       \[\Hom\ctSet(X,A\times B)\cong\Hom\ctSet(X,A)\times\Hom\ctSet(X,B).\]
\end{itemize}
\Todo{}
\begin{remark}\label{rmk_prodotto}
 Questa proprietà è caratterizzante del prodotto cartesiano: una terna costituita da un insieme \(P\) e due funzioni \(p_1,p_2\) del tipo
 \[
 ...
 \]
\end{remark}
In effetti, è immediato verificare che \(A\times B\) e le proiezioni canoniche \(\pi_1,\pi_2\) soddisfano questa proprietà. Ma se \(P\) soddisfa questa proprietà, possiamo dimostrare quanto segue.
\begin{lemma}
 \Todo{Essenziale unicità del prodotto cartesiano}
 Se \(P\) soddisfa la proprietà enunciata in \ref{rmk_prodotto}, allora esiste un'unica biiezione \(P\cong A\times B\).
\end{lemma}
Si osservi di nuovo il ruolo dell'aggettivo \emph{unica}: fissate le terne \(()\) e \(()\) (e solo allora), la biiezione possibile è \emph{solo una}.
\begin{proof}
 \Todo{C'è una biiezione}
 \Todo{E' unica}
\end{proof}
Si osservi che in particolare, istanziando la proprietà di \ref{rmk_prodotto} quando \(X = \{\bullet\}\) è un insieme con un solo elemento, le funzioni \(g\colon \{*\}\to B\), \(f\colon \{*\}\to A\) scelgono rispettivamente un elemento \(g(\bullet) = b \in B\) e \(f(\bullet) = a \in A\); la funzione \(f\pmap g\colon \{*\}\to P\) sceglie a sua volta un elemento \((f\pmap g)(\bullet) = t \in P\), che però ora deve essere della forma \(\pair{p_1 t}{p_2 t}\).

La proprietà di \ref{} si traduce allora in: per ogni coppia di elementi \(a\in A\) e \(b\in B\), esiste (suriettività) un unico (iniettività) elemento \(c\in P\) tale che \(p_1 t=a\) e \(p_2 t=b\). Questo è un modo alternativo di stabilire una biiezione \(\tau : P\to A\times B\) soggetta alla proprietà addizionale di commutare con le proiezioni, cioè tale che \(\pi_1\cmp \tau =p_1\) e \(\pi_2\cmp \tau =p_2\).

In sintesi, abbiamo enunciato e dimostrato la
\begin{definition}[Proprietà universale del prodotto cartesiano di insiemi]
 \Todo{Proprietà universale del prodotto cartesiano di insiemi}
\end{definition}
\subsection{Prodotti in altre categorie `concrete'}
\Todo{in varie categorie algebriche, negli spazi topologici, nelle categorie di insiemi ordinati, in alcune piccole categorie di funtori... }
...
\]
che commuta con le proiezioni, cioè  tale che \(\pi_1\cmp\varphi=p_1\) e \(\pi_2\cmp\varphi=p_2\).
