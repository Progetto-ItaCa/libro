...
In maniera molto informale, una \emph{proprietà universale} è la proprietà di essere terminale in una opportuna categoria, e dualmente una proprietà \emph{couniversale} è la proprietà di essere iniziale.
...
che è terminale in una categoria i cui oggetti sono coni della stessa forma. Al variare del diagramma \(D\) (quanto è complesso il suo dominio, quanto è complessa la sua azione su oggetti e frecce dello stesso), varia la complessità interna dell'oggetto terminale, che riesce a descrivere oggetti molto strutturati (i numeri \(p\)-adici, si veda \ref{ex_lim_padici}; i frattali, si veda \ref{ex_lim_frattali}; i nuclei di omomorfismi, si veda \ref{ex_lim_kernel}; i grafici di funzioni reali o complesse, si veda \ref{ex_lim_grafici}; etc). L'oggetto \(X\) si dice allora il \emph{limite} del diagramma \(D\).
...
\Todo{molti esempi disseminati per il capitolo 1 sono stati architettati per evitare di usare il linguaggio dei limiti;	ora diventano molto più snelli.}
\begin{proposition}
