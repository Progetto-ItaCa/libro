...
\end{remark}
\begin{definition}\label{def_rifl_limiti}
 \Todo{riflessione di limiti}
\end{definition}
\begin{definition}\label{def_creaz_limiti}
 \Todo{Creazione di limiti}
\end{definition}
...
\end{theorem}
\begin{theorem}\label{rep_preservano_lim}
 \Todo{tutti i rappresentabili preservano i limiti}
\end{theorem}
\begin{theorem}
...
\end{corollary}
\begin{proposition}
 I funtori conservativi finitamente continui \(U\) sono fedeli, perché se \(f, g\colon X \rightrightarrows Y\) sono tali che \(U(f) = U(g)\), allora il morfismo mediatore

 \(U(\mathrm{Eq}(f, g)) \to \mathrm{Eq}(U(f), U(g)) = U(X)\)

 è un isomorfismo per preservazione dei limiti, e allora il morfismo canonico \(\mathrm{Eq}(f, g) \to X\) è un isomorfismo per la conservatività di \(U\), e ciò implica \(f = g\).
\end{proposition}
\begin{proof}
 \Todo{}
\end{proof}
...
\begin{proposition}[Esattezza e successioni esatte corte]\label{prop_esattezza_successioni_esatte_corte}\index{Successione esatta corta}\index{Quadrato esatto!relazione con successioni esatte}
 \Todo{}
\end{proposition}
Raccogliamo alcuni risultati sulle successioni esatte corte
\begin{proposition}
 Pullback di una s.e.s induce iso sui ker
\end{proposition}
\begin{proposition}
 Lemma dei 5 e dei 9
\end{proposition}
...
\begin{definition}[Diagramma iniziale, diagramma finale]\label{def_diagramma_iniziale_finale}\index{Diagramma!iniziale}\index{Diagramma!finale}\index{Funtore!iniziale}\index{Funtore!finale}
 \Todo{}
\end{definition}
Il contenuto di \ref{def_fun_initfin} e dei risultati seguenti, su funtori iniziali e finali, ora è in un contesto più comprensibile ed espressivo; invitiamo chi legge a riguardare a quegli esempi fatti senza molta tecnologia.

\begin{definition}[Densità e codensità di funtori e sottocategorie]\label{def_funtore_denso_codenso}\index{Funtore!denso}\index{Funtore!codenso}\index{Sottocategoria!densa}\index{Sottocategoria!codensa}
 \Todo{}
\end{definition}
