...
ossia di un oggetto iniziale in una categoria di coconi per \(D\). L'oggetto \(X\) si dice allora il \emph{colimite} del diagramma \(D\). Anche i colimiti riescono a descrivere oggetti molto strutturati (i numeri naturali, si veda \ref{ex_colim_nat}; i prodotti semidiretti di gruppi e monoidi, si veda \ref{exa_semidiretto}; la gerarchia cumulativa degli insiemi, si veda \ref{exa_gerarchia}; le liste a valori in un alfabeto \(S\), si veda \ref{ex_colim_liste}; i cosiddetti \emph{catamorfismi}, si veda \ref{ex_colim_cata};\dots).
...
\begin{itemize}\index{Gruppo!--- topologico}\index{Limite!--- di catena}\index{Numeri \(p\)-adici}\index{aaa_Zp@\(\bbZ_p\)}
 ...
 \item il motivo per cui su certi oggetti complicati è possibile imporre una grande varietà di strutture interagenti tra loro; ad esempio, si può costruire il \emph{gruppo profinito} degli interi \(p\)-adici (con \(p\in\bbZ\) un numero primo) come il limite \(\bbZ_p\) (che per ora va intuito come `quel termine che sta a destra di tutti i fattori \(\bbZ/p^n\bbZ\)') della catena
       \[\xymatrix{
       ...
       dove ciascuna mappa \(\varphi_n \colon \bbZ/p^n\bbZ \to \bbZ/p^{n-1}\bbZ\) è la mappa di riduzione modulo \(p\), suriettiva e di nucleo \(p^{n-1}\bbZ/p^n\bbZ\). Su \(\bbZ_p\) coesistono in armonia molte strutture: c'è innanzitutto una struttura di anello, dato che ogni termine della catena ha un'operazione di moltiplicazione; questo conferisce a \(\bbZ_p\) una proprietà universale nella categoria \(\ctRing\) degli anelli commutativi. C'è poi una topologia naturale su \(\bbZ_p\) (che lo rende uno \emph{spazio di Stone} oltre che un gruppo topologico) che è la meno fine che rende continue tutte le proiezioni canoniche \(\bbZ_p \to \bbZ/p^n\bbZ\); questo conferisce a \(\bbZ_p\) una proprietà universale nella categoria \(\ctTop\). Questo fenomeno è generale: su certi oggetti algebrici ottenuti come limiti di questo tipo, compare una topologia non discreta \emph{anche se tutte le componenti del diagramma sono spazi discreti}.

       Questa topologia ha un senso molto preciso, in termini della struttura algebrica di \(\bbZ_p\); gli elementi di \(\bbZ_p\) si possono vedere come `serie di potenze nella variabile \(p\)' nel senso che un elemento \(x\in\bbZ_p\) è una successione	\(\overline x = (x_0,x_1,x_2,x_3\dots)\) tale che per ogni \(n\ge 0\) si abbia \(x_n\equiv x_{n+1}\pmod{p^n}\); del resto, se \(x_0 = k_0+x_1\), \(x_1 = k_1p+x_2\), \(x_2 = k_2p^2+x_3\), \dots, si ha che
       \[\overline x \approx k_0 + k_1p + k_2p^2 + k_3p^3 + k_4p^4 + k_5p^5 + \dots \]
       Queste serie formali, o meglio, l'estensione di questo anello ai cosiddetti `numeri \(p\)-adici' \(\bbQ_p\) (ottenuti come campo di frazioni di \(\bbZ_p\)), sono di fondamentale importanza in algebra, geometria algebrica, e in teoria dei numeri (si vedano, ad esempio, i testi \cite{a,b,c}).
\end{itemize}
...
Data una famiglia di oggetti di \(\ctC\), soggetta a determinate relazioni (il `diagramma' in cui sono vere delle commutatività della forma \(\{a_i=b_i\mid i\in \ctJ\}\) --che per ora non definiamo in modo preciso); si può trovare `l'oggetto più grande che è soluzione delle equazioni imposte dalla relazioni'? (E qual è il significato di questa qualificazione?) Dualmente: si può trovare `l'oggetto più piccolo in cui le relazioni sono state forzate a essere vere'?

Il `massimo dei minoranti', cioè l'oggetto più grande che è soluzione delle equazioni imposte dalle relazioni, è quel che si dice il \emph{limite} del diagramma; il `minimo dei maggioranti', cioè l'oggetto più piccolo in cui le relazioni sono state forzate a essere vere, ne è il \emph{colimite}.

In questo senso, limiti e colimiti sono `soluzioni' a problemi di ottimizzazione, o di minimo-massimo, che coinvolgono oggetti e frecce di una categoria. Ma sono anche `soluzioni' a problemi di interdipendenza tra una serie di dati: un limite interagisce con gli altri oggetti del diagramma in modo tale che ogni vincolo \(a_i=b_i\) sia realizzato e accessibile per proiezione; un colimite, dualmente, è un oggetto che interagisce con gli altri oggetti del diagramma in modo che ogni vincolo \(a_i=b_i\) venga imposto, e la classe di equivalenza di ciascun termine, rispetto alla famiglia di quelle identificazioni, sia accessibile per immersione.
...
\begin{proof}
 ...
 Se \(F\natIso \Hom{\ctC}(X_F,\blank)\), allora \(\Elts{\ctC}F\) è isomorfa alla categoria succuba \(X_F/\ctC\), che ha un oggetto iniziale \((X_F,\id_{X_F})\) (cosa che è facile da verificare). Dunque, anche \(\Elts{\ctC}F\) ha un oggetto iniziale in virtù di \ref{equivs_preservano_init_term}.

 L'implicazione non ovvia è l'altra: supponiamo che \(\Elts{\ctC}F\) abbia un oggetto iniziale \((\init,\xi\in F\init)\), e ci proponiamo di costruire un isomorfismo naturale \(F\natIso \Hom{\ctC}(\init,\blank)\).

 Tale oggetto iniziale realizza la seguente proprietà:
 \begin{quote}
  Per ogni oggetto \((C,x)\) di \(\Elts{\ctC}F\), esiste un'unica freccia \(f \colon \init \to C\) in \(\ctC\) tale che \(Ff(\xi) = x\).
 \end{quote}
 Del resto questa proprietà è precisamente ciò che rende la funzione \(FC \to \Hom{\ctC}(\init,C)\) iniettiva (unicità di \(f\)) e suriettiva (esistenza di \(f\)) per ogni \(C\in\ctC_0\). Dunque, disponiamo di una famiglia di biiezioni
 \[\dmFun{\theta_C}{FC}{\Hom{\ctC}(\init,C)}\]
 una per ogni \(C\in\ctC_0\); resta da dimostrare che questa è una trasformazione naturale (per via di \ref{natiso_inversa}, poi, sarà naturale anche la sua inversa componente per componente, e quindi avremo concluso). Data una freccia \(u \colon C \to C'\) di \(\ctC\), dobbiamo dimostrare che il quadrato
 \[\xymatrix{
  FC \ar[r]^-{\theta_C}\ar[d]_{Fu} & \Hom{\ctC}(\init,C) \ar[d]^{u\cmp\blank} \\
  FC' \ar[r]_-{\theta_{C'}} & \Hom{\ctC}
  (\init,C')
  }\]
 è commutativo. Per questo è sufficiente verificare che la composizione \(u\cmp f\) è l'unica freccia \(f'=\theta_{C'}(Fu(x))\) tale che \(F(f')(\xi)=Fu(x)\); questo è ovvio:
 \[F(u\cmp f)(\xi) = (Fu\cmp Ff)(\xi) = Fu\bigl(Ff(\xi)\bigr)=Fu(x)\]
 per come \(f = \theta_C(x)\) è definita. Questo conclude la dimostrazione.
 %Sia \(x\in FC\); allora, per definizione di \(\theta_C\), esiste un'unica freccia \(f \colon \init \to C\) tale che \(Ff(\xi) = x\). Dunque, \(Fu(Ff(\xi)) = Fu(x)\), e allora \(F(u\cmp f)(\xi) = Fu(x)\). Ma \(u\cmp f \colon \init \to C'\) è una freccia in \(\ctC\), e quindi, per la proprietà di inizialità di \((\init,\xi)\), deve essere l'unica freccia con questa proprietà. Dunque, \(u\cmp f = \theta_{C'}(Fu(x))\), e il quadrato è commutativo.
\end{proof}
