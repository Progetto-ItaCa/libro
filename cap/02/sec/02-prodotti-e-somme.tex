...
\begin{hExample}[Regole di introduzione ed eliminazione per il prodotto Cartesiano]{fund}\label{intro_elim_prodotto}
 ...
 \begin{enumtag}{iep}
  \item \label{iep_1} ogni volta che viene dato un elemento \(t \in A\times B\), si ottiene un elemento \(\pi_A t\) la `prima componente di \(t\)';
  \item \label{iep_2} ogni volta che viene dato un elemento \(t \in A\times B\), si ottiene un elemento \(\pi_B t\) la `seconda componente di \(t\)';
  \item \label{iep_3} ogni volta che vengono dati due elementi \(a \in A, b \in B\), si ottiene un elemento \(\pair ab \in A\times B\);
  \item \label{iep_4} ogni elemento \(t \in A\times B\) è della forma \(\pair{\pi_A\, t}{\pi_B\, t}\).
 \end{enumtag}
 ...
 \begin{itemize}
  ...
  \item le deduzioni in \ref{intro_elim_prodotto} valgono per `lambda-astrazione' di \(t\), cioè vale l'uguaglianza
        \[{\pi_A}\pmap{\pi_B} = \id_{A\times B} \]
 \end{itemize}
\end{hExample}
In questi termini, il prodotto è descritto come una operazione di costruzione di un insieme a partire da due dati: questo è ragionevole, dato che abbiamo già visto in \ref{exam_bifuntori}.\ref{eb_2} che esiste un (bi)funtore
...
\begin{definition}\label{pruniv_prodotto_tfae}
 ...
 \begin{itemize}
  \item (proprietà universale diagrammatica) per ogni spanna
        ...
        esiste un unico omomorfismo di spanne denotato \(f\pmap g\)	che rende commutativo il diagramma
        \[\xymatrix{
         & X \ar@{.>}[d]|{f\pmap g} \ar[dl]_f \ar[dr]^g\\
...
\begin{proposition}\label{prodotto_terminal_span}
 Si rammenti la definizione della categoria \(\ctSpan(A,B)\) in \ref{def_span_e_cospan}, delle spanne di insiemi, che ha
 \begin{itemize}
...
\begin{definition}[Prodotto binario in \(\ctC\)]\label{def_prodotto_in_una_cat}
 ...
 \begin{enumtag}{pd}
  \item\label{pd_1} per ogni diagramma \(A \xot{z_A} Z \xto{z_B} B\), esiste un'unica freccia \({z_A}\pmap {z_B} \colon Z\to P\)	che rende commutativi i due triangoli del diagramma
  \[\xymatrix{
   & Z \ar@{.>}[d]|{{z_A}\pmap {z_B}} \ar[dl]_{z_A}\ar[dr]^{z_B}\\
...
\end{remark}
\begin{remark}\label{eqn_del_prodotto}
 Nella categoria degli insiemi è facile verificare le seguenti identità tra funzioni \(p\colon A\to B,q \colon A\to C,f \colon X\to Y,g \colon X\to Z\), \(u\colon B\to X\), \(v\colon C\to Y\):
 \[(u\times v)\cmp (p\pmap q) = (u\cmp p)\pmap (v\cmp q), \qquad f\pmap g = (f\times g)\cmp\Delta\]
 dove \(\Delta=\id_X\pmap \id_X\) è la diagonale \(x\mapsto \pair xx\) (e quindi la seconda equazione è un caso particolare della prima).

 Cioè, i diagrammi
 % https://q.uiver.app/#q=
 \[\begin{tikzcd}[cramped]
   A && X & {X\times X} \\
   {B\times C} & {X\times Y} && {Y\times Z}
   \arrow["{p\pmap q}"', from=1-1, to=2-1]
   \arrow["{up\pmap vq}"{description}, from=1-1, to=2-2]
   \arrow["\Delta", from=1-3, to=1-4]
   \arrow["{f\pmap g}"{description}, from=1-3, to=2-4]
   \arrow["{f\times g}", from=1-4, to=2-4]
   \arrow["{u\times v}"', from=2-1, to=2-2]
  \end{tikzcd}\]
 sono commutativi. Del resto è facile dimostrare identità analoghe (i diagrammi appena disegnati hanno senso in ogni categoria dove i prodotti esistano!) usando la proprietà universale: la composizione \((u\times v)\cmp (p\pmap q)\) soddisfa la proprietà universale di \((u\cmp p)\pmap (v\cmp q)\) perché
 \begin{align*}
  \underline{\pi_1\cmp (u\times v)}\cmp (p\pmap q) & = u\cmp \underline{\pi_1 \cmp (p\pmap q)} \\
                                                   & = u\cmp p;                                \\
  \underline{\pi_2\cmp (u\times v)}\cmp (p\pmap q) & =v\cmp \underline{\pi_2 \cmp (p\pmap q)}  \\
                                                   & =v\cmp q,
 \end{align*}
 come si può osservare seguendo le frecce nel diagramma
 % https://q.uiver.app/#q=
 \[\begin{tikzcd}[cramped]
   & A &&& \\
   && {B\times C} & {X\times Y} \\
   && C & Y \\
   B &&&& X
   \arrow["{p\pmap q}", from=1-2, to=2-3]
   \arrow["q"', curve={height=12pt}, from=1-2, to=3-3]
   \arrow["p"', curve={height=12pt}, from=1-2, to=4-1]
   \arrow["{u\times v}", from=2-3, to=2-4]
   \arrow["{\pi_2}"', from=2-3, to=3-3]
   \arrow["{\pi_1}", from=2-3, to=4-1]
   \arrow["{\pi_2}", from=2-4, to=3-4]
   \arrow["{\pi_1}", curve={height=-6pt}, from=2-4, to=4-5]
   \arrow["v"', from=3-3, to=3-4]
   \arrow["u", from=4-1, to=4-5]
  \end{tikzcd}\]
\end{remark}
\begin{remark}\label{congiunta_monicita}
 La proprietà universale di \ref{} implica che la spanna universale \(A\xot{\pi_1} A\times B \xto{\pi_2} B\) fatta dalle proiezioni sia una coppia \emph{congiuntamente mono}: questo significa che se \(\pi_1\cmp f  = \pi_1\cmp g\) e \(\pi_2\cmp f = \pi_2\cmp g\), allora \(f=g\) (ed è necessario che \emph{entrambe} le uguaglianze nella premessa siano vere).
\end{remark}
\subsection{Prodotti in altre categorie `concrete'}
I primi corsi di algebra hanno presentato diverse classi di strutture come monoidi, gruppi, anelli, e a volte reticoli algebrici, grafi, etc.; in ciascuna di queste strutture esiste una nozione di prodotto tra due oggetti, che è caratterizzata da una proprietà universale del tutto analoga a quella del prodotto Cartesiano di insiemi. Cioè: il prodotto di due gruppi (o anelli, spazi vettoriali, reticoli, grafi\dots) \(G\) e \(H\) si definisce ponendo sul prodotto Cartesiano \(G\times H\) degli insiemi sottostanti una operazione componente per componente, non perché questa definizione sia ovvia, ma perché è l'unica che soddisfa la giusta proprietà universale.
\begin{hExample}[Prodotti in categorie di insiemi strutturati]{fund}\label{prodotti_cat_strutturate}
 ...
 Il prodotto di due gruppi \(G,H\) si costruisce ponendo sul prodotto Cartesiano \(G\times H\) la moltiplicazione definita componente per componente,
 \[(g,h) \cdot_{G\times H} (g',h') \defeq (g\cdot_G g', h\cdot_H h');\]
 l'identità è ovviamente \((1_G,1_H)\), l'inverso di \((g,h)\) è \((g^{-1},h^{-1})\).

 Similmente si procede per definire il prodotto quando i due gruppi sono abeliani, nel caso di due spazi vettoriali \(V,W\), di due anelli, et cetera, fino al punto che è facile sospettare ci sia un teorema generale che afferma che in ogni categoria di modelli di una segnatura algebrica (come in \ref{}), il prodotto di due modelli	\(A\) e \(B\) si costruisce ponendo sul prodotto Cartesiano (fatto in \(\ctSet\)) dei loro supporti, una struttura definita componente per componente. Questo è vero: i prodotti in categorie di modelli di una segnatura algebrica, e in effetti tutti i limiti (si veda \ref{}) sono \emph{creati} (si veda \ref{}) dal funtore dimenticante di \ref{exa_funtori_liberi_forgetti},
 \[\dmFun{U}{\ctMod(\Omega,a)}{\ctSet}\]
\end{hExample}
\begin{hExample}[Prodotti nella categoria degli spazi topologici]{fund}\label{ex_prodotti_in_top}
 Nella categoria \(\ctTop\) degli spazi topologici e funzioni continue di \ref{ex_cat_top} il prodotto di due spazi topologici \((X,\tau_X),(Y,\tau_Y)\) si realizza ponendo sul prodotto Cartesiano degli insiemi \(X,Y\) la topologia \(\tau_X\times \tau_Y\) definita dalla prebase
 \[\{U\times Y\mid U\in \tau_X\}\cup\{X\times V\mid V\in \tau_Y\}\]
 (si prendono cioè unioni arbitrarie di intersezioni finite di elementi di elementi della forma sopra detta). Questa si dice \emph{topologia prodotto} o \emph{topologia di Tychonoff}. Alternativamente, ma solo nel caso in cui si considerino prodotti di un numero finito di spazi, si può considerare la topologia \(\tau_X\boxtimes\tau_Y\) delle `scatole' che ha per prebase
 \[\{U\times V\mid U\in \tau_X,V\in \tau_Y\}\]
 È facile notare che queste due prebasi danno luogo alla stessa topologia su \(X\times Y\); ma in generale la topologia delle scatole è quindi più fine (cioè, ha più aperti) della topologia di Tychonoff. Quando avremo introdotto i prodotti infiniti questo argomento abbozzato sarà chiaro: sia \(X=\bbR\) (o per quel che importa, un qualsiasi spazio con topologia né discreta né indiscreta); la funzione
 \[\xymatrix{\Delta : X \ar[r] & X^\bbN=\prod_{n\in\bbN} X}\]
 che viene indotta dall'identità su ogni componente non è continua nella topologia \(\tau_X^{\boxtimes\bbN}\) delle scatole: la continuità va controllata su troppi aperti per essere soddisfatta. Invece, la funzione è continua se il suo codominio è \((X^\bbN,\tau_X^{\times\bbN})\).
\end{hExample}
...
\end{hExample}
\begin{hExample}[Prodotti in categorie della forma \(\copresh\ctC\)]{fund}\label{ex_prodotti_in_copresh}
 \Todo{e in particolare nelle categorie di funtori che hanno una presentazione `concreta', come i grafi diretti.}
 In tutte le categoria della forma \(\copresh\ctC\) (e quindi nelle categorie di \ref{},\ref{}, \ref{}) il prodotto di due funtori \(F,G \colon\ctC\fun\ctSet\) si calcola `punto a punto' nel senso che enunciamo dopo aver espresso la proprietà universale in questo caso specifico:
 \begin{quote}
  Per ogni spanna di trasformazioni naturali\Todo{...}
 \end{quote}
 In particolare, vengono date delle trasformazioni naturali, fatte da molte componenti, e va indotta una trasformazione naturale, fatta da molte componenti, la cui naturalità non è a priori completamente immediata.

 D'altra parte, per fortuna, la questione si risolve in modo molto semplice: c'è un'unica scelta ovvia per definire la trasformazione naturale \(\alpha\pmap\beta \colon H \natto F\times G\), la cui componente in \(C\in\ctC_0\) è la funzione
 \[\xymatrix{
   {\alpha_C}\pmap{\beta_C} : HC \ar[r] & FC\times GC
  }\]
 La condizione che \(\alpha\pmap\beta\) così definita sia naturale equivale a chiedere che dato \(u \colon C\to C'\) in \(\hom(\ctC)\) il quadrato di funzioni
 \[\xymatrix{
  HC \ar[r]^-{\alpha\pmap\beta_C} \ar[d]_{Hu} & FC\times GC \ar[d]^-{Fu\times Gu} \\
  HC' \ar[r]_-{\alpha\pmap\beta_{C'}} & FC'\times GC'
  }\]
 sia commutativo; del resto, se \(x\in HC\) si ha
 \begin{align*}
  \bigl((Fu\times Gu)\cmp\alpha\pmap\beta_C\bigr)(x) & = \bigl( Fu(\alpha_C(x)) , Gu(\beta_C(x)) \bigr)       \\
                                                     & = \bigl( \alpha_{C'}(Hu(x)) , \beta_{C'}(Hu(x)) \bigr) \\
                                                     & = {\alpha_{C'}}\pmap{\beta_{C'}}(Hu(x)).
 \end{align*}
 In particolare, nella categoria dei multidigrafi di \ref{}, sappiamo che il prodotto si calcola nel modo seguente:
 il multidigrafo prodotto \((G_0,G_1)\times(H_0.H_1)\) \emph{deve} avere per vertici il prodotto cartesiano dei vertici, e per lati il prodotto cartesiano degli insiemi dei lati; dominio e codominio vengono allora dati come le funzioni
 \[\xymatrix{
   s_G\pmap s_H,t_G\pmap t_H:  G_1\times H_1\ar@<.3em>[r]\ar@<-.3em>[r] & G_0\times H_0
  }\]
  Questa maniera di ragionare verrà ampiamente generalizzata da \ref{}, dove mostreremo che tutti i limiti di $\presh\ctC$ si calcolano `punto a punto' (e dunque, in particolare, recupereremo quanto appena dimostrato).
\end{hExample}
Altre maniere di definire prodotti tra grafi\footnote{Ad esempio: potremmo dire che esiste un lato \((x,y) \to (x',y')\) se e solo se \(x=x'\) ed esiste un lato \(y\to y'\) oppure esiste un lato \(x\to x'\) e \(y=y'\).} sono interessanti per altri motivi, ma non realizzano la proprietà universale del prodotto nella categoria \(\ctGph\).
\begin{figure}
 \Todo{figura}
 \caption{Diversi tipi di prodotti tra grafi; solo il prodotto \(\ctG\times\ctH\) realizza la proprietà universale giusta; il prodotto \(\ctG\boxdot\ctH\) ha altre interessanti proprietà \cite{foo}.}
\end{figure}
...
\begin{example}[Prodotti in \(\ctRel\)]\label{ex_prodotti_in_rel}
 ...
 \[p_A(t)=\begin{cases}
  ...
  p_B(t)=\begin{cases}
   ...
   \{t\}     & t\in B
  \end{cases}\]
  Invitiamo chi legge a controllare, per esercizio, che questa spanna soddisfa davvero la proprietà universale del prodotto!
\end{example}
\begin{example}[Prodotti di pseudoinsiemi]\label{ex_prodotti_in_bishop}
 ...
 La proprietà universale è soddisfatta: se \(g \colon (Z,\approx)\to (Y,\sim_Y)\) e \(f \colon (Z,\approx)\to (X,\sim_X)\) sono funzioni che preservano le rispettive relazioni di equivalenza, allora la funzione \(f\pmap g \colon Z\to X\times Y\) definita da \(f\pmap g(z) = (f(z),g(z))\) è l'unica tale che
...
\begin{example}[Prodotti nell'incubo \(\ctSet/A\), e nella succuba \(A/\ctSet\)]\label{ex_prodotti_in_succuba_incubo}
 ...
 \] con la proprietà che \(g\cmp r = f\cmp l=h\); ma allora la funzione \(l\pmap h \colon Z\to X\times Y\) fattorizza sul sottoggetto \(\pull XYA\) come è stato definito, perché questa condizione è precisamente necessaria e sufficiente affinché \((l(z),r(z)) \in \pull XYA\).

 Sarà la sezione \ref{}, sui \emph{prodotti fibrati}, a chiarire la questione in modo definitivo.
\end{example}
\begin{example}[Prodotti in una categoria-poset]\label{ex_prodotti_in_catcomepos}
 La proprietà universale del prodotto, in una categoria-ordine nel senso di \ref{}, asserisce che $x\times y$ è un elemento \(z\) tale che \(p\le x\) e \(p\le y\), e se \(w\le x\) e \(w\le y\) allora \(w\le p\). Ma allora, \(z\) è un prodotto di \(x\) e \(y\) se e solo se \(z\) è l'\emph{estremo inferiore} $x\land y$ di \(x\) e \(y\), nel senso già noto dalla teoria degli insiemi ordinati (una notazione piuttosto comune per tale estremo inferiore, insieme al simbolo infisso $x\land y$, è $\inf\{x,y\}$ o $\glb\{x,y\}$ --in inglese, \emph{greatest lower bound}, che si può tradurre come \emph{massimo dei minoranti}).
