...
Nella categoria di insiemi e funzioni, l'\emph{equalizzatore} di due frecce \(f,g \colon A \toto B\) è il sottoinsieme \(E(f,g) \defeq \{a\in A\mid fa=ga\} \subseteq A\). È evidente il motivo per cui questa nozione è importante: permette di esprimere diagrammaticamente il luogo delle soluzioni, o luogo degli zeri, di un'equazione o famiglia di equazioni. In effetti, un `sistema di equazioni' \(\{f_i=g_i\}\) per \(f_i,g_i \colon A_i \to B_i\), indicizzate da un insieme \(I\), si può pensare come una famiglia di uguaglianze della forma \(f_i = g_i\), una per ogni \(i\in I\). Allora, l'insieme
\[\{a\in A\mid \forall i\in I\colon f_ia=g_ia\}\]
è un sottoinsieme \(\prod_{i\in I} E(f_i,g_i) \subseteq \prod_{i\in I} A_i\). Questa semplice osservazione, debitamente generalizzata, sarà di fondamentale importanza per il calcolo e la caratterizzazione dei limiti in categorie generiche.
\subsection{Equalizzatori}
\begin{definition}[Equalizzatore di due funzioni]
 Siano \(f,g\colon A\toto B\) due funzioni; l'\emph{equalizzatore} di \(f\) e \(g\) si definisce come la coppia \((E(f,g),e)\) dove \(E(f,g)=\{a\in A\mid fa=ga\}\subseteq A\) e \(e\colon E(f,g)\to A\) è l'inclusione insiemistica.
\end{definition}
\begin{remark}
 \begin{itemize}
  \item L'insieme
...
\end{example}
\begin{remark}
 Una importante osservazione sulla non-esistenza di equalizzatori in certe categorie dove gli oggetti devono essere `lisci'.
\end{remark}
\subsection{Coequalizzatori}\label{subsec_coeqs_set}
...
\Todo{}
Più in generale,
\begin{definition}\label{rel_equiv_generata}
 \Todo{relazione di equivalenza generata da \(R\)}
\end{definition}
\begin{remark}
...
\begin{example}\label{ex_eqs_in_grpabmod}
 \Todo{Equalizzatori in Grp, Ab, RMod ...}
\end{example}
\begin{example}\label{ex_eqs_in_topposptset}
 \Todo{Equalizzatori in \(\ctTop\), \(\ctPos\), \(\ctSet_*\)}
\end{example}
\begin{example}\label{ex_coeqs_in_grpabmod}
 \Todo{Coequalizzatori in Grp, Ab, RMod ...}
\end{example}
\begin{example}\label{ex_coeqs_in_topposptset}
 \Todo{Coequalizzatori in Top, Pos, pointed sets ...}
\end{example}
\begin{example}\label{ex_ses_come_universali}
 Successioni corte esatte a destra, esatte a sinistra, esatte.
\end{example}
% \begin{example}
...
\begin{example}\label{ex_lim_grafici}
 Nella categoria degli insiemi, il \emph{grafico} di una funzione \(f\) si può esprimere come prodotto fibrato. Infatti, se \(f \colon A\to B\) è una funzione, il grafico di \(f\) è precisamente l'insieme delle coppie \((a,b)\in A\times B\) tali che \(f(a)=b\), cioè il prodotto fibrato \(\pull ABB\) sopra $f$ e $\id_B$:
 \[\xymatrix{
   \{(a,b)\mid b=f(a)\} \ar[r]\ar[d] & B \ar[d]^{\id_B} \\
   A \ar[r]_{f} & B
  }\]
\end{example}
\begin{notation}\label{exa_cart_quadrati}\index{Cartesiano!quadrato ---o}
 \Todo{Quadrati `Cartesiani' e `coCartesiani'}
\end{notation}
\begin{remark}
  \Todo{Proprietà universale del prodotto fibrato e nozione di freccia Cartesiana}
\end{remark}
\begin{example}\label{exa_semidiretto}\index{Prodotto semidiretto}
 \Todo{Prodotto semidiretto come somma amalgamata}
\end{example}
\begin{example}\label{exa_vanchiappe}\index{van Kampen@Egbert van Kampen}
 \Todo{Teorema di van Kampen}
\end{example}
\begin{example}\label{exa_quoziente_R_su_I}\index{Quozienti!--- di anelli}
 Quozienti di anelli per ideali, come somme amalgamate
\end{example}
\begin{esercizi}
 \item
 \item
