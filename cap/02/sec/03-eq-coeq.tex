...
\begin{example}\label{ex_lim_grafici}
 Nella categoria degli insiemi, il grafico di una funzione \(f\) si può esprimere come un prodotto fibrato. Infatti, se \(f \colon A\to B\) è una funzione, il grafico di \(f\) è precisamente l'insieme delle coppie \((a,b)\in A\times B\) tali che \(f(a)=b\), cioè il prodotto fibrato \(\pull ABB\):
 \[\xymatrix{
...
\begin{notation}\label{exa_cart_quadrati}\index{Cartesiano!quadrato ---o}
 Quadrati `Cartesiani' e `coCartesiani'
\end{notation}
\begin{example}\label{exa_semidiretto}\index{Prodotto semidiretto}
 Prodotto semidiretto come somma amalgamata
\end{example}
